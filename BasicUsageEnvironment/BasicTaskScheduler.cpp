...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
    ...
    if (fEventTriggersAreBeingUsed) {
        // Look for an event trigger that needs handling (making sure that we
        // make forward progress through all possible triggers):
...
void BasicTaskScheduler ::setBackgroundHandling(
    int socketNum, int conditionSet, BackgroundHandlerProc* handlerProc,
    void* clientData) {
