...
int main(int argc, char** argv) {
    ...
    // comment out the "return 0;" statement above), and if you don't intend to
    // do anything more with the "TaskScheduler" and "UsageEnvironment" objects,
...
void shutdownStream(RTSPClient* rtspClient, int exitCode) {
    ...
    if (--rtspClientCount == 0) {
        ...
        // you might want to comment this out, and replace it with
        // "eventLoopWatchVariable = 1;", so that we leave the LIVE555 event
        // loop, and continue running "main()".)
        exit(exitCode);
    }
}
