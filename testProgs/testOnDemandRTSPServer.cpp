...
#define STREAM_USING_SRTP True
    ...
    // A MPEG-1 or 2 video elementary stream:
    {
        ...
        sms->addSubsession(MPEG1or2VideoFileServerMediaSubsession ::createNew(
            *env, inputFileName, reuseFirstSource, iFramesOnly));
        rtspServer->addServerMediaSession(sms);
    ...
    // A MPEG-2 Transport Stream:
    {
        ...
        sms->addSubsession(MPEG2TransportFileServerMediaSubsession ::createNew(
            *env, inputFileName, indexFileName, reuseFirstSource));
        rtspServer->addServerMediaSession(sms);
    ...
    // A Matroska ('.mkv') file, with video+audio+subtitle streams:
    {
        ...
        // otherwise, because the stream has no tracks, we don't add a
    ...
    // (Note: ".webm' files are special types of Matroska files, so we use the
    // same code as the Matroska ('.mkv') file code above.)
    {
    ...
    // (Note: ".opus' files are special types of Ogg files, so we use the same
    // code as the Ogg ('.ogg') file code above.)
    {
    ...
    // source:
    {
        ...
        // This causes the server to take its input from the stream sent by the
        // "testMPEG2TransportStreamer" demo application. (Note: If the input
        // UDP source is unicast rather than multicast, then change this to
        // NULL.)
        ...
        sms->addSubsession(MPEG2TransportUDPServerMediaSubsession ::createNew(
            *env, inputAddressStr, inputPortNum, inputStreamIsRawUDP));
        rtspServer->addServerMediaSession(sms);
