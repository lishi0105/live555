...
void play() {
    ...
    *env << bitsPerSecond << " bits-per-second\n";
    ...
    unsigned char payloadFormatCode =
        96;  // by default, unless a static RTP payload type can be used
    ...
    if (audioFormat == WA_PCM) {
        if (bitsPerSample == 16) {
            ...
            // Add a filter that converts from raw 16-bit PCM audio (in
            // little-endian order) to 8-bit u-law audio:
            sessionState.source = uLawFromPCMAudioSource::createNew(
                *env, wavSource, 1 /*little-endian*/);
            if (sessionState.source == NULL) {
                *env << "Unable to create a u-law filter from the PCM audio "
                        "source: "
                     << env->getResultMsg() << "\n";
                exit(1);
            }
            ...
            *env << "Converting to 8-bit u-law audio for streaming => "
            ...
            if (sessionState.source == NULL) {
                *env << "Unable to create a little->bit-endian order filter "
                        "from the PCM audio source: "
                     << env->getResultMsg() << "\n";
                exit(1);
            }
        ...
        } else {  // bitsPerSample == 8 (we assume that bitsPerSample == 4 is
        ...
        *env << "Unknown audio format code \"" << audioFormat
             << "\" in WAV file header\n";
        exit(1);
    }
    ...
    // application, or the "LIVE555 Media Server" - not this application - as a
    // model.
    ...
    // 'groupsock':
    sessionState.sink = SimpleRTPSink::createNew(
        *env, sessionState.rtpGroupsock, payloadFormatCode, samplingFrequency,
        "audio", mimeType, numChannels);

    // Create (and start) a 'RTCP instance' for this RTP sink:
    const unsigned estimatedSessionBandwidth =
        (bitsPerSecond + 500) / 1000;  // in kbps; for RTCP b/w share
