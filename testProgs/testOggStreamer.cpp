...
void onOggFileCreation(OggFile* newFile, void* clientData) {
    ...
    for (unsigned i = 0; i < numTracks; ++i) {
        ...
        trackState[i].source = oggFile->createSourceForStreaming(
        ...
        if (trackState[i].source != NULL) {
            ...
            trackState[i].sink = oggFile->createRTPSinkForTrackNumber(
    ...
    if (sms->numSubsessions() == 0) {
        *env << "Error: The Ogg file \"" << inputFileName
             << "\" has no streamable tracks\n";
        *env << "(Perhaps the file does not exist, is not an 'Ogg' file, or "
                "has no tracks that we know how to stream.)\n";
        exit(1);
    }
...
void afterPlaying(void* /*clientData*/) {
    *env << "...done reading from file\n";
    ...
    // Create a new demultiplexor from our Ogg file, then new data sources for
    // each track:
    oggDemux = oggFile->newDemux();
    for (i = 0; i < numTracks; ++i) {
