...
Boolean parseMikeyUnknown(u_int8_t const*& /*ptr*/, u_int8_t const* /*endPtr*/,
...
#define testSize(n)                           \
    do {                                      \
        if (ptr + (n) > endPtr) return False; \
    } while (0)

Boolean parseMikeyHDR(u_int8_t const*& ptr, u_int8_t const* endPtr,
                      u_int8_t& nextPayloadType) {
    ...
    fprintf(stderr, "\tCS ID map type:%d (%s)\n", CS_ID_map_type,
            CS_ID_map_type == 0 ? "SRTP-ID" : "unknown");
    if (CS_ID_map_type != 0) return False;

    fprintf(stderr, "\tCS ID map info:\n");
    testSize(numCryptoSessions *
             (1 + 4 + 4));  // the size of the "CS ID map info"
    for (u_int8_t i = 1; i <= numCryptoSessions; ++i) {
        ...
        fprintf(stderr,
                "\tPolicy_no_%d: %d;\tSSRC_%d: 0x%08x; ROC_%d: 0x%08x\n", i,
                policy_no, i, ssrc, i, roc);
    }

    return True;
}

static Boolean parseKeyDataSubPayload(u_int8_t const*& ptr,
...
Boolean parseMikeyKEMAC(u_int8_t const*& ptr, u_int8_t const* endPtr,
                        u_int8_t& nextPayloadType) {
    ...
    fprintf(stderr, "\tEncr alg: %d (%s)\n", encrAlg,
            encrAlg == 0   ? "NULL"
            : encrAlg == 1 ? "AES-CM-128"
            : encrAlg == 2 ? "AES-KW-128"
                           : "unknown");
...
Boolean parseMikeyT(u_int8_t const*& ptr, u_int8_t const* endPtr,
                    u_int8_t& nextPayloadType) {
...
static char const* SRTPPolicyParamTypeExplanation[] = {
    "Encryption algorithm",       "Session Encryption key length",
    "Authentication algorithm",   "Session Authentication key length",
    "Session Salt key length",    "SRTP Pseudo Random Function",
    "Key derivation rate",        "SRTP encryption off/on",
    "SRTCP encryption off/on",    "Sender's FEC order",
    "SRTP authentication off/on", "Authentication tag length",
    "SRTP prefix length",
};

static Boolean parseSRTPPolicyParam(u_int8_t const*& ptr,
                                    u_int8_t const* endPtr) {
    ...
    while (ptr < endPtr) {
        ...
        fprintf(stderr, "\t\ttype: %d (%s); ", ppType,
                ppType > MAX_SRTP_POLICY_PARAM_TYPE
                    ? "unknown"
                    : SRTPPolicyParamTypeExplanation[ppType]);
...
Boolean parseMikeySP(u_int8_t const*& ptr, u_int8_t const* endPtr,
                     u_int8_t& nextPayloadType) {
...
Boolean parseMikeyRAND(u_int8_t const*& ptr, u_int8_t const* endPtr,
                       u_int8_t& nextPayloadType) {
...
typedef Boolean(parseMikeyPayloadFunc)(u_int8_t const*& ptr,
                                       u_int8_t const* endPtr,
                                       u_int8_t& nextPayloadType);
...
int main(int argc, char** argv) {
    ...
    do {
        ...
        while (nextPayloadType != 0 /* Last payload */) {
            fprintf(stderr, "%s:\n", payloadTypeName[nextPayloadType]);
            if (!(*payloadParser[nextPayloadType])(ptr, endPtr,
                                                   nextPayloadType))
                break;
        }
    } while (0);
