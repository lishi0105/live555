...
// A program that converts a H.264 (Elementary Stream) video file into sequence
// of HLS (HTTP Live Streaming) segments, plus a ".m3u8" file that can be
// accessed via a web browser. main program
...
int main(int argc, char** argv) {
    ...
    // Create a 'framer' filter for this file source, to generate presentation
    // times for each NAL unit:
    H264VideoStreamFramer* framer = H264VideoStreamFramer::createNew(
        *env, inputSource, True /*includeStartCodeInOutput*/,
    ...
    MediaSink* outputSink =
        HLSSegmenter::createNew(*env, OUR_HLS_SEGMENTATION_DURATION,
                                OUR_HLS_FILENAME_PREFIX, segmentationCallback);
...
void segmentationCallback(void* /*clientData*/, char const* segmentFileName,
    ...
    if (ourM3U8Fid == NULL) {
        ...
        char* ourM3U8FileName = new char[strlen(OUR_HLS_FILENAME_PREFIX) +
                                         5 /*strlen(".m3u8")*/ + 1];
