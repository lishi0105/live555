...
// A program that tests 'trick mode' operations on a MPEG-2 Transport Stream
// file, by generating a new Transport Stream file that represents the result of
// the 'trick mode' operation (seeking and/or fast forward/reverse play). For
// this to work, there must also be an index file present, in the same directory
...
void usage() {
    ...
    *env << "\t\t<start-time> is the starting play time in seconds (0 for the "
            "start)\n";
    *env << "\t\t<scale> is a non-zero integer, representing the playing speed "
            "(use 1 for normal play; use a negative number for reverse play)\n";
    exit(1);
}
...
int main(int argc, char const** argv) {
    ...
    FramedSource* input = ByteStreamFileSource::createNew(
        *env, inputFileName, TRANSPORT_PACKET_SIZE);
    if (input == NULL) {
        *env << "Failed to open input file \"" << inputFileName
             << "\" (does it exist?)\n";
        exit(1);
    }

    // Check whether the corresponding index file exists.
    // The index file name is the same as the input file name, except with
    // suffix ".tsx":
    ...
    if (indexFile == NULL) {
        *env << "Failed to open index file \"" << indexFileName
             << "\" (does it exist?)\n";
        exit(1);
    }

    // Create a filter that generates trick mode data from the input and index
    // files:
    MPEG2TransportStreamTrickModeFilter* trickModeFilter =
        MPEG2TransportStreamTrickModeFilter::createNew(*env, input, indexFile,
                                                       scale);

    if (startTime > 0.0f) {
        // Seek the input Transport Stream and Index files to the specified
        // start time:
        unsigned long tsRecordNumber, indexRecordNumber;
        indexFile->lookupTSPacketNumFromNPT(startTime, tsRecordNumber,
                                            indexRecordNumber);
        if (!trickModeFilter->seekTo(tsRecordNumber,
                                     indexRecordNumber)) {  // TARFU!
            *env << "Failed to seek trick mode filter to ts #"
                 << (unsigned)tsRecordNumber << ", ix #"
                 << (unsigned)indexRecordNumber << "(for time " << startTime
                 << ")\n";
            exit(1);
        }
    }
    ...
    newTransportStream->addNewVideoSource(trickModeFilter,
                                          indexFile->mpegVersion());
    ...
    *env << "Writing output file \"" << outputFileName << "\" (start time "
         << startTime << ", scale " << scale << ")...";
