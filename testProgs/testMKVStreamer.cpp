...
// A test program that reads a ".mkv" (i.e., Matroska) file, demultiplexes each
// track (video, audio, subtitles), and streams each track using RTP multicast.
// main program
...
// An array of structures representing the state of the video, audio, and
// subtitle tracks:
static struct {
...
int main(int argc, char** argv) {
    ...
    ((struct sockaddr_in&)destinationAddress).sin_addr.s_addr =
        chooseRandomIPv4SSMAddress(*env);
    // Note: This is a multicast address.  If you wish instead to stream
    // using unicast, then you should use the "testOnDemandRTSPServer"
    // test program - not this test program - as a model.

    // Create our RTSP server.  (Receivers will need to use RTSP to access the
    // stream.)
    ...
    // (Note that this object is not created immediately, but instead via a
    // callback.)
...
void onMatroskaFileCreation(MatroskaFile* newFile, void* /*clientData*/) {
    ...
    // Create source streams, "RTPSink"s, and "RTCPInstance"s for each preferred
    // track;
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        trackState[i].source = matroskaFile->createSourceForStreaming(
            baseSource, trackNumber, estBitrate, numFiltersInFrontOfTrack);
        trackState[i].sink = NULL;  // by default; may get changed below
        trackState[i].rtcp = NULL;  // ditto

        if (trackState[i].source != NULL) {
            Groupsock* rtpGroupsock =
                new Groupsock(*env, destinationAddress, rtpPortNum, ttl);
            Groupsock* rtcpGroupsock =
                new Groupsock(*env, destinationAddress, rtpPortNum + 1, ttl);
            rtpPortNum += 2;

            trackState[i].sink = matroskaFile->createRTPSinkForTrackNumber(
                trackNumber, rtpGroupsock, 96 + i);
            if (trackState[i].sink != NULL) {
                if (trackState[i].sink->estimatedBitrate() > 0) {
                    estBitrate =
                        trackState[i].sink->estimatedBitrate();  // hack
                }
                trackState[i].rtcp = RTCPInstance::createNew(
                    *env, rtcpGroupsock, estBitrate, CNAME, trackState[i].sink,
                    NULL /* we're a server */, True /* we're a SSM source */);
                // Note: This starts RTCP running automatically

                // Having set up a track for streaming, add it to our RTSP
                // server's "ServerMediaSession":
                sms->addSubsession(PassiveServerMediaSubsession::createNew(
                    *trackState[i].sink, trackState[i].rtcp));
            }
        }
    }

    if (sms->numSubsessions() == 0) {
        *env << "Error: The Matroska file \"" << inputFileName
             << "\" has no streamable tracks\n";
        *env << "(Perhaps the file does not exist, or is not a 'Matroska' "
                "file.)\n";
        exit(1);
    }
...
void afterPlaying(void* /*clientData*/) {
    *env << "...done reading from file\n";
    ...
    // Create a new demultiplexor from our Matroska file, then new data sources
    // for each track:
    matroskaDemux = matroskaFile->newDemux();
    for (i = 0; i < 3; ++i) {
        if (trackState[i].trackNumber != 0) {
            FramedSource* baseSource =
                matroskaDemux->newDemuxedTrackByTrackNumber(
                    trackState[i].trackNumber);

            unsigned estBitrate, numFiltersInFrontOfTrack;
            trackState[i].source = matroskaFile->createSourceForStreaming(
                baseSource, trackState[i].trackNumber, estBitrate,
    ...
    *env << "Beginning to read from file...\n";
    ...
    for (unsigned i = 0; i < 3; ++i) {
        if (trackState[i].sink != NULL && trackState[i].source != NULL) {
            trackState[i].sink->startPlaying(*trackState[i].source,
                                             afterPlaying, NULL);
        }
    }
}
