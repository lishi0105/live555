...
// A demo application that receives a UDP multicast stream, replicates it (using
// the "StreamReplicator" class), and retransmits one replica stream to another
// (multicast or unicast) address & port, and writes the other replica stream to
// a file.
//
// main program
...
void startReplicaUDPSink(StreamReplicator* replicator,
                         ...
                         portNumBits outputPortNum);  // forward
void startReplicaFileSink(StreamReplicator* replicator,
                          char const* outputFileName);  // forward
...
int main(int argc, char** argv) {
    ...
    unsigned char const inputTTL =
        0;  // we're only reading from this mcast group
    ...
    // input stream, and start it. If you wish, you can duplicate this line -
    // with different network addresses and ports - to create multiple output
    // UDP streams:
    startReplicaUDPSink(replicator, "239.255.43.43", 4444);

    // Then create a file 'sink' object to receive a replica of the input
    // stream, and start it. If you wish, you can duplicate this line - with a
    // different file name - to create multiple output files:
    startReplicaFileSink(replicator, "test.out");

    // Finally, enter the 'event loop' (which is where most of the 'real work'
    // in a LIVE555-based application gets done):
...
void startReplicaUDPSink(StreamReplicator* replicator,
    ...
    Groupsock* outputGroupsock =
        new Groupsock(*env, outputAddress, outputPort, outputTTL);
    ...
    MediaSink* sink =
        BasicUDPSink::createNew(*env, outputGroupsock, maxPacketSize);
