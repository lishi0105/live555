...
// client functionality into your own application), then we don't recommend
// using this code as a model, because it is too complex (with many options).
...
void continueAfterOPTIONS(RTSPClient* client, int resultCode,
                          char* resultString);
void continueAfterDESCRIBE(RTSPClient* client, int resultCode,
                           char* resultString);
...
void continueAfterTEARDOWN(RTSPClient* client, int resultCode,
                           char* resultString);
...
void usage() {
    *env << "Usage: " << progName
         << " [-p <startPortNum>] [-r|-q|-4|-i] [-a|-v] [-V] [-d <duration>] "
            "[-D <max-inter-packet-gap-time> [-c] [-S <offset>] [-n] [-O]"
         << (controlConnectionUsesTCP ? " [-t|-T <http-port>]" : "")
         << " [-u <username> <password>"
         << (allowProxyServers ? " [<proxy-server> [<proxy-server-port>]]" : "")
         << "]"
         << (supportCodecSelection
                 ? " [-A <audio-codec-rtp-payload-format-code>|-M "
                   "<mime-subtype-name>]"
                 : "")
...
int main(int argc, char** argv) {
    ...
    while (argc > 1) {
        ...
        switch (opt[1]) {
            ...
            case 'r': {  // do not receive data (instead, just 'play' the
                         // stream(s))
            ...
            case 'I': {  // specify input interface...
                ...
                if (interfaceAddress.ss_family == AF_INET) {
                    ReceivingInterfaceAddr =
                        ((sockaddr_in&)interfaceAddress).sin_addr.s_addr;
                }
                if (interfaceAddress.ss_family == AF_INET6) {
                    ReceivingInterfaceAddr6 =
                        ((sockaddr_in6&)interfaceAddress).sin6_addr;
                }
                ++argv;
                --argc;
                break;
            ...
            case 'L': {  // receive/record an 'application' (e.g., metadata)
                         // stream only
            ...
            case 'd': {  // specify duration, or how much to delay after end
                         // time
                float arg;
                ...
                if (argv[2][0] ==
                    ...
                    // a 'negative' argument was specified; use this for
                    // "durationSlop":
            ...
            case 'D': {  // specify maximum number of seconds to wait for
                         // packets:
            ...
            case 'P': {  // specify an interval (in seconds) between writing
                         // successive output files
            ...
            case 'T': {
                ...
                if (controlConnectionUsesTCP) {
                    if (argc > 3 && argv[2][0] != '-') {
                        ...
                        if (sscanf(argv[2], "%hu", &tunnelOverHTTPPortNum) ==
                                1 &&
            ...
            case 'k': {  // specify a username and password to be used to
            ...
            case 'K': {  // Send periodic 'keep-alive' requests to keep broken
                         // server sessions alive
            ...
            case 'M': {  // specify a MIME subtype for a dynamic RTP payload
                         // type
                mimeSubtype = argv[2];
            ...
            case 'w': {  // specify a width (pixels) for an output QuickTime or
                         // AVI movie
            ...
            case 'h': {  // specify a height (pixels) for an output QuickTime or
                         // AVI movie
            ...
            case 'f': {  // specify a frame rate (per second) for an output QT
                         // or AVI movie
            ...
            case 'F': {  // specify a prefix for the audio and video output
                         // files
            ...
            case 'g': {  // specify a user agent name to use in outgoing
                         // requests
                userAgent = argv[2];
                ++argv;
                --argc;
            ...
            // Note: The following option is deprecated, and may someday be
            // removed:
            case 'l': {  // try to compensate for packet loss by repeating
                         // frames
            ...
            case 'H': {  // generate hint tracks (as well as the regular data
                         // tracks)
            ...
            case 'U': {
                // specify initial absolute seek time (trick play), using a
                // string of the form "YYYYMMDDTHHMMSSZ" or
    ...
    if (outputCompositeFile && !movieWidthOptionSet) {
        *env << "Warning: The -q, -4 or -i option was used, but not -w.  "
                "Assuming a video width of "
             << movieWidth << " pixels\n";
    }
    if (outputCompositeFile && !movieHeightOptionSet) {
        *env << "Warning: The -q, -4 or -i option was used, but not -h.  "
                "Assuming a video height of "
             << movieHeight << " pixels\n";
    }
    if (outputCompositeFile && !movieFPSOptionSet) {
        *env << "Warning: The -q, -4 or -i option was used, but not -f.  "
                "Assuming a video frame rate of "
...
void createOutputFiles(char const* periodicFilenameSuffix) {
    ...
    if (outputQuickTimeFile || outputAVIFile) {
        if (periodicFilenameSuffix[0] == '\0') {
            // Normally (unless the '-P <interval-in-seconds>' option was given)
            // we output to 'stdout':
            ...
            snprintf(outFileName, sizeof outFileName, "%s%s.%s", prefix,
                     periodicFilenameSuffix,
                     outputAVIFile       ? "avi"
                     : generateMP4Format ? "mp4"
                                         : "mov");
        }

        if (outputQuickTimeFile) {
            qtOut = QuickTimeFileSink::createNew(
                *env, *session, outFileName, fileSinkBufferSize, movieWidth,
                movieHeight, movieFPS, packetLossCompensate, syncStreams,
                generateHintTracks, generateMP4Format);
            if (qtOut == NULL) {
                *env << "Failed to create a \"QuickTimeFileSink\" for "
                        "outputting to \""
        ...
        while ((subsession = iter.next()) != NULL) {
            ...
            if (strcmp(subsession->mediumName(), "video") == 0) {
                if (strcmp(subsession->codecName(), "H264") == 0) {
                    ...
                    // 'start codes', and (at the start) the SPS and PPS NAL
                    // units:
                    fileSink = H264VideoFileSink::createNew(
                        *env, outFileName,
                        subsession->fmtp_spropparametersets(),
                        fileSinkBufferSize, oneFilePerFrame);
                } else if (strcmp(subsession->codecName(), "H265") == 0) {
                    // For H.265 video stream, we use a special sink that adds
                    // 'start codes', and (at the start) the VPS, SPS, and PPS
                    // NAL units:
                    fileSink = H265VideoFileSink::createNew(
                        *env, outFileName, subsession->fmtp_spropvps(),
                ...
                if (strcmp(subsession->codecName(), "AMR") == 0 ||
                    ...
                    // For AMR audio streams, we use a special sink that inserts
                    // AMR frame hdrs:
                    fileSink = AMRAudioFileSink::createNew(
                        *env, outFileName, fileSinkBufferSize, oneFilePerFrame);
                } else if (strcmp(subsession->codecName(), "VORBIS") == 0 ||
                           strcmp(subsession->codecName(), "OPUS") == 0) {
                    createOggFileSink = True;
                } else if (strcmp(subsession->codecName(), "MPEG4-GENERIC") ==
                           0) {
                    // For AAC audio, we use a regular file sink, but add a
                    // 'ADTS framer' filter to the end of the data source, so
                    // that the resulting file is playable:
            ...
            if (createOggFileSink) {
                fileSink = OggFileSink ::createNew(
                    *env, outFileName, subsession->rtpTimestampFrequency(),
                    subsession->fmtp_config());
            } else if (fileSink == NULL) {
                // Normal case:
                fileSink = FileSink::createNew(
                    *env, outFileName, fileSinkBufferSize, oneFilePerFrame);
            }
            ...
            if (subsession->sink == NULL) {
                *env << "Failed to create FileSink for \"" << outFileName
                ...
                if (strcmp(subsession->mediumName(), "video") == 0 &&
                    ...
                    // from the SDP description contains useful VOL etc.
                    // headers. Insert this data at the front of the output
                    // file:
                    unsigned configLen;
...
void createPeriodicOutputFiles() {
    ...
    periodicFileOutputTask = env->taskScheduler().scheduleDelayedTask(
        fileOutputInterval * 1000000, (TaskFunc*)periodicFileOutputTimerHandler,
        (void*)NULL);
}
...
void setupStreams() {
    ...
    char const* absStartTime = initialAbsoluteSeekTime != NULL
...
void signalHandlerShutdown(int /*sig*/) {
    *env << "Got shutdown signal\n";
    waitForResponseToTEARDOWN =
        False;  // to ensure that we end, even if the server does not respond to
                // our TEARDOWN
    shutdown(0);
}
