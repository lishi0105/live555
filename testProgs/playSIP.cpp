...
Medium* createClient(UsageEnvironment& env, char const* /*url*/,
    ...
    return ourSIPClient = SIPClient::createNew(
               env, desiredAudioRTPPayloadFormat, mimeSubtype, verbosityLevel,
               applicationName);
}
...
void getSDPDescription(RTSPClient::responseHandler* afterFunc) {
    ...
    if (proxyServerName != NULL) {
        ...
        if (addresses.numAddresses() == 0) {
            ...
            ourSIPClient->setProxyServer(proxyServerAddress,
                                         proxyServerPortNum);
        }
    }
    ...
    if (username != NULL && password != NULL) {
        result =
            ourSIPClient->inviteWithPassword(streamURL, username, password);
    } else {
