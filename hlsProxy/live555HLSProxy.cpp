...
// A program that acts as a proxy for a RTSP stream, converting it into a
// sequence of HLS (HTTP Live Streaming) segments, plus a ".m3u8" file that can
// be accessed via a web browser. main program

#include "BasicUsageEnvironment.hh"
#include "liveMedia.hh"

#define RTSP_CLIENT_VERBOSITY_LEVEL \
    0  // set to 1 for more verbose output from the "RTSPClient"
#define OUR_HLS_SEGMENTATION_DURATION 6 /*seconds*/
#define OUR_HLS_REWIND_DURATION \
    60 /*seconds: How far back in time a browser can seek*/
...
void usage() {
    *env << "usage:\t" << programName
         << " [-u <username> <password>] [-t|-T <http-port>] <input-RTSP-url> "
            "<HLS-prefix>\n";
    *env << "   or:\t" << programName
         << " -R [<port-num>] [-U <username-for-REGISTER> "
            "<password-for-REGISTER>] <HLS-prefix>\n";
    exit(1);
}

// Forward function definitions:
void continueAfterClientCreation0(RTSPClient* rtspClient,
...
int main(int argc, char** argv) {
    ...
    *env << "LIVE555 HLS Proxy, documented at "
            "http://www.live555.com/hlsProxy/\n";
    *env << "\t(LIVE555 Streaming Media version "
    ...
    while (argc > 1) {
        ...
        switch (opt[1]) {
            ...
            case 't': {  // stream RTP and RTCP over the TCP 'control'
                         // connection
                streamUsingTCP = True;
                break;
            }
            ...
            case 'R': {
                ...
                if (argc > 2 && argv[2][0] != '-') {
                    // The next argument is the REGISTER handler server port
                    // number:
                    if (sscanf(argv[2], "%hu",
                               &handlerServerForREGISTERCommandPortNum) == 1 &&
            ...
            case 'U': {  // specify a username and password to be used to
                         // authentication an incoming "REGISTER" command (for
                         // use with -R)
                if (argc < 4)
                    usage();  // there's no argv[3] (for the "password")
    ...
    if (createHandlerServerForREGISTERCommand) {
        ...
        if (usernameForREGISTER != NULL) {
            *env << "The '-U <username-for-REGISTER> <password-for-REGISTER>' "
                    "option can be used only with -R\n";
            usage();
        }
        ...
        RTSPClient* rtspClient =
            RTSPClient::createNew(*env, rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL,
                                  programName, tunnelOverHTTPPortNum);
        if (rtspClient == NULL) {
...
void continueAfterClientCreation1(RTSPClient* rtspClient) {
    // Having created a "RTSPClient" object, send a RTSP "DESCRIBE" command for
    // the URL:
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode,
    ...
    do {
        ...
        // Create a Transport Stream to multiplex the Elementary Stream data
        // from each subsession:
    ...
    if (subsession != NULL) {
        ...
        if ((strcmp(subsession->mediumName(), "video") == 0 &&
            ...
            (strcmp(subsession->mediumName(), "audio") == 0 &&
             strcmp(subsession->codecName(), "MPEG4-GENERIC" /*aka. AAC*/) ==
                 0)) {
            // Use this subsession.
            ++numUsableSubsessions;
            if (!subsession->initiate()) {
                *env << *rtspClient << "Failed to initiate the \""
                     << *subsession << "\" subsession: " << env->getResultMsg()
                     << "\n";
            } else {
                *env << *rtspClient << "Initiated the \"" << *subsession
                     << "\" subsession\n";
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode,
    ...
    do {
        ...
        *env << *rtspClient << "Set up the \"" << *subsession
             << "\" subsession\n";

        // Feed this subsession's input source into the Transport Stream:
        if (strcmp(subsession->mediumName(), "video") == 0) {
            // Create a 'framer' filter for the input source, to put the stream
            // of NAL units into a form that's usable to output to the Transport
            // Stream. (Note that we use a *DiscreteFramer*, because the input
            // source is a stream of discrete
            ...
            if (strcmp(subsession->codecName(), "H264") == 0) {
                ...
                framer = H264VideoStreamDiscreteFramer::createNew(
                    *env, subsession->readSource(),
                    True /*includeStartCodeInOutput*/,
                    True /*insertAccessUnitDelimiters*/);

                // Add any known SPS and PPS NAL units to the framer, so they'll
                // get output ASAP:
                u_int8_t* sps = NULL;
                unsigned spsSize = 0;
                u_int8_t* pps = NULL;
                ...
                SPropRecord* sPropRecords = parseSPropParameterSets(
                    subsession->fmtp_spropparametersets(), numSPropRecords);
                ...
                framer = H265VideoStreamDiscreteFramer::createNew(
                    *env, subsession->readSource(),
                    True /*includeStartCodeInOutput*/,
                    True /*insertAccessUnitDelimiters*/);

                // Add any known VPS, SPS and PPS NAL units to the framer, so
                // they'll get output ASAP:
                u_int8_t* vps = NULL;
                unsigned vpsSize = 0;
                u_int8_t* sps = NULL;
                unsigned spsSize = 0;
                ...
                unsigned ppsSize = 0;
                unsigned numSPropRecords;
                SPropRecord *sPropRecordsVPS, *sPropRecordsSPS,
                    *sPropRecordsPPS;

                sPropRecordsVPS = parseSPropParameterSets(
                    subsession->fmtp_spropvps(), numSPropRecords);
                if (numSPropRecords > 0) {
                    vps = sPropRecordsVPS[0].sPropBytes;
                    vpsSize = sPropRecordsVPS[0].sPropLength;
                }

                sPropRecordsSPS = parseSPropParameterSets(
                    subsession->fmtp_spropsps(), numSPropRecords);
                if (numSPropRecords > 0) {
                    sps = sPropRecordsSPS[0].sPropBytes;
                    spsSize = sPropRecordsSPS[0].sPropLength;
                }

                sPropRecordsPPS = parseSPropParameterSets(
                    subsession->fmtp_sproppps(), numSPropRecords);
                if (numSPropRecords > 0) {
                    pps = sPropRecordsPPS[0].sPropBytes;
                    ppsSize = sPropRecordsPPS[0].sPropLength;
                }

                framer->setVPSandSPSandPPS(vps, vpsSize, sps, spsSize, pps,
                                           ppsSize);
            ...
            // Create a 'framer' filter for the input source, to add a ADTS
            // header to each AAC frame, to make the audio playable.
            ADTSAudioStreamDiscreteFramer* framer =
                ADTSAudioStreamDiscreteFramer::createNew(
...
void segmentationCallback(void* clientData, char const* segmentFileName,
                          double segmentDuration);  // forward
void afterPlaying(void* clientData);                // forward

void startPlayingSession(RTSPClient* rtspClient) {
    // Having successfully setup the session, create a data sink for it, and
    ...
    MediaSink* sink = HLSSegmenter::createNew(
        *env, OUR_HLS_SEGMENTATION_DURATION, hlsPrefix, segmentationCallback);
    ...
    *env << "Beginning to read...\n";
    ...
    if (session->absStartTime() != NULL) {
        ...
        rtspClient->sendPlayCommand(
            *session, continueAfterPLAY, session->absStartTime(),
            session->absEndTime(), 1.0f, ourAuthenticator);
    } else {
        duration = session->playEndTime() - session->playStartTime();
        rtspClient->sendPlayCommand(*session, continueAfterPLAY, 0.0f, -1.0f,
                                    1.0f, ourAuthenticator);
    }
}

// A record that defines a segment that has been written.  These records are
// kept in a list:
class SegmentRecord {
public:
    SegmentRecord(char const* segmentFileName, double segmentDuration)
        : fNext(NULL),
          fSegmentFileName(strDup(segmentFileName)),
          fSegmentDuration(segmentDuration) {}
...
void segmentationCallback(void* /*clientData*/, char const* segmentFileName,
    ...
    SegmentRecord* newSegment =
        new SegmentRecord(segmentFileName, segmentDuration);
    if (tail != NULL) {
    ...
    fprintf(stderr,
            "Wrote segment \"%s\" (duration: %f seconds) -> %f seconds of data "
            "stored\n",
    ...
    while (totalDuration > OUR_HLS_REWIND_DURATION) {
        ...
        fprintf(stderr,
                "\tDeleting segment \"%s\" (duration: %f seconds) -> %f "
                "seconds of data stored\n",
                segmentToRemove->fileName(), segmentToRemove->duration(),
                totalDuration);
        if (unlink(segmentToRemove->fileName()) != 0) {
            *env << "\t\tunlink(\"" << segmentToRemove->fileName()
                 << "\") failed: " << env->getResultMsg() << "\n";
        }
    ...
    if (ourM3U8FileName == NULL) {
        ourM3U8FileName =
            new char[strlen(hlsPrefix) + 5 /*strlen(".m3u8")*/ + 1];
    ...
    if (ourM3U8Fid == NULL) {
        *env << "Failed to open file \"" << ourM3U8FileName
             << "\": " << env->getResultMsg();
        exit(1);
    }
    ...
    for (SegmentRecord* segment = head; segment != NULL;
    ...
    if (isFirstTime) {
        fprintf(stderr,
                "Wrote index file \"%s\"; the stream can now be played from a "
                "URL pointing to this file.\007\n",
                ourM3U8FileName);
        isFirstTime = False;
    }
}
