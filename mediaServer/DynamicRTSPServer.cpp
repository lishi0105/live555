...
DynamicRTSPServer::DynamicRTSPServer(UsageEnvironment& env, int ourSocketIPv4,
...
static void onMatroskaDemuxCreation(MatroskaFileServerDemux* newDemux,
    ...
    MatroskaDemuxCreationState* creationState =
        (MatroskaDemuxCreationState*)clientData;
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
    ...
    // Use the file name extension to determine the type of
    // "ServerMediaSession":
    ...
    if (strcmp(extension, ".aac") == 0) {
        ...
        OutPacketBuffer::maxSize =
            100000;  // allow for some possibly large H.264 frames
        ...
        NEW_SMS("H.265 Video");
        OutPacketBuffer::maxSize =
            100000;  // allow for some possibly large H.265 frames
        ...
        // Use an index file name that's the same as the TS file name, except
        // with ".tsx":
        unsigned indexFileNameLen =
            strlen(fileName) + 2;  // allow for trailing "x\0"
        ...
        // Assumed to be a Matroska file (note that WebM ('.webm') files are
        // also Matroska files)
        OutPacketBuffer::maxSize =
            300000;  // allow for some possibly large VP8 or VP9 frames
        ...
        creationState.watchVariable = 0;
        OggFileServerDemux::createNew(env, fileName, onOggDemuxCreation,
                                      &creationState);
        env.taskScheduler().doEventLoop(&creationState.watchVariable);
