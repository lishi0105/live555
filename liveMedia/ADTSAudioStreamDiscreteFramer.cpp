...
ADTSAudioStreamDiscreteFramer* ADTSAudioStreamDiscreteFramer ::createNew(
    UsageEnvironment& env, FramedSource* inputSource, char const* configStr) {
    u_int16_t configValue = 0;

    if (configStr != NULL && strlen(configStr) >= 4) {
        configValue = (hexToBinary(configStr[0]) << 12) |
                      (hexToBinary(configStr[1]) << 8) |
                      (hexToBinary(configStr[2]) << 4) |
                      hexToBinary(configStr[3]);
    }

    // Unpack the 2-byte 'config' value to get "profile",
    // "samplingFrequencyIndex", and "channelConfiguration":
    ...
    return new ADTSAudioStreamDiscreteFramer(env, inputSource, profile,
                                             samplingFrequencyIndex,
                                             channelConfiguration);
}

ADTSAudioStreamDiscreteFramer ::ADTSAudioStreamDiscreteFramer(
    UsageEnvironment& env, FramedSource* inputSource, u_int8_t profile,
    ...
    fADTSHeader[1] =
        0xF1;  // last 4 bits of syncword; version 0; layer 0; protection absent
    fADTSHeader[2] = (profile << 6) | (samplingFrequencyIndex << 2) |
...
void ADTSAudioStreamDiscreteFramer::doGetNextFrame() {
    // Arrange to read data (an AAC audio frame) from our data source, directly
    // into the downstream object's buffer, allowing for the ATDS header in
    // front. Make sure there's enoughn space:
    ...
    fInputSource->getNextFrame(fTo + ADTS_HEADER_SIZE,
                               fMaxSize - ADTS_HEADER_SIZE, afterGettingFrame,
                               this, FramedSource::handleClosure, this);
}

void ADTSAudioStreamDiscreteFramer ::afterGettingFrame(
    void* clientData, unsigned frameSize, unsigned numTruncatedBytes,
    struct timeval presentationTime, unsigned durationInMicroseconds) {
    ADTSAudioStreamDiscreteFramer* source =
        (ADTSAudioStreamDiscreteFramer*)clientData;
    source->afterGettingFrame1(frameSize, numTruncatedBytes, presentationTime,
                               durationInMicroseconds);
}

void ADTSAudioStreamDiscreteFramer ::afterGettingFrame1(
