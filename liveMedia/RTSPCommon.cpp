...
Boolean parseRTSPRequestString(
    ...
    unsigned resultURLSuffixMaxSize, char* resultCSeq,
    unsigned resultCSeqMaxSize, char* resultSessionIdStr,
    unsigned resultSessionIdStrMaxSize, unsigned& contentLength,
    Boolean& urlIsRTSPS) {
    ...
    for (; (int)j < (int)(reqStrSize - 8); ++j) {
        if ((reqStr[j] == 'r' || reqStr[j] == 'R') &&
            ...
            if (reqStr[j + 4] == ':' && reqStr[j + 5] == '/') {
                j += 6;
                if (reqStr[j] == '/') {
                    // This is a "rtsp(s)://" URL; skip over the host:port part
                    // that follows:
                    ++j;
                    while (j < reqStrSize && reqStr[j] != '/' &&
                           reqStr[j] != ' ')
                        ++j;
                } else {
...
Boolean parseRangeParam(char const* paramStr, double& rangeStart,
                        double& rangeEnd, char*& absStartTime,
