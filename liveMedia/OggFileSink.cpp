...
void OggFileSink::addData(unsigned char const* data, unsigned dataSize,
    ...
    if (fIsTheora) {
        ...
        if (!(data[0] >= 0x80 &&
              data[0] <=
                  0x82)) {  // for header pages, "fGranulePosition" remains 0
    ...
    if (!fHaveWrittenFirstFrame) {
        ...
        if (fConfigStr != NULL && fConfigStr[0] != '\0') {
            ...
            parseVorbisOrTheoraConfigStr(
                fConfigStr, identificationHdr, identificationHdrSize,
                commentHdr, commentHdrSize, setupHdr, setupHdrSize, identField);
