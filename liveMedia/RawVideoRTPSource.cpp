...
RawVideoRTPSource ::RawVideoRTPSource(UsageEnvironment& env, Groupsock* RTPgs,
...
void RawVideoBufferedPacket::getNextEnclosedFrameParameters(
    ...
    frameDurationInMicroseconds =
        0;  // because all lines within the same packet are from the same frame
    ...
    // packet, and the packet's 'M' bit was set:
    fOurSource->fCurrentPacketCompletesFrame =
        fOurSource->fCurPacketMarkerBit &&
        fOurSource->fNextLine == fOurSource->fNumLines - 1;
