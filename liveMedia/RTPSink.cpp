...
void RTPSink::setupForSRTP(u_int8_t const* MIKEYStateMessage,
    ...
    delete fMIKEYState;
    fMIKEYState =
        MIKEYState::createNew(MIKEYStateMessage, MIKEYStateMessageSize);
    fCrypto = new SRTPCryptographicContext(*fMIKEYState);
}
...
char* RTPSink::rtpmapLine() const {
    if (rtpPayloadType() >= 96) {  // the payload format type is dynamic
        ...
        unsigned rtpmapLineSize = strlen(rtpmapFmt) + 3 /* max char len */ +
...
char* RTPSink::keyMgmtLine() {
    ...
    if (fMIKEYState != NULL && (mikeyMessage = fMIKEYState->generateMessage(
                                    mikeyMessageSize)) != NULL) {
        char const* const keyMgmtFmt = "a=key-mgmt:mikey %s\r\n";
        char* base64EncodedData =
            base64Encode((char*)mikeyMessage, mikeyMessageSize);
        delete[] mikeyMessage;

        unsigned keyMgmtLineSize =
            strlen(keyMgmtFmt) + strlen(base64EncodedData);
...
void RTPTransmissionStatsDB ::noteIncomingRR(
    u_int32_t SSRC, struct sockaddr_storage const& lastFromAddress,
...
void RTPTransmissionStats ::noteIncomingRR(
    struct sockaddr_storage const& lastFromAddress, unsigned lossStats,
