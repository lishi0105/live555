...
Boolean MPEG2TransportStreamParser ::processStreamPacket(
    ...
    do {
        ...
        if (pusi &&
            pidState->stream_type !=
                0x05 /*these special private streams don't have PES hdrs*/) {
        ...
        double pts =
            pidState->lastSeenPTS == 0.0 ? fLastSeenPCR : pidState->lastSeenPTS;
    ...
    do {
        ...
        if (!isSpecialStreamId[stream_id]) {
            ...
            if (PES_extension_flag) {
                ...
                fprintf(
                    stderr,
                    "\t\t\tPES_extension: flags: 0x%02x (PES_private_data:%d; "
                    "pack_header_field:%d; program_packet_sequence_counter:%d; "
                    "P_STD_buffer:%d; PES_extension_2:%d\n",
...
PIDState_STREAM::PIDState_STREAM(MPEG2TransportStreamParser& parser,
                                 u_int16_t pid, u_int16_t programNumber,
                                 u_int8_t streamType)
