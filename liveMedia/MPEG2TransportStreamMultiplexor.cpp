...
void MPEG2TransportStreamMultiplexor ::setTimedSegmentation(
    unsigned segmentationDuration, onEndOfSegmentFunc* onEndOfSegmentFunc,
        ...
        if (streamType == 0) {
            ...
            if ((stream_id & 0xF0) == 0xE0) {  // video
                streamType =
                    ...
                    : mpegVersion == 2           ? 2
                    : mpegVersion == 4           ? 0x10
                    : mpegVersion == 5 /*H.264*/ ? 0x1B
                                                 : 0x24 /*assume H.265*/;
            } else if ((stream_id & 0xE0) == 0xC0) {  // audio
                streamType = mpegVersion == 1   ? 3
                             : mpegVersion == 2 ? 4
                             : mpegVersion == 3 ? 6
                                                : 0xF;
            } else if (stream_id ==
...
void MPEG2TransportStreamMultiplexor ::deliverDataToClient(
    u_int16_t pid, unsigned char* buffer, unsigned bufferSize,
            ...
            if (numHeaderBytes > 5) {
                ...
                *header++ = flags;
                if (willAddPCR) {
                    ...
                    *header++ =
                        (u_int8_t)fPCR.extension;  // low 8 bits of extension
                    ...
                    if (fSegmentationDuration > 0) {
                        ...
                        double lastSubSegmentDuration =
                            (fPreviousPTS == 0.0) || (pts < fPreviousPTS)
                                ? 0.0
                                : pts - fPreviousPTS;
...
void MPEG2TransportStreamMultiplexor::deliverPATPacket() {
    ...
    *pat++ = 0;  // table_id
    *pat++ =
        0xB0;  // section_syntax_indicator; 0; reserved, section_length (high)
    ...
    *pat++ =
        0xE0 | (OUR_PROGRAM_MAP_PID >> 8);  // reserved; program_map_PID (high)
