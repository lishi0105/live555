...
MatroskaFileServerDemux ::newServerMediaSubsessionByTrackNumber(
    ...
    if (strcmp(track->mimeType, "audio/MPEG") == 0) {
        result =
            MP3AudioMatroskaFileServerMediaSubsession::createNew(*this, track);
    } else {
    ...
    if (result != NULL) {
        ...
        fprintf(
            stderr,
            "Created 'ServerMediaSubsession' object for track #%d: %s (%s)\n",
    ...
    if (clientSessionId != 0 && clientSessionId == fLastClientSessionId) {
        demuxToUse =
            fLastCreatedDemux;  // use the same demultiplexor as before
                                // Note: This code relies upon the fact that the
                                // creation of streams for different client
                                // sessions do not overlap - so all demuxed
                                // tracks are created for one "MatroskaDemux" at
                                // a time. Also, the "clientSessionId != 0" test
                                // is a hack, because 'session 0' is special;
                                // its audio and video streams are created and
                                // destroyed one-at-a-time, rather than both
                                // streams being created, and then (later) both
                                // streams being destroyed (as is the case for
                                // other ('real') session ids).  Because of
                                // this, a separate demultiplexor is used for
                                // each 'session 0' track.
    }
...
void MatroskaFileServerDemux ::onDemuxDeletion(
    void* clientData, MatroskaDemux* demuxBeingDeleted) {
