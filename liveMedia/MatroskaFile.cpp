...
public:
    ...
    static void addCuePoint(CuePoint*& root, double cueTime,
...
void MatroskaFile ::createNew(UsageEnvironment& env, char const* fileName,
...
MatroskaFile::MatroskaFile(UsageEnvironment& env, char const* fileName,
                           onCreationFunc* onCreation,
                           void* onCreationClientData,
                           char const* preferredLanguage)
    : Medium(env),
...
MatroskaDemux* MatroskaFile ::newDemux(
    MatroskaDemuxOnDeletionFunc* onDeletionFunc, void* objectToNotify) {
...
void MatroskaFile::removeDemux(MatroskaDemux* demux) {
    DemuxRecord* demuxRecord =
        (DemuxRecord*)(fDemuxesTable->Lookup((char const*)demux));
    if (demuxRecord != NULL) {
...
void MatroskaFile::getH264ConfigData(MatroskaTrack const* track, u_int8_t*& sps,
...
void MatroskaFile::getH265ConfigData(MatroskaTrack const* track, u_int8_t*& vps,
                                     unsigned& vpsSize, u_int8_t*& sps,
                                     unsigned& spsSize, u_int8_t*& pps,
                                     unsigned& ppsSize) {
    vps = sps = pps = NULL;
...
void MatroskaFile ::getVorbisOrTheoraConfigData(
    MatroskaTrack const* track, u_int8_t*& identificationHeader,
    ...
    do {
        ...
        Boolean isTheora =
            strcmp(track->mimeType, "video/THEORA") == 0;  // otherwise, Vorbis
...
FramedSource* MatroskaFile ::createSourceForStreaming(
    FramedSource* baseSource, unsigned trackNumber, unsigned& estBitrate,
...
RTPSink* MatroskaFile ::createRTPSinkForTrackNumber(
    unsigned trackNumber, Groupsock* rtpGroupsock,
    ...
    do {
        ...
        if (strcmp(track->mimeType, "audio/L16") == 0) {
            result = SimpleRTPSink::createNew(
                ...
                track->samplingFrequency, "audio", "L16", track->numChannels);
            ...
            getVorbisOrTheoraConfigData(
                track, identificationHeader, identificationHeaderSize,
            ...
            result = RawVideoRTPSink::createNew(
                envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
                track->pixelWidth, track->pixelHeight, track->bitDepth,
            ...
            result = H265VideoRTPSink::createNew(
                envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, vps, vpsSize,
    ...
    do {
        ...
        if (strcmp(track->mimeType, "video/H264") == 0) {
            ...
            result = H264VideoFileSink::createNew(
                ...
                MAX_KEY_FRAME_SIZE);  // extra large buffer size for large key
                                      // frames
        ...
        if (createOggFileSink) {
            ...
            if (strcmp(track->mimeType, "audio/VORBIS") == 0 ||
                ...
                u_int32_t identField =
                    0xFACADE;  // Can we get a real value from the file somehow?
...
void MatroskaTrackTable::add(MatroskaTrack* newTrack, unsigned trackNumber) {
    ...
    MatroskaTrack* existingTrack =
        (MatroskaTrack*)fTable->Add((char const*)trackNumber, newTrack);
...
MatroskaDemux::MatroskaDemux(MatroskaFile& ourFile)
    ...
    fOurParser = new MatroskaFileParser(
        ourFile, ByteStreamFileSource::createNew(envir(), ourFile.fileName()),
...
void MatroskaDemux::resetState() {
    // Iterate through all of our 'demuxed tracks', calling 'reset()' on each
    // one.
    HashTable::Iterator* iter =
        HashTable::Iterator::create(*fDemuxedTracksTable);
    ...
    while ((demuxedTrack = (MatroskaDemuxedTrack*)iter->next(trackNumber)) !=
           NULL) {
        demuxedTrack->reset();
    }
    delete iter;
...
CuePoint::CuePoint(double cueTime, u_int64_t clusterOffsetInFile,
                   unsigned blockNumWithinCluster)
    : fBalance(0),
      fCueTime(cueTime),
...
void CuePoint::addCuePoint(CuePoint*& root, double cueTime,
                           u_int64_t clusterOffsetInFile,
                           unsigned blockNumWithinCluster,
                           Boolean& needToReviseBalanceOfParent) {
    ...
    if (root == NULL) {
        root =
            new CuePoint(cueTime, clusterOffsetInFile, blockNumWithinCluster);
        ...
        if (needToReviseOurBalance) {
            ...
            if (root->fBalance == 0) {
                ...
                root->fBalance =
                    -1 +
                    2 * direction;  // -1 for "direction" 0; 1 for "direction" 1
                needToReviseBalanceOfParent = True;
