...
MIKEYState* MIKEYState::createNew(u_int8_t const* messageToParse,
                                  unsigned messageSize) {
    Boolean parsedOK;
    MIKEYState* newMIKEYState =
        new MIKEYState(messageToParse, messageSize, parsedOK);
...
u_int8_t* MIKEYState::generateMessage(unsigned& messageSize) const {
    ...
    // ASSERT: fTotalPayloadByteCount == the sum of all of the payloads'
    // "fDataSize"s
    messageSize = fTotalPayloadByteCount;
    u_int8_t* resultMessage = new u_int8_t[messageSize];
    u_int8_t* p = resultMessage;

    for (MIKEYPayload* payload = fHeaderPayload; payload != NULL;
...
MIKEYState::MIKEYState(u_int8_t const* messageToParse, unsigned messageSize,
                       Boolean& parsedOK)
    :  // Set encryption/authentication parameters to default values (that may
       // be overwritten later as we parse the message):
      fEncryptSRTP(False),
      fEncryptSRTCP(False),
...
Boolean MIKEYState ::parseNonHDRPayload(u_int8_t const*& ptr,
    ...
    do {
        ...
        switch (ourPayloadType) {
            ...
            case SP: {  // RFC 3830, section 6.10
                ...
                while (1) {
                    ...
                    switch (ppType) {
                        ...
                        case 2: {  // Authentication algorithm: we handle only
                                   // NULL and HMAC-SHA-1
                            if (ppLength != 1) break;
                            u_int8_t value = ptr[0];
                            if (value > 1) break;  // unsupported algorithm
                            if (value > 0) fUseAuthentication = True;
                            // Note: this might get changed by a subsequent
                            // "off/on" entry
                            policyIsOK = True;
                            break;
                        }
                        ...
                        default: {  // a policy type that we don't handle; still
                                    // OK
                            policyIsOK = True;
                            break;
                        }
            ...
            case KEMAC: {  // RFC 3830, section 6.2
                ...
                // Parse the 'key data sub-payload':
                {
                    ...
                    // Check the "Type" and "KV" fields; we support only TEK and
                    // SPI/MKI Note: This means that we'll reject the
                    // "a=key-mgmt" SDP MIKEY data from
                    //   an Axis camera, because that doesn't specify SPI/MKI.
                    //   But that's what we want, because the Axis camera's
                    //   "a=key-mgmt" SDP MIKEY data is meant to be ignored by
                    //   clients.
                    if (*subPtr++ != ((2 << 4) | 1))
                        break;  // Type 2 (TEK) | KV 1 (SPI/MKI)
                    u_int16_t keyDataLen = get2Bytes(subPtr);
                    // The key data length must be 30 (encryption key length
                    // (16) + salt length (14)):
                    if (keyDataLen != 30) break;

                    // Make sure we have enough space for the key data and the
                    // "SPI Length" field:
                ...
                if (macAlg == 0 /*NULL*/)
                    ...
                    break;  // unknown or unsupported MAC algorithm => parse
                            // fails
        ...
        addNewPayload(
            new MIKEYPayload(*this, ourPayloadType, payloadStart, payloadSize));
    ...
    switch (payloadType) {
        ...
        case T: {  // RFC 3830, section 6.6
            ...
            *p++ = 0;  // TS type: NTP-UTC
            ...
            u_int32_t ntpSeconds =
                timeNow.tv_sec + 0x83AA7E80;  // 1970 epoch -> 1900 epoch
            addWord(p, ntpSeconds);
            double fractionalPart =
                (timeNow.tv_usec / 15625.0) * 0x04000000;  // 2^32/10^6
            u_int32_t ntpFractionOfSecond =
                (u_int32_t)(fractionalPart + 0.5);  // round
        ...
        case RAND: {  // RFC 3830, section 6.11
            ...
            u_int8_t* p = fData;
            *p++ = 0;  // no next payload (initially)
            unsigned const numRandomWords =
                4;  // number of 32-bit words making up the RAND value
        ...
        case SP: {  // RFC 3830, section 6.10
            ...
            *p++ = 0;  // Protocol type: SRTP
            ...
            add1BytePolicyParam(p, 7 /*SRTP encryption off/on*/,
                                fOurMIKEYState.encryptSRTP());
            add1BytePolicyParam(p, 8 /*SRTCP encryption off/on*/,
                                fOurMIKEYState.encryptSRTCP());
            add1BytePolicyParam(p, 10 /*SRTP authentication off/on*/,
                                fOurMIKEYState.useAuthentication());
...
MIKEYPayload::MIKEYPayload(MIKEYState& ourMIKEYState, u_int8_t payloadType,
                           u_int8_t const* data, unsigned dataSize)
    : fOurMIKEYState(ourMIKEYState),
      fPayloadType(payloadType),
      fDataSize(dataSize),
      fNext(NULL) {
    fData = new u_int8_t[fDataSize];
