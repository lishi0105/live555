...
RawVideoRTPSink* RawVideoRTPSink ::createNew(
    UsageEnvironment& env, Groupsock* RTPgs, u_int8_t rtpPayloadFormat,
    unsigned width, unsigned height, unsigned depth, char const* sampling,
    char const* colorimetry) {
    if (sampling == NULL || colorimetry == NULL) return NULL;

    return new RawVideoRTPSink(env, RTPgs, rtpPayloadFormat, width, height,
                               depth, sampling, colorimetry);
}

RawVideoRTPSink ::RawVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs,
                                  u_int8_t rtpPayloadFormat, unsigned width,
                                  unsigned height, unsigned depth,
                                  char const* sampling, char const* colorimetry)
    : VideoRTPSink(env, RTPgs, rtpPayloadFormat, 90000, "RAW"),
      fLineIndex(0),
      fP(width, height, depth, sampling) {
...
RawVideoRTPSink::~RawVideoRTPSink() { delete[] fFmtpSDPLine; }
...
void RawVideoRTPSink ::doSpecialFrameHandling(
    ...
    u_int16_t* offsets;
    unsigned numLines =
        getNumLinesInPacket(fragmentationOffset, lengths, offsets);
...
unsigned RawVideoRTPSink::specialHeaderSize() const {
    ...
    u_int16_t* offsets;
    unsigned numLines =
        getNumLinesInPacket(curFragmentationOffset(), lengths, offsets);
    delete[] lengths;
    delete[] offsets;
    return 2 + (6 * numLines);
}

unsigned RawVideoRTPSink ::getNumLinesInPacket(unsigned fragOffset,
                                               u_int16_t*& lengths,
                                               u_int16_t*& offsets) const {
    ...
    if (fragOffset >= fP.frameSize) {
        envir() << "RawVideoRTPSink::getNumLinesInPacket(): bad fragOffset "
                << fragOffset << "\n";
        return 0;
    }
...
#define MAX_LINES_IN_PACKET 100
    ...
    while (1) {
        ...
        remainingSizeInPacket -= remainingSizeInPacket %
                                 fP.pgroupSize;  // use only multiple of pgroup
        lengthArray[numLines - 1] = remainingLineSize < remainingSizeInPacket
                                        ? remainingLineSize
                                        : remainingSizeInPacket;
        offsetArray[numLines - 1] =
            (offsetWithinLine * fP.numPixelsInPgroup) / fP.pgroupSize;
        // Note that the offsets are in specified to be in pixels (not octets,
        // nor pgroups)
...
RawVideoFrameParameters ::RawVideoFrameParameters(unsigned width,
    ...
    unsigned const numPgroupsPerScanLine =
        (width * scanLineIterationStep + (numPixelsInPgroup - 1)) /
        numPixelsInPgroup;
    // Note: This rounds up to the next 'pgroup'; see RFC 4175, section 4.3,
    // paragraph 2
