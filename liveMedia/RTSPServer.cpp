...
char* RTSPServer::rtspURLPrefix(int clientSocket, Boolean useIPv6) const {
    ...
    if (portNumHostOrder == defaultPortNum) {
        sprintf(urlBuffer, "rtsp%s://%s%s%s/", fOurConnectionsUseTLS ? "s" : "",
                addressPrefixInURL, AddressString(ourAddress).val(),
                addressSuffixInURL);
    } else {
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
    ...
    // would make no sense (as SRTP would add extra overhead for no benefit).
    ...
    if (fHTTPServerSocketIPv4 >= 0 || fHTTPServerSocketIPv6 >= 0) {
        ...
        envir().taskScheduler().turnOnBackgroundReadHandling(
            fHTTPServerSocketIPv4, incomingConnectionHandlerHTTPIPv4, this);
        envir().taskScheduler().turnOnBackgroundReadHandling(
            fHTTPServerSocketIPv6, incomingConnectionHandlerHTTPIPv6, this);
        return True;
    }
...
void RTSPServer ::setTLSState(char const* certFileName,
                              char const* privKeyFileName, Boolean weServeSRTP,
                              Boolean weEncryptSRTP) {
    ...
    // stream over TLS, but without SRTP (as SRTP would add extra overhead for
    // no benefit).
}
...
Boolean RTSPServer::specialClientAccessCheck(
...
void RTSPServer ::specialHandlingOfAuthenticationFailure(
    int /*clientSocket*/, struct sockaddr_storage const& /*clientAddr*/,
    char const* /*urlSuffix*/) {
...
RTSPServer::~RTSPServer() {
    ...
    while (
        (sotcp = (streamingOverTCPRecord*)fTCPStreamingDatabase->getFirst()) !=
        NULL) {
        delete sotcp;
    }
...
void RTSPServer::incomingConnectionHandlerHTTPIPv4(void* instance,
                                                   int /*mask*/) {
...
void RTSPServer::incomingConnectionHandlerHTTPIPv6(void* instance,
                                                   int /*mask*/) {
    RTSPServer* server = (RTSPServer*)instance;
...
void RTSPServer::RTSPClientConnection ::handleCmd_DESCRIBE(
    ...
    // "urlTotalSuffix":
    fOurServer.lookupServerMediaSession(urlTotalSuffix,
                                        DESCRIBELookupCompletionFunction, this);
}

void RTSPServer::RTSPClientConnection ::DESCRIBELookupCompletionFunction(
    void* clientData, ServerMediaSession* sessionLookedUp) {
    RTSPServer::RTSPClientConnection* connection =
        (RTSPServer::RTSPClientConnection*)clientData;
    connection->handleCmd_DESCRIBE_afterLookup(sessionLookedUp);
}

void RTSPServer::RTSPClientConnection ::handleCmd_DESCRIBE_afterLookup(
...
static void lookForHeader(char const* headerName, char const* source,
                          unsigned sourceLen, char* resultStr,
...
void RTSPServer::RTSPClientConnection::handleCmd_redirect(
    char const* urlSuffix) {
    ...
    snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
             ...
             "%s"
             "Location: %s%s\r\n\r\n",
             fCurrentCSeq, dateHeader(), urlPrefix, urlSuffix);
    delete[] urlPrefix;
}
...
Boolean RTSPServer::RTSPClientConnection::parseHTTPRequestString(
    ...
    unsigned sessionCookieMaxSize, char* acceptStr, unsigned acceptStrMaxSize) {
...
Boolean RTSPServer::RTSPClientConnection ::handleHTTPCmd_TunnelingPOST(
    ...
    fInputTLS->nullify();  // so that our destructor doesn't reset the copied
                           // TLS state
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
    ...
    do {
        ...
        Boolean parseSucceeded = parseRTSPRequestString(
            ...
            sizeof sessionIdStr, contentLength, urlIsRTSPS);
        ...
        if (parseSucceeded) {
            ...
            fprintf(
                stderr,
                "parseRTSPRequestString() succeeded, returning cmdName \"%s\", "
                "urlPreSuffix \"%s\", urlSuffix \"%s\", CSeq \"%s\", "
            ...
            // (i.e., "rtsps" instead of "rtsp", or vice versa), then send back
            // a 'redirect':
            if (urlIsRTSPS != fOurRTSPServer.fOurConnectionsUseTLS) {
                ...
                if (sscanf((char*)fRequestBuffer, "%*s %s", url) == 1) {
                    ...
                    handleCmd_REGISTER(
                        cmdName, url, urlSuffix, (char const*)fRequestBuffer,
    ...
    --fRecursionCount;
    // If it has a scheduledDelayedTask, don't delete the instance or close the
    // sockets. The sockets can be reused in the task.
...
Boolean RTSPServer::RTSPClientConnection ::authenticationOK(
    char const* cmdName, char const* urlSuffix, char const* fullRequestStr) {
    ...
    if (!isInitial401) {  // this is an actual authentication failure
        fOurRTSPServer.specialHandlingOfAuthenticationFailure(
            fClientInputSocket, fClientAddr, urlSuffix);
    }
    return False;
}
...
void RTSPServer::RTSPClientConnection ::changeClientInputSocket(
    int newSocketNum, ServerTLSState const* newTLSState,
...
static void parseTransportHeader(char const* buf, StreamingMode& streamingMode,
    ...
    while (sscanf(fields, "%[^;\r\n]", field) == 1) {
        if (strcmp(field, "RTP/AVP/TCP") == 0) {
            ...
            clientRTPPortNum = p1;
            clientRTCPPortNum = streamingMode == RAW_UDP
                                    ? 0
                                    : p2;  // ignore the second port number if
...
void RTSPServer::RTSPClientSession ::SETUPLookupCompletionFunction1(
    void* clientData, ServerMediaSession* sessionLookedUp) {
    RTSPServer::RTSPClientSession* session =
        (RTSPServer::RTSPClientSession*)clientData;
    session->handleCmd_SETUP_afterLookup1(sessionLookedUp);
}

void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup1(
    ServerMediaSession* sms) {
    if (sms != NULL) {
        // The lookup succeeded; continue working with the returned
        // "ServerMediaSession":
        handleCmd_SETUP_afterLookup2(sms);
        return;
    }

    // The lookup failed.  Check for the special case (noted above), before we
    // give up:
    ...
    if (fURLPreSuffix[0] == '\0') {
        ...
        concatenatedStreamName =
            new char[strlen(fURLPreSuffix) + strlen(fURLSuffix) +
    ...
    // Check again:
    fOurServer.lookupServerMediaSession(streamName,
                                        SETUPLookupCompletionFunction2, this,
                                        fOurServerMediaSession == NULL);
    delete[] concatenatedStreamName;
}

void RTSPServer::RTSPClientSession ::SETUPLookupCompletionFunction2(
    void* clientData, ServerMediaSession* sessionLookedUp) {
    RTSPServer::RTSPClientSession* session =
        (RTSPServer::RTSPClientSession*)clientData;
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup2(
    ServerMediaSession* sms) {
    do {
        if (sms == NULL) {
        ...
        if (fTrackId != NULL && fTrackId[0] != '\0') {  // normal case
            for (trackNum = 0; trackNum < fNumStreamStates; ++trackNum) {
                ...
                if (subsession != NULL &&
                    strcmp(fTrackId, subsession->trackId()) == 0)
                    break;
            }
        ...
        parseTransportHeader(fFullRequestStr, streamingMode,
        ...
        if ((streamingMode == RTP_TCP && rtpChannelId == 0xFF) ||
            (streamingMode != RTP_TCP &&
             fOurClientConnection->fClientOutputSocket !=
                 fOurClientConnection->fClientInputSocket)) {
        ...
        if (parseRangeHeader(fFullRequestStr, rangeStart, rangeEnd, absStart,
        ...
        getsockname(fOurClientConnection->fClientInputSocket,
                    (struct sockaddr*)&sourceAddr, &namelen);

        subsession->getStreamParameters(
            fOurSessionId, fOurClientConnection->fClientAddr, clientRTPPort,
            clientRTCPPort, fStreamStates[trackNum].tcpSocketNum, rtpChannelId,
            rtcpChannelId, &fOurClientConnection->fTLS, destinationAddress,
        ...
        if (fIsMulticast) {
            switch (streamingMode) {
                case RTP_UDP: {
                    snprintf((char*)fOurClientConnection->fResponseBuffer,
                             sizeof fOurClientConnection->fResponseBuffer,
                             "RTSP/1.0 200 OK\r\n"
                             "CSeq: %s\r\n"
                             "%s"
                             "Transport: "
                             "RTP/"
                             "%s;multicast;destination=%s;source=%s;port=%d-%d;"
                             "ttl=%d\r\n"
                             "Session: %08X%s\r\n\r\n",
                             fOurClientConnection->fCurrentCSeq, dateHeader(),
            ...
            switch (streamingMode) {
                case RTP_UDP: {
                    snprintf(
                        (char*)fOurClientConnection->fResponseBuffer,
                        sizeof fOurClientConnection->fResponseBuffer,
                        "RTSP/1.0 200 OK\r\n"
                        "CSeq: %s\r\n"
                        "%s"
                        "Transport: "
                        "RTP/"
                        "%s;unicast;destination=%s;source=%s;client_port=%d-%d;"
                        "server_port=%d-%d\r\n"
                        "Session: %08X%s\r\n\r\n",
                        fOurClientConnection->fCurrentCSeq, dateHeader(),
                        fOurRTSPServer.fWeServeSRTP ? "SAVP" : "AVP",
...
void RTSPServer::RTSPClientSession ::handleCmd_PLAY(
    ...
    if (!sawScaleHeader) {
        buf[0] =
            '\0';  // Because we didn't see a Scale: header, don't send one back
    } else {
        sprintf(buf, "Scale: %f\r\n", scale);
    }
    ...
    char* absEnd = NULL;
    Boolean startTimeIsNow;
    Boolean sawRangeHeader = parseRangeHeader(
        fullRequestStr, rangeStart, rangeEnd, absStart, absEnd, startTimeIsNow);
            ...
            if (fStreamStates[i].subsession != NULL) {
                ...
                if (absStart != NULL) {
                    ...
                    fStreamStates[i].subsession->seekStream(
                        fOurSessionId, fStreamStates[i].streamToken, absStart,
                        absEnd);
                } else {
                    ...
                    if (!sawRangeHeader || startTimeIsNow) {
                        ...
                        double streamDuration =
                            0.0;  // by default; means: stream until the end of
                                  // the media
    ...
    if (absStart != NULL) {
        ...
        if (absEnd == NULL) {
            ...
            rangeHeaderBuf = new char[100 + strlen(absStart) +
                                      strlen(absEnd)];  // ample space
