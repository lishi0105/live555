...
MP3AudioMatroskaFileServerMediaSubsession ::createNew(
    MatroskaFileServerDemux& demux, MatroskaTrack* track, Boolean generateADUs,
...
MP3AudioMatroskaFileServerMediaSubsession ::
    MP3AudioMatroskaFileServerMediaSubsession(MatroskaFileServerDemux& demux,
                                              MatroskaTrack* track,
                                              Boolean generateADUs,
                                              Interleaving* interleaving)
    : MP3AudioFileServerMediaSubsession(demux.envir(), demux.fileName(), False,
...
void MP3AudioMatroskaFileServerMediaSubsession ::seekStreamSource(
    FramedSource* inputSource, double& seekNPT, double /*streamDuration*/,
    u_int64_t& /*numBytes*/) {
    ...
    if (aduStream != NULL)
        aduStream
            ->resetInput();  // because we're about to seek within its source
