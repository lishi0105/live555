...
static int read_decoder_table(unsigned char* fi) {
    ...
    for (n = 0; n < HTN; n++) {
        ...
        if (strcmp(command, ".reference") == 0) {
            ...
            rsf_ht[n].ref = -1;
            rsf_ht[n].val =
                (unsigned char(*)[2]) new unsigned char[2 *
                                                        (rsf_ht[n].treelen)];
...
static int rsf_huffman_decoder(BitVector& bv, struct huffcodetab const* h,
...
static void rsf_huffman_encoder(BitVector& bv, struct huffcodetab* h, int x,
                                int y, int v, int w);  // forward
...
static void rsf_huffman_encoder(BitVector& bv, struct huffcodetab* h, int x,
    ...
    if (h->tablename[0] == '3' &&
        ...
        if (x > 1 || y > 1 || v > 1 || w > 1) {
            ...
            fprintf(
                stderr,
                "rsf_huffman_encoder quad sanity check fails: %x,%x,%x,%x\n", x,
                y, v, w);
#endif
        }
        ...
        if (v != 0 || w != 0) {
            ...
            fprintf(
                stderr,
                "rsf_huffman_encoder dual sanity check 1 fails: %x,%x,%x,%x\n",
                x, y, v, w);
#endif
        }
        ...
        if (x > 255 || y > 255) {
            ...
            fprintf(
                stderr,
                "rsf_huffman_encoder dual sanity check 2 fails: %x,%x,%x,%x\n",
                x, y, v, w);
#endif
        }
