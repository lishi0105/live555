...
#define TS_FROM_ES_LOW_WATER_MARK \
...
public:
    InputESSourceRecord(MPEG2TransportStreamFromESSource& parent,
                        FramedSource* inputSource, u_int8_t streamId,
...
void MPEG2TransportStreamFromESSource ::addNewInputSource(
    FramedSource* inputSource, u_int8_t streamId, int mpegVersion,
    int16_t PID) {
...
InputESSourceRecord ::InputESSourceRecord(
    MPEG2TransportStreamFromESSource& parent, FramedSource* inputSource,
    u_int8_t streamId, int mpegVersion, InputESSourceRecord* next, int16_t PID)
    : fNext(next),
      fParent(parent),
...
void InputESSourceRecord::askForNewData() {
    ...
    if (fInputBufferBytesAvailable < TS_FROM_ES_LOW_WATER_MARK &&
        ...
        fInputSource->getNextFrame(
            ...
            INPUT_BUFFER_SIZE - fInputBufferBytesAvailable, afterGettingFrame,
            this, FramedSource::handleClosure, &fParent);
    }
}

Boolean InputESSourceRecord::deliverBufferToClient() {
    if (fInputBufferInUse ||
        fInputBufferBytesAvailable < TS_FROM_ES_LOW_WATER_MARK)
        return False;
...
void InputESSourceRecord ::afterGettingFrame1(unsigned frameSize,
    ...
    if (fInputBufferBytesAvailable == SIMPLE_PES_HEADER_SIZE) {
        ...
        fprintf(
            stderr,
            "PES header: stream_id 0x%02x, pts: %u.%06u => SCR 0x%x%08x:%03x\n",
