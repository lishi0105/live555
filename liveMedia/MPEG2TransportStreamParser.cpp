...
MPEG2TransportStreamParser ::MPEG2TransportStreamParser(
    FramedSource* inputSource, FramedSource::onCloseFunc* onEndFunc,
    ...
    if (StreamTypes[0x01].dataType ==
        ...
        StreamTypes[0x10] =
            StreamType("MPEG-4 H.263 based video", StreamType::VIDEO, ".mpv");
        StreamTypes[0x1B] =
...
Boolean MPEG2TransportStreamParser::parse() {
    ...
    try {
        while (1) {
            ...
            Boolean pusi =
                (flagsPlusPID & 0x4000) != 0;  // payload_unit_start_indicator
...
u_int8_t MPEG2TransportStreamParser::parseAdaptationField() {
    ...
    if (totalAdaptationFieldSize != 1 + adaptation_field_length) {
        fprintf(
            stderr,
            "MPEG2TransportStreamParser::parseAdaptationField() Warning: Got "
            "an inconsistent \"totalAdaptationFieldSize\" %d; expected %d\n",
...
PIDState::PIDState(MPEG2TransportStreamParser& parser, u_int16_t pid,
                   PIDType pidType)
...
StreamType ::StreamType(char const* description, enum dataType dataType,
