...
unsigned RTSPClient::sendSetParameterCommand(MediaSession& session,
    ...
    unsigned result = sendRequest(
        new RequestRecord(++fCSeq, "SET_PARAMETER", responseHandler, &session,
                          NULL, False, 0.0, 0.0, 0.0, paramString));
    delete[] paramString;
    return result;
}
...
unsigned RTSPClient::sendGetParameterCommand(MediaSession& session,
    ...
    char* paramString =
        new char[parameterNameLen + 3];  // the 3 is for \r\n + the '\0' byte
    ...
    unsigned result = sendRequest(
        new RequestRecord(++fCSeq, "GET_PARAMETER", responseHandler, &session,
                          NULL, False, 0.0, 0.0, 0.0, paramString));
    delete[] paramString;
    return result;
}
...
Boolean RTSPClient::parseRTSPURL(char const* url, char*& username,
                                 char*& password, NetAddress& address,
                                 portNumBits& portNum, char const** urlSuffix) {
    do {
        // Parse the URL as
        ...
        if (_strncasecmp(url, rtspPrefix, rtspPrefixLength) == 0) {
            ...
            envir().setResultMsg(
                "URL does not begin with \"rtsp://\" or \"rtsps://\"");
            break;
        }
        ...
        portNum =
            defaultPortNumber;  // unless it's specified explicitly in the URL
        char nextChar = *from;
        if (nextChar == ':') {
            int portNumInt;
...
unsigned RTSPClient::responseBufferSize =
    20000;  // default value; you can reassign this in your application if you
            // need to
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
    ...
    : Medium(env),
      ...
      fTLS(*this),
      fPOSTSocketTLS(*this) {
    fInputTLS = fOutputTLS =
        &fTLS;  // fOutputTLS will change if we're doing RTSP-over-HTTPS
    setBaseURL(rtspURL);
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
    ...
    do {
        ...
        unsigned cmdSize = strlen(cmdFmt) + strlen(request->commandName()) +
                           ...
                           strlen(fRequireStr) + strlen(extraHeaders) +
        ...
        sprintf(cmd, cmdFmt, request->commandName(), cmdURL, protocolStr,
                ...
                fRequireStr, extraHeaders, contentLengthHeader, contentStr);
...
Boolean RTSPClient::setRequestFields(RequestRecord* request, char*& cmdURL,
    ...
    if (strcmp(request->commandName(), "DESCRIBE") == 0) {
        ...
        cmdURL =
            new char[strlen(prefix) + strlen(separator) + strlen(suffix) + 1];
        ...
        if (streamUsingTCP) {  // streaming over the RTSP connection
            ...
            Boolean requestMulticastStreaming =
                ...
                (forceMulticastOnUnspecified &&
                 addressIsNull(connectionAddress));
            transportTypeStr =
        ...
        // The "Transport:", "Session:" (if present), "Blocksize:" (if present),
        // and "KeyMgmt:" (if present) headers make up the 'extra headers':
...
int RTSPClient::openConnection() {
    do {
        ...
        if (!parseRTSPURL(fBaseURL, username, password, destAddress, urlPortNum,
            ...
            break;
        if (urlPortNum == 322)
            fTLS.isNeeded = True;  // port 322 is a special case: "rtsps"
        ...
        fInputSocketNum =
            setupStreamSocket(envir(), 0, fServerAddress.ss_family);
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
    ...
    if (fVerbosityLevel >= 1) {
        envir() << "Connecting to " << AddressString(fServerAddress).val()
                << ", port " << remotePortNum << " on socket " << socketNum
                << "...\n";
    }
    if (connect(socketNum, (struct sockaddr*)&fServerAddress,
                addressSize(fServerAddress)) != 0) {
...
char* RTSPClient::createKeyMgmtString(char const* url,
    ...
    if ((mikeyState = subsession.getMIKEYState()) == NULL ||
        (mikeyMessage = mikeyState->generateMessage(mikeyMessageSize)) ==
            NULL) {
        keyMgmtStr = strDup("");
    } else {
        char const* keyMgmtFmt =
            "KeyMgmt: prot=mikey; uri=\"%s\"; data=\"%s\"\r\n";
        char* base64EncodedData =
            base64Encode((char*)mikeyMessage, mikeyMessageSize);
        delete[] mikeyMessage;

        unsigned keyMgmtSize =
            strlen(keyMgmtFmt) + strlen(url) + strlen(base64EncodedData);
...
Boolean RTSPClient ::parseResponseCode(char const* line, unsigned& responseCode,
...
void RTSPClient::handleIncomingRequest() {
    ...
    if (!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen,
                                ...
                                sizeof sessionId, contentLength, urlIsRTSPS)) {
        return;
    } else {
        if (fVerbosityLevel >= 1) {
...
Boolean RTSPClient::checkForHeader(char const* line, char const* headerName,
        ...
        if (streamUsingTCP) {
            ...
            if (subsession.rtpSource() != NULL) {
                subsession.rtpSource()->setStreamSocket(
                    fInputSocketNum, subsession.rtpChannelId, fInputTLS);
            ...
            if (subsession.rtcpInstance() != NULL)
                subsession.rtcpInstance()->setStreamSocket(
                    fInputSocketNum, subsession.rtcpChannelId, fInputTLS);
...
void RTSPClient::constructSubsessionURL(MediaSubsession const& subsession,
    ...
    if (isAbsoluteURL(suffix)) {
        ...
        separator =
            (prefixLen == 0 || prefix[prefixLen - 1] == '/' || suffix[0] == '/')
                ? ""
                : "/";
    }
}
...
Boolean RTSPClient::setupHTTPTunneling1() {
    ...
    return sendRequest(
               new RequestRecord(1, "GET", responseHandlerForHTTP_GET)) != 0;
}
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode,
    ...
    do {
        ...
        ignoreSigPipeOnSocket(
            fOutputSocketNum);  // so that servers on the same host that killed
                                // don't also kill us
...
void RTSPClient::connectionHandler1() {
    ...
    do {
        ...
        // Note: Normally "fOutputSocketNum" == "fInputSocketNum" here, except
        // when we're connecting to the second (i.e., "POST") connection when
        // doing RTSP-over-HTTP:
        if (getsockopt(fOutputSocketNum, SOL_SOCKET, SO_ERROR, (char*)&err,
                       &len) < 0 ||
            err != 0) {
        ...
        // Note: Normally "fOutputTLS" == "fInputTLS" here, except when we're
        // connecting to the second (i.e., "POST") connection when doing
        // RTSP-over-HTTP:
        if (fOutputTLS->isNeeded) {
            // We need to complete an additional TLS connection:
...
void RTSPClient::incomingDataHandler1() {
    int bytesRead = read((u_int8_t*)&fResponseBuffer[fResponseBytesAlreadySeen],
    ...
    do {
        ...
        do {
            ...
            while (1) {
                ...
                if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
                    ...
                    while ((request = fRequestsAwaitingResponse.dequeue()) !=
                        ...
                        if (request->cseq() <
                            ...
                            if (fVerbosityLevel >= 1 &&
                                strcmp(request->commandName(), "POST") != 0) {
                                envir()
                                    << "WARNING: The server did not respond to "
                                       "our \""
                                    << request->commandName()
                    ...
                    // appropriate/correct,
                               ...
                               // Media" software,
                               rangeParamsStr
                               // so that other projects that use the code could
                               // benefit (not just your own project).
                               )) {
            ...
            if (contentLength > numBodyBytes) {
                ...
                if (fVerbosityLevel >= 1) {
                    envir()
                        << "Have received " << fResponseBytesAlreadySeen
                        << " total bytes of a "
                ...
                if (foundRequest != NULL)
                    fRequestsAwaitingResponse.putAtHead(
                        foundRequest);  // put our request record back; we need
                                        // it again
                ...
                if (responseCode == 200) {
                    ...
                    if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
                        if (!handleSETUPResponse(
                                *foundRequest->subsession(), sessionParamsStr,
                                transportParamsStr,
...
RTSPClient::RequestRecord::RequestRecord(
    unsigned cseq, char const* commandName, responseHandler* handler,
...
HandlerServerForREGISTERCommand* HandlerServerForREGISTERCommand ::createNew(
    ...
    return new HandlerServerForREGISTERCommand(
        env, creationFunc, ourSocketIPv4, ourSocketIPv6, ourPort, authDatabase,
