...
void GenericMediaServer ::lookupServerMediaSession(
    ...
    // Default implementation: Do a synchronous lookup, and call the completion
    // function:
    if (completionFunc != NULL) {
        (*completionFunc)(completionClientData,
                          getServerMediaSession(streamName));
    }
}
...
static void lsmsMemberFunctionCompletionFunc(
    void* clientData, ServerMediaSession* sessionLookedUp) {
    lsmsMemberFunctionRecord* memberFunctionRecord =
        (lsmsMemberFunctionRecord*)clientData;
    (memberFunctionRecord->fServer->*(memberFunctionRecord->fMemberFunc))(
        sessionLookedUp);
...
void GenericMediaServer ::lookupServerMediaSession(
    ...
    struct lsmsMemberFunctionRecord* memberFunctionRecord =
        new struct lsmsMemberFunctionRecord;
    memberFunctionRecord->fServer = this;
    memberFunctionRecord->fMemberFunc = memberFunc;

    GenericMediaServer ::lookupServerMediaSession(
        streamName, lsmsMemberFunctionCompletionFunc, memberFunctionRecord);
}
...
void GenericMediaServer::removeServerMediaSession(char const* streamName) {
    lookupServerMediaSession(streamName,
                             &GenericMediaServer::removeServerMediaSession);
}
...
GenericMediaServer ::GenericMediaServer(UsageEnvironment& env,
    ...
    ignoreSigPipeOnSocket(
        fServerSocketIPv4);  // so that clients on the same host that are killed
                             // don't also kill us
    ignoreSigPipeOnSocket(fServerSocketIPv6);  // ditto

    // Arrange to handle connections from others:
    env.taskScheduler().turnOnBackgroundReadHandling(
        fServerSocketIPv4, incomingConnectionHandlerIPv4, this);
    env.taskScheduler().turnOnBackgroundReadHandling(
        fServerSocketIPv6, incomingConnectionHandlerIPv6, this);
}
...
void GenericMediaServer::cleanup() {
    ...
    while ((serverMediaSession =
        ...
        removeServerMediaSession(
            serverMediaSession);  // will delete it, because it no longer has
                                  // any 'client session' objects using it
    }
...
void GenericMediaServer::incomingConnectionHandlerIPv4(void* instance,
                                                       int /*mask*/) {
    GenericMediaServer* server = (GenericMediaServer*)instance;
    server->incomingConnectionHandlerIPv4();
}
void GenericMediaServer::incomingConnectionHandlerIPv6(void* instance,
                                                       int /*mask*/) {
    GenericMediaServer* server = (GenericMediaServer*)instance;
...
GenericMediaServer::ClientConnection ::ClientConnection(
    ...
    if (useTLS) {
        ...
        fTLS.setCertificateAndPrivateKeyFileNames(
            ourServer.fTLSCertificateFileName,
            ourServer.fTLSPrivateKeyFileName);
        fTLS.isNeeded = True;

        fTLS.tlsAcceptIsNeeded =
            True;  // call fTLS.accept() the next time the socket is readable
    }
...
void GenericMediaServer::ClientConnection::incomingRequestHandler() {
    if (fInputTLS->tlsAcceptIsNeeded) {  // we need to successfully call
        ...
        if (fInputTLS->accept(fOurSocket) <= 0)
    ...
    if (fInputTLS->isNeeded) {
        ...
        struct sockaddr_storage
            dummy;  // 'from' address, meaningless in this case

        bytesRead = readSocket(envir(), fOurSocket,
...
ServerMediaSession* GenericMediaServer::getServerMediaSession(
    char const* streamName) {
