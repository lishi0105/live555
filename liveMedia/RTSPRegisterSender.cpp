...
RTSPRegisterSender* RTSPRegisterSender ::createNew(
    ...
    char const* proxyURLSuffix, Boolean reuseConnection, int verbosityLevel,
    char const* applicationName) {
    return new RTSPRegisterSender(
        env, remoteClientNameOrAddress, remoteClientPortNum, rtspURLToRegister,
        rtspResponseHandler, authenticator, requestStreamingViaTCP,
        proxyURLSuffix, reuseConnection, verbosityLevel, applicationName);
}

void RTSPRegisterSender::grabConnection(
    int& sock, struct sockaddr_storage& remoteAddress) {
    sock = grabSocket();
...
RTSPRegisterSender ::RTSPRegisterSender(
    ...
    char const* proxyURLSuffix, Boolean reuseConnection, int verbosityLevel,
    char const* applicationName)
    : RTSPRegisterOrDeregisterSender(env, remoteClientNameOrAddress,
                                     remoteClientPortNum, authenticator,
...
Boolean RTSPRegisterSender::setRequestFields(
    ...
    if (strcmp(request->commandName(), "REGISTER") == 0) {
        ...
        sprintf(
            transportHeaderStr, transportHeaderFmt,
...
RTSPRegisterSender::RequestRecord_REGISTER ::RequestRecord_REGISTER(
    ...
    : RTSPRegisterOrDeregisterSender::RequestRecord_REGISTER_or_DEREGISTER(
          cseq, "REGISTER", rtspResponseHandler, rtspURLToRegister,
          proxyURLSuffix),
...
RTSPDeregisterSender* RTSPDeregisterSender ::createNew(
    ...
    return new RTSPDeregisterSender(
        ...
        rtspURLToDeregister, rtspResponseHandler, authenticator, proxyURLSuffix,
        verbosityLevel, applicationName);
}
...
RTSPDeregisterSender::RequestRecord_DEREGISTER ::RequestRecord_DEREGISTER(
    ...
    : RTSPRegisterOrDeregisterSender::RequestRecord_REGISTER_or_DEREGISTER(
          cseq, "DEREGISTER", rtspResponseHandler, rtspURLToDeregister,
          proxyURLSuffix) {}
