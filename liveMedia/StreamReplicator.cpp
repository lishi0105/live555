...
void StreamReplicator::getNextFrame(StreamReplica* replica) {
    ...
    if (fPrimaryReplica == NULL) {
        ...
        // the 'primary' replica - meaning that we read the frame into its
        ...
        if (fInputSource != NULL)
            fInputSource->getNextFrame(
                fPrimaryReplica->fTo, fPrimaryReplica->fMaxSize,
...
void StreamReplicator::deactivateStreamReplica(
    ...
    // Check whether the replica being deactivated is the 'primary' replica, or
    ...
    if (replicaBeingDeactivated == fPrimaryReplica) {
        ...
        if (fReplicasAwaitingCurrentFrame == NULL) {
            ...
            // There's another replica that we can use as a replacement 'primary
            // replica':
        ...
        // Check whether the read into the old primary replica's buffer is still
        ...
        if (fInputSource != NULL) {
            if (fInputSource->isCurrentlyAwaitingData()) {
                ...
                // We need to stop it, and retry the read with a new primary (if
                // available)
                ...
                // The read into the old primary replica's buffer has already
                // completed.  Copy the data to the new primary replica (if
                // any):
                if (fPrimaryReplica != NULL) {
                    StreamReplica::copyReceivedFrame(fPrimaryReplica,
                                                     replicaBeingDeactivated);
                } else {
                    // We don't have a new primary replica, so we can't copy the
        ...
        // The replica that's being removed was not our 'primary replica', but
        ...
        // so now we need to complete delivery to the primary replica:
        if (fPrimaryReplica != NULL && fInputSource != NULL &&
...
void StreamReplicator::removeStreamReplica(StreamReplica* replicaBeingRemoved) {
    ...
    if (fNumReplicas == 0)
        fprintf(
            stderr, "StreamReplicator::removeStreamReplica() Internal "
                    "Error!\n");  // should not happen
    --fNumReplicas;
...
void StreamReplicator::afterGettingFrame(unsigned frameSize,
                                         ...
                                         struct timeval presentationTime,
                                         unsigned durationInMicroseconds) {
    // The frame was read into our primary replica's buffer.  Update the primary
...
void StreamReplicator::deliverReceivedFrame() {
    ...
    // complete delivery to the 'primary replica' itself.
    StreamReplica* replica;
    while ((replica = fReplicasAwaitingCurrentFrame) != NULL) {
        fReplicasAwaitingCurrentFrame = replica->fNext;
        replica->fNext = NULL;
        ...
        replica->fFrameIndex =
            1 -
            replica->fFrameIndex;  // toggle it (0<->1), because this replica no
                                   // longer awaits the current frame
        ++fNumDeliveriesMadeSoFar;

        // Assert: fNumDeliveriesMadeSoFar < fNumActiveReplicas; // because we
        // still have the 'primary replica' to deliver to
    ...
    if (fNumDeliveriesMadeSoFar == fNumActiveReplicas - 1 &&
        ...
        if (fReplicasAwaitingNextFrame != NULL) {
            ...
            // so make it the next 'primary replica':
        ...
        // Complete delivery to the 'primary' replica (thereby completing all
