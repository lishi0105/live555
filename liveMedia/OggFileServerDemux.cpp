...
void OggFileServerDemux ::createNew(UsageEnvironment& env, char const* fileName,
    ...
    if (clientSessionId != 0 && clientSessionId == fLastClientSessionId) {
        demuxToUse =
            fLastCreatedDemux;  // use the same demultiplexor as before
                                ...
                                // tracks are created for one "OggDemux" at a
                                // time. Also, the "clientSessionId != 0" test
...
void OggFileServerDemux::onDemuxDeletion(void* clientData,
