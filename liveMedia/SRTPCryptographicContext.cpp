...
SRTPCryptographicContext ::SRTPCryptographicContext(
    MIKEYState const& mikeyState)
#ifndef NO_OPENSSL
    : fMIKEYState(mikeyState),
      fHaveReceivedSRTPPackets(False),
      fHaveSentSRTPPackets(False),
      fSRTCPIndex(0) {
    // Begin by doing a key derivation, to generate the keying data that we
    // need:
...
Boolean SRTPCryptographicContext ::processIncomingSRTPPacket(
    u_int8_t* buffer, unsigned inPacketSize, unsigned& outPacketSize) {
#ifndef NO_OPENSSL
    do {
        if (inPacketSize < 12) {  // For SRTP, 12 is the minimum packet size (if
                                  // unauthenticated)
#ifdef DEBUG
            fprintf(stderr,
                    "SRTPCryptographicContext::processIncomingSRTPPacket(): "
                    "Error: Packet size %d is too short for SRTP!\n",
                    inPacketSize);
#endif
            break;
        }
        ...
        if (inPacketSize <= numBytesPastEncryption) {
            ...
            fprintf(stderr,
                    "SRTPCryptographicContext::processIncomingSRTPPacket(): "
                    "Error: Packet size %d is too short for encrpytion and/or "
                    "authentication!\n",
                    inPacketSize);
#endif
            break;
        ...
        // Begin by figuring out this packet's 'index' (ROC and RTP sequence
        // number), and the ROC and RTP sequence number that will be used
        // subsequently, provided that this packet authenticates OK:
        ...
        if (!fHaveReceivedSRTPPackets) {
            ...
            // Check whether the sequence number has rolled over, or is
            // out-of-order:
        ...
        if (weAuthenticate()) {
            ...
            unsigned const numBytesToAuthenticate =
                inPacketSize -
                (SRTP_MKI_LENGTH + SRTP_AUTH_TAG_LENGTH);  // ASSERT: > 0
            u_int8_t const* authenticationTag =
                &buffer[inPacketSize - SRTP_AUTH_TAG_LENGTH];

            if (!verifySRTPAuthenticationTag(buffer, numBytesToAuthenticate,
                                             thisPacketsROC,
                                             authenticationTag)) {
#ifdef DEBUG
                fprintf(stderr,
                        "SRTPCryptographicContext::processIncomingSRTPPacket():"
                        " Failed to authenticate incoming SRTP packet!\n");
#endif
                break;
            }
        }
        ...
        if (weEncryptSRTP()) {
            // Decrypt the SRTP packet.  It has the index "thisPacketsROC" with
            // "rtpSeqNum":
            ...
            u_int32_t const SSRC = (buffer[8] << 24) | (buffer[9] << 16) |
                                   (buffer[10] << 8) | buffer[11];
            decryptSRTPPacket(index, SSRC, &buffer[offsetToEncryptedBytes],
                              numEncryptedBytes);
            outPacketSize =
                inPacketSize - numBytesPastEncryption;  // trim to what we use
        }
...
Boolean SRTPCryptographicContext ::processIncomingSRTCPPacket(
    u_int8_t* buffer, unsigned inPacketSize, unsigned& outPacketSize) {
#ifndef NO_OPENSSL
    do {
        if (inPacketSize < 12) {
            // For SRTCP, 8 is the minumum RTCP packet size, but there's also a
            // mandatory
            //   4-byte "E+SRTCP index" word.
#ifdef DEBUG
            fprintf(stderr,
                    "SRTPCryptographicContext::processIncomingSRTCPPacket(): "
        ...
        unsigned const numBytesPastEncryption =
            4 /*E+SRTCP index*/ + SRTP_MKI_LENGTH +
            (weAuthenticate() ? SRTP_AUTH_TAG_LENGTH : 0);
        if (inPacketSize <= numBytesPastEncryption) {
#ifdef DEBUG
            fprintf(stderr,
                    "SRTPCryptographicContext::processIncomingSRTCPPacket(): "
        ...
        if (weAuthenticate()) {
            ...
            if (!verifySRTCPAuthenticationTag(buffer, numBytesToAuthenticate,
                                              authenticationTag)) {
#ifdef DEBUG
                fprintf(stderr,
                        "SRTPCryptographicContext::processIncomingSRTCPPacket()"
        ...
        if (weEncryptSRTCP()) {
            ...
            if ((E_plus_SRTCPIndex & 0x80000000) != 0) {
                ...
                u_int32_t const SSRC = (buffer[4] << 24) | (buffer[5] << 16) |
                                       (buffer[6] << 8) | buffer[7];
                decryptSRTCPPacket(index, SSRC, &buffer[offsetToEncryptedBytes],
                                   numEncryptedBytes);
            }
            outPacketSize =
                inPacketSize - numBytesPastEncryption;  // trim to what we use
...
Boolean SRTPCryptographicContext ::processOutgoingSRTPPacket(
    u_int8_t* buffer, unsigned inPacketSize, unsigned& outPacketSize) {
#ifndef NO_OPENSSL
    do {
        unsigned const minRTPHeaderSize = 12;
        if (inPacketSize < minRTPHeaderSize) {  // packet is too small
            // Hack: Let small, non RTCP packets through w/o encryption; they
            // may be used to punch through NATs
        ...
        if (weEncryptSRTP()) {
            ...
            if ((buffer[0] & 0x10) != 0) {
                ...
                if (inPacketSize < rtpHeaderSize + 4) {
                    ...
                    fprintf(
                        stderr,
                        "SRTPCryptographicContext::processOutgoingSRTPPacket():"
                        " Error: Packet size %d is shorter than the minimum "
                        "specified RTP header size %d!\n",
                        inPacketSize, rtpHeaderSize + 4);
#endif
            ...
            if (!fHaveSentSRTPPackets) {
                ...
                if (rtpSeqNum == 0)
                    ++fSendingROC;  // increment the ROC when the RTP seq num
                                    // rolls over
            }
            u_int64_t index = (fSendingROC << 16) | rtpSeqNum;

            unsigned const numEncryptedBytes =
                inPacketSize - offsetToEncryptedBytes;  // ASSERT: >= 0
            u_int32_t const SSRC = (buffer[8] << 24) | (buffer[9] << 16) |
                                   (buffer[10] << 8) | buffer[11];
        ...
        if (weAuthenticate()) {
            ...
            // authentication tag. (Next, the MKI will take its place.)
            ...
            // Generate and add an authentication tag over the whole packet,
            // plus the ROC:
            outPacketSize += generateSRTPAuthenticationTag(
                buffer, outPacketSize, &buffer[outPacketSize]);
        }
...
Boolean SRTPCryptographicContext ::processOutgoingSRTCPPacket(
    u_int8_t* buffer, unsigned inPacketSize, unsigned& outPacketSize) {
#ifndef NO_OPENSSL
    do {
        // Encrypt the appropriate part of the packet.
        ...
        if (weEncryptSRTCP()) {
            ...
            if (inPacketSize < unencryptedHeaderSize) {  // packet is too small
                // Hack: Let small, non RTCP packets through w/o encryption;
                // they may be used to punch through NATs
                outPacketSize = inPacketSize;
                return True;
            }
            unsigned const encryptedDataSize =
                inPacketSize - unencryptedHeaderSize;

            u_int8_t* const dataToEncrypt = &buffer[unencryptedHeaderSize];
            u_int32_t const ssrc = (buffer[4] << 24) | (buffer[5] << 16) |
                                   (buffer[6] << 8) | buffer[7];
            encryptSRTCPPacket(fSRTCPIndex, ssrc, dataToEncrypt,
                               encryptedDataSize);
            eFlag = 0x80;
        }
        ...
        // Generate and add an authentication tag over the data built so far
        // (except the MKI)
        outPacketSize += generateSRTCPAuthenticationTag(
            buffer, outPacketSize - SRTP_MKI_LENGTH, &buffer[outPacketSize]);
...
#ifndef NO_OPENSSL
unsigned SRTPCryptographicContext ::generateSRTPAuthenticationTag(
...
Boolean SRTPCryptographicContext ::verifySRTPAuthenticationTag(
    u_int8_t* dataToAuthenticate, unsigned numBytesToAuthenticate,
    u_int32_t roc, u_int8_t const* authenticationTag) {
    // Append the (4-byte) 'ROC' (roll-over counter) to "dataToAuthenticate"
    // before computing the authentication tag.  We can do this because we have
    // enough space after "dataToAuthenticate":
    //   - If we're encrypted, then there's assumed to be a (4-byte) MKI there.
    //   Just overwrite
    //     that.  (If we need the MKI, we could copy it beforehand; later, allow
    //     for there being no MKI #####)
    //   - If we're not encrypted, then the ROC will overwrite part of the
    //   existing
    ...
    if (fMIKEYState.encryptSRTP() && haveMKI) {
        ...
        memcpy(authenticationTagCopy, authenticationTag,
               sizeof authenticationTagCopy);
    ...
    return verifyAuthenticationTag(fDerivedKeys.srtp, dataToAuthenticate,
                                   numBytesToAuthenticate,
                                   existingAuthenticationTag);
}

Boolean SRTPCryptographicContext ::verifySRTCPAuthenticationTag(
    u_int8_t const* dataToAuthenticate, unsigned numBytesToAuthenticate,
    u_int8_t const* authenticationTag) {
    return verifyAuthenticationTag(fDerivedKeys.srtcp, dataToAuthenticate,
                                   numBytesToAuthenticate, authenticationTag);
}
...
unsigned SRTPCryptographicContext ::generateAuthenticationTag(
    derivedKeys& keysToUse, u_int8_t const* dataToAuthenticate,
    unsigned numBytesToAuthenticate, u_int8_t* resultAuthenticationTag) {
    if (SRTP_AUTH_TAG_LENGTH > SHA1_DIGEST_LEN)
        return 0;  // sanity check; shouldn't happen
    u_int8_t computedAuthTag[SHA1_DIGEST_LEN];
    HMAC_SHA1(keysToUse.authKey, sizeof keysToUse.authKey, dataToAuthenticate,
              numBytesToAuthenticate, computedAuthTag);
...
Boolean SRTPCryptographicContext ::verifyAuthenticationTag(
    derivedKeys& keysToUse, u_int8_t const* dataToAuthenticate,
    unsigned numBytesToAuthenticate, u_int8_t const* authenticationTag) {
    u_int8_t computedAuthTag[SHA1_DIGEST_LEN];
    HMAC_SHA1(keysToUse.authKey, sizeof keysToUse.authKey, dataToAuthenticate,
              numBytesToAuthenticate, computedAuthTag);
...
void SRTPCryptographicContext::cryptData(derivedKeys& keys, u_int64_t index,
                                         u_int32_t ssrc, u_int8_t* data,
                                         unsigned numDataBytes) {
    // Begin by constructing the IV: (salt * 2^16) XOR (ssrc * 2^64) XOR (index
    // * 2^16)
    ...
    // (This is based upon the fact that SRTP_CIPHER_KEY_LENGTH ==
    // SRTP_CIPHER_SALT_LENGTH + 2)
    ...
    // Now generate as many blocks of the keystream as we need, by repeatedly
    // encrypting the IV using our cipher key.  (After each step, we increment
    // the IV by 1.) We then XOR the keystream into the provided data, to do the
    // en/decryption.
    do {
        EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
        if (ctx == NULL) break;

        if (EVP_EncryptInit(ctx, EVP_aes_128_ecb(), keys.cipherKey,
                            NULL /*no IV*/) != 1)
            break;
        ...
        while (numDataBytes > 0) {
            ...
            if (EVP_EncryptUpdate(ctx, keyStream, &numBytesEncrypted, iv,
                                  SRTP_CIPHER_KEY_LENGTH) != 1)
                break;

            unsigned numBytesToUse = numDataBytes < numBytesEncrypted
                                         ? numDataBytes
                                         : numBytesEncrypted;
...
#define deriveKey(label, resultKey) \
    deriveSingleKey(masterKey, salt, label, sizeof resultKey, resultKey)

void SRTPCryptographicContext ::deriveKeysFromMaster(
    u_int8_t const* masterKey, u_int8_t const* salt,
...
void SRTPCryptographicContext ::deriveSingleKey(u_int8_t const* masterKey,
    ...
    // Fill in the first bytes of "counter" with our 'salt'; set the remaining
    // bytes to zero:
    ...
    do {
        ...
        if (EVP_EncryptInit(ctx, EVP_aes_128_ecb(), masterKey,
                            NULL /*no IV*/) != 1)
            break;
        ...
        while (numBytesRemaining > 0) {
            ...
            int numBytesEncrypted;
            if (EVP_EncryptUpdate(ctx, ciphertext, &numBytesEncrypted,
                                  plaintext, KDF_PRF_CIPHER_BLOCK_LENGTH) != 1)
                break;

            unsigned numBytesToCopy = numBytesRemaining < numBytesEncrypted
                                          ? numBytesRemaining
                                          : numBytesEncrypted;
