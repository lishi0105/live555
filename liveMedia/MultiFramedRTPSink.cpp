...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env, Groupsock* rtpGS,
    ...
    if (numFrameBytesToUse > 0) {
        ...
        if (fOutBuf->wouldOverflow(frameSize)) {
            ...
            fOutBuf->setOverflowData(
                fOutBuf->curPacketSize() + numFrameBytesToUse, overflowBytes,
                presentationTime, durationInMicroseconds);
    ...
    if (fNumFramesUsedSoFar > 0) {
        ...
        if ((our_random() % 10) != 0)  // simulate 10% packet loss #####
            ...
            if (fCrypto != NULL) {  // Encrypt/tag the data before sending it:
#ifndef NO_OPENSSL
                // Hack: Because the MKI + authentication tag at the end of the
                // packet would overwrite any following (still to be sent) frame
                // data, we can't encrypt/tag the packet in place.  Instead, we
                // have to make a copy (on the stack) of the packet, before
                // encrypting/tagging/sending it:
                if (fOutBuf->curPacketSize() + SRTP_MKI_LENGTH +
                        SRTP_AUTH_TAG_LENGTH >
                    MAX_UDP_PACKET_SIZE) {
                    fprintf(
                        stderr,
                        "MultiFramedRTPSink::sendPacketIfNecessary(): Fatal "
                ...
                if (fCrypto->processOutgoingSRTPPacket(
                        packet, fOutBuf->curPacketSize(), newPacketSize)) {
