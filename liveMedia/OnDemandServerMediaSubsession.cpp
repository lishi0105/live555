...
OnDemandServerMediaSubsession ::OnDemandServerMediaSubsession(
    UsageEnvironment& env, Boolean reuseFirstSource, portNumBits initialPortNum,
    ...
    if (fSDPLines == NULL) {
        ...
        if (dummyRTPSink != NULL) {
            if (fParentSession->streamingUsesSRTP) {
                fMIKEYStateMessage = dummyRTPSink->setupForSRTP(
                    fParentSession->streamingIsEncrypted,
...
void OnDemandServerMediaSubsession ::getStreamParameters(
        ...
        if (clientRTPPort.num() != 0 ||
            ...
            if (clientRTCPPort.num() == 0) {
                ...
                for (serverPortNum = fInitialPortNum;; ++serverPortNum) {
                    ...
                    rtpGroupsock = createGroupsock(
                        nullAddress(destinationAddress.ss_family),
                        serverRTPPort);
                ...
                rtpSink = mediaSource == NULL
                              ? NULL
                              : createNewRTPSink(rtpGroupsock, rtpPayloadType,
                                                 mediaSource);
                if (rtpSink != NULL) {
                    if (fParentSession->streamingUsesSRTP) {
    ...
    if (tcpSocketNum < 0) {  // UDP
        destinations =
            new Destinations(destinationAddress, clientRTPPort, clientRTCPPort);
    } else {  // TCP
...
void OnDemandServerMediaSubsession ::setStreamSourceDuration(
    FramedSource* /*inputSource*/, double /*streamDuration*/,
    u_int64_t& numBytes) {
    // Default implementation: Do nothing
    numBytes = 0;
}
...
void OnDemandServerMediaSubsession ::sendRTCPAppPacket(
    u_int8_t subtype, char const* name, u_int8_t* appDependentData,
...
void OnDemandServerMediaSubsession ::setSDPLinesFromRTPSink(
    ...
    char const* const sdpFmt =
        ...
        "a=control:%s\r\n";
    unsigned sdpFmtSize =
        strlen(sdpFmt) + strlen(mediaType) + 5 /* max short len */ + 1 +
        3                                                    /* max char len */
        + 3 /*IP4 or IP6*/ + strlen(ipAddressStr.val()) + 20 /* max int len */
        + strlen(rtpmapLine) + strlen(keyMgmtLine) + strlen(rtcpmuxLine) +
        strlen(rangeLine) + strlen(auxSDPLine) + strlen(trackId());
...
void StreamState ::startPlaying(
    Destinations* dests, unsigned clientSessionId, TaskFunc* rtcpRRHandler,
    ...
    if (dests->isTCP) {
        ...
        if (fRTCPInstance != NULL) {
            fRTCPInstance->addStreamSocket(
                dests->tcpSocketNum, dests->rtcpChannelId, dests->tlsState);
            ...
            fRTCPInstance->setSpecificRRHandler(
                tcpSocketNumAsAddress, dests->rtcpChannelId, rtcpRRHandler,
  ...
  if (fRTCPInstance != NULL) {
    ...
    if (dests->isTCP) {
        if (fRTPSink != NULL) {
            ...
            // "RTSPClientConnection" object from being closed after handling a
            // "TEARDOWN": #####
