...
public:
    AVIIndexRecord(unsigned chunkId, unsigned flags, unsigned offset,
                   unsigned size)
        : fNext(NULL),
          fChunkId(chunkId),
          fFlags(flags),
...
AVIFileSink::AVIFileSink(UsageEnvironment& env, MediaSession& inputSession,
...
size +=
    addWord(AVIF_TRUSTCKTYPE | AVIF_HASINDEX | AVIF_ISINTERLEAVED);  // dwFlags
