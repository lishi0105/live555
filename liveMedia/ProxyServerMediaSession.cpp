...
ProxyServerMediaSession* ProxyServerMediaSession ::createNew(
    ...
    char const* inputStreamURL, char const* streamName, char const* username,
...
Groupsock* ProxyServerMediaSession ::createGroupsock(
    struct sockaddr_storage const& addr, Port port) {
...
RTCPInstance* ProxyServerMediaSession ::createRTCP(
    Groupsock* RTCPgs, unsigned totSessionBW, /* in kbps */
    unsigned char const* cname, RTPSink* sink) {
    // Default implementation; may be redefined by subclasses:
...
static void continueAfterOPTIONS(RTSPClient* rtspClient, int resultCode,
                                 char* resultString) {
...
ProxyRTSPClient::ProxyRTSPClient(ProxyServerMediaSession& ourServerMediaSession,
    ...
    : RTSPClient(ourServerMediaSession.envir(), rtspURL, verbosityLevel,
                 "ProxyRTSPClient",
...
ProxyServerMediaSubsession ::ProxyServerMediaSubsession(
    MediaSubsession& mediaSubsession, portNumBits initialPortNum,
    Boolean multiplexRTCPWithRTP)
...
FramedSource* ProxyServerMediaSubsession::createNewStreamSource(
    ...
    if (fClientMediaSubsession.readSource() == NULL) {
        if (sms->fTranscodingTable == NULL ||
            ...
            fClientMediaSubsession
                .receiveRawMP3ADUs();  // hack for proxying MPA-ROBUST streams
        if (sms->fTranscodingTable == NULL ||
            !sms->fTranscodingTable->weWillTranscode("video", "JPEG"))
            fClientMediaSubsession
                .receiveRawJPEGFrames();  // hack for proxying JPEG/RTP streams.
            ...
            FramedFilter* normalizerFilter =
                sms->fPresentationTimeSessionNormalizer
                    ->createNewPresentationTimeSubsessionNormalizer(
                        ...
                        fClientMediaSubsession.rtpSource(), fCodecName);
            ...
            if (strcmp(fCodecName, "H264") == 0) {
                ...
                fClientMediaSubsession.addFilter(
                    MPEG1or2VideoStreamDiscreteFramer ::createNew(
                        envir(), fClientMediaSubsession.readSource(), False,
                        5.0, True /* leave PTs unmodified*/));
...
void ProxyServerMediaSubsession::closeStreamSource(
    FramedSource* /*inputSource*/) {
    if (verbosityLevel() > 0) {
    ...
    if (fHaveSetupStream) {
        ...
        ProxyRTSPClient* const proxyRTSPClient = sms->fProxyRTSPClient;
        if (proxyRTSPClient
                ->fLastCommandWasPLAY) {  // so that we send only one "PAUSE";
                                          // not one for each subsession
            if (fParentSession->referenceCount() > 1) {
                ...
                // whole stream. In principle, we would send a "PAUSE" only for
                // the sub-stream here, but some back-end servers might
                // mis-handle that by pausing the entire stream. So instead, we
                // do nothing here.
                // proxyRTSPClient->sendPauseCommand(fClientMediaSubsession,
                // NULL, proxyRTSPClient->auth());
            } else {
                // Normal case: There are no other clients still streaming
                // (parts of) this stream. Send a "PAUSE" for the whole stream.
...
RTPSink* ProxyServerMediaSubsession ::createNewRTPSink(
    ...
    if (strcmp(fCodecName, "H264") == 0 || strcmp(fCodecName, "H265") == 0 ||
        ...
        // it:
        ssNormalizer =
            (PresentationTimeSubsessionNormalizer*)(((FramedFilter*)inputSource)
                                                        ->inputSource());
    } else {
