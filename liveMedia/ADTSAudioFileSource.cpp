...
#endif
        return new ADTSAudioFileSource(
            env, fid, profile, sampling_frequency_index, channel_configuration);
    } while (0);
...
ADTSAudioFileSource ::ADTSAudioFileSource(UsageEnvironment& env, FILE* fid,
                                          u_int8_t profile,
...
void ADTSAudioFileSource::doGetNextFrame() {
    ...
    fprintf(
        stderr,
        "Read frame: syncword 0x%x, protection_absent %d, frame_length %d\n",
