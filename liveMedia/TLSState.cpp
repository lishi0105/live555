...
int TLSState::read(u_int8_t* buffer, unsigned bufferSize) {
    ...
    if (result <= 0) {
        if (SSL_get_error(fCon, result) == SSL_ERROR_WANT_READ) {
            // The data can't be delivered yet.  Return 0 (bytes read); we'll
            // try again later
            return 0;
        }
    ...
    if (sslConnectResult > 0) {
        ...
        fClient.envir().taskScheduler().setBackgroundHandling(
            ...
            sslGetErrorResult == SSL_ERROR_WANT_READ ? SOCKET_READABLE
                                                     : SOCKET_WRITABLE,
            (TaskScheduler::BackgroundHandlerProc*)&RTSPClient::
                connectionHandler,
            &fClient);
...
void ServerTLSState ::setCertificateAndPrivateKeyFileNames(
