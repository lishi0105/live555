...
public:
    ChunkDescriptor(int64_t offsetInFile, unsigned size, unsigned frameSize,
                    unsigned frameDuration, struct timeval presentationTime);
...
private:
    ...
    void useFrameForHinting(unsigned frameSize, struct timeval presentationTime,
                            unsigned startSampleNumber);

    // used by the above two routines:
    unsigned useFrame1(unsigned sourceDataSize, struct timeval presentationTime,
...
private:
    ...
    struct {
        ...
        unsigned rtpHeader;
        unsigned char
            numSpecialHeaders;  // used when our RTP source has special headers
...
QuickTimeFileSink::QuickTimeFileSink(
    UsageEnvironment& env, MediaSession& inputSession,
    char const* outputFileName, unsigned bufferSize, unsigned short movieWidth,
...
QuickTimeFileSink* QuickTimeFileSink::createNew(
    UsageEnvironment& env, MediaSession& inputSession,
    char const* outputFileName, unsigned bufferSize, unsigned short movieWidth,
    unsigned short movieHeight, unsigned movieFPS, Boolean packetLossCompensate,
    ...
    QuickTimeFileSink* newSink = new QuickTimeFileSink(
        env, inputSession, outputFileName, bufferSize, movieWidth, movieHeight,
        movieFPS, packetLossCompensate, syncStreams, generateHintTracks,
        generateMP4Format);
...
void QuickTimeFileSink ::noteRecordedFrame(
    MediaSubsession& /*inputSubsession*/, unsigned /*packetDataSize*/,
...
void QuickTimeFileSink ::afterGettingFrame(
    void* clientData, unsigned packetDataSize, unsigned numTruncatedBytes,
...
Boolean SubsessionIOState::setQTstate() {
    ...
    char const* noCodecWarning3 =
        "\" track, so we'll insert a dummy \"????\" Media Data Atom instead.  "
        "A separate, codec-specific editing pass will be needed before this "
        "track can be played.\n";
    ...
    envir()
        << ", so a track for the \"" << fOurSubsession.mediumName() << "/"
        << fOurSubsession.codecName()
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize,
    ...
    if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
        ...
        for (unsigned i = 0; i < numPTEntries; ++i) {
            ...
            unsigned dataFrameSize =
                (i + 1 < numPTEntries) ? maxPacketSize
    ...
    fPrevFrameState.rtpHeader =
        rs->curPacketMarkerBit() << 23 | (rs->rtpPayloadFormat() & 0x7F) << 16;
    if (hack263) {
...
unsigned SubsessionIOState::useFrame1(unsigned sourceDataSize,
    ...
    if (fTailChunk == NULL) {
        ...
        newTailChunk =
            fTailChunk->extendChunk(destFileOffset, sourceDataSize, frameSize,
                                    frameDuration, presentationTime);
    }
...
ChunkDescriptor ::ChunkDescriptor(int64_t offsetInFile, unsigned size,
...
ChunkDescriptor* ChunkDescriptor ::extendChunk(
    int64_t newOffsetInFile, unsigned newSize, unsigned newFrameSize,
    ...
    unsigned QuickTimeFileSink::addAtom_genericMedia() {
        ...
        unsigned QuickTimeFileSink::addAtom_soundMediaGeneral() {
            ...
            unsigned QuickTimeFileSink::addAtom_Qclp() {
                ...
                addAtom(frma);
                size +=
                    add4ByteString(fCurrentIOState->fQTAudioDataType);  // ???
                addAtomEnd;
                ...
                unsigned QuickTimeFileSink::addAtom_mp4a() {
                    ...
                    addAtom(esds);
                    // Get the source's 'config' information.  (We need the size
                    // now.)
                    unsigned configSize;
                    MediaSubsession& subsession =
                    ...
                    if (strcmp(subsession.mediumName(), "audio") ==
                        ...
                        size += addWord(0x00000000);  // ???
                        size +=
                            addWord(0x03808080);  // ES_DescrTag+length coding
                        size += addWord((31 + configSize)
                                        << 24);  // length (includes 'config'
                                                 // and SLConfigDescriptor)
                        // DecoderConfigDescriptor
                        size += addWord(0x04808080);  // DecoderConfigDescrTag +
                                                      // length coding
                        size += addWord(
                            ((17 + configSize) << 24) |
                            0x00401500);  // length (includes 'config' info)
                        ...
                        size += addByte(0x80);  // ???
                               ...
                               0) {                   // MPEG-4 video
                        size += addWord(0x00000000);  // ???
                        size += addWord(((22 + configSize) << 16) |
                                        0x03000000);  // ES_DescrTag + length
                                                      // (incl. 'config')
                    ...
                    if (strcmp(subsession.mediumName(), "audio") ==
                        0) {  // MPEG-4 audio
                        size += addWord(
                            0x06808080);  // SLConfigDescrTag + length coding
                        size += addHalfWord(0x0102);  // length(1) + ???
                    } else {                          // MPEG-4 video
                        size += addHalfWord(
                            0x0601);            // SLConfigDescrTag + length(1)
                        size += addByte(0x02);  // ???
                    }
                    addAtomEnd;
                    ...
                    size += addWord(0x00000000);  // Reserved
                    size +=
                        addWord(0x00000001);  // Reserved+Data reference index
                    ...
                    size +=
                        addZeroWords(6);  // Compressor name (continued - zero)
                    size +=
                        addWord(0x00000018);  // Compressor name (final)+Depth
                    ...
                    size +=
                        addZeroWords(4);  // Compressor name (continued - zero)
                    size +=
                        addWord(0x00000018);  // Compressor name (final)+Depth
                    size += addHalfWord(0xffff);  // Color table id
                    size += addAtom_esds();       // ESDescriptor
                    ...
                    unsigned QuickTimeFileSink::addAtom_rtp() {
                        ...
                        if (haveSingleEntryTable) {
                            if (fCurrentIOState->isHintTrack() &&
                                ...
                                sampleSize =
                                    fCurrentIOState->fHeadChunk->fFrameSize /
                                    fCurrentIOState->fQTSamplesPerFrame;
                            } else {
                                // The following doesn't seem right, but seems
                                // to do the right thing:
                                sampleSize = fCurrentIOState
                                                 ->fQTTimeUnitsPerSample;  //???
                            }
                        } else {
                        ...
                        size += addWord(0x00000000);  // Version+flags
                        size += addWord(
                            fCurrentIOState->fNumChunks);  // Number of entries
