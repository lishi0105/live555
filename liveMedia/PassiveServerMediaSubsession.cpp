...
char const* PassiveServerMediaSubsession::sdpLines(int /*addressFamily*/) {
    if (fSDPLines == NULL) {
        ...
        if (fParentSession
                ->streamingUsesSRTP) {  // Hack to set up for SRTP/SRTCP
        ...
        unsigned sdpFmtSize =
            ...
            3 /* max char len */
            + 3 /*IP4 or IP6*/ + strlen(groupAddressStr.val()) +
            3    /* max char len */
            + 20 /* max int len */
        ...
        sprintf(sdpLines, sdpFmt,
                ...
                gs.groupAddress().ss_family == AF_INET
                    ? "IP4"
                    : "IP6",            // c= address type
...
void PassiveServerMediaSubsession ::getStreamParameters(
    ...
    if (addressIsNull(destinationAddress)) {
        ...
        if (fRTCPInstance != NULL) {
            ...
            rtcpGS->changeDestinationParameters(destinationAddress, 0,
                                                destinationTTL);
        }
    }
...
void PassiveServerMediaSubsession::startStream(
    ...
    unsigned streamBitrate =
        fRTCPInstance == NULL ? 50 : fRTCPInstance->totSessionBW();  // in kbps
    unsigned rtpBufSize =
...
void PassiveServerMediaSubsession ::getRTPSinkandRTCP(void* /*streamToken*/,
                                                      RTPSink*& rtpSink,
                                                      RTCPInstance*& rtcp) {
    rtpSink = &fRTPSink;
    rtcp = fRTCPInstance;
