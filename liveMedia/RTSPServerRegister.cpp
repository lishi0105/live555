...
static void rtspRegisterResponseHandler(RTSPClient* rtspClient, int resultCode,
...
public:
    RegisterRequestRecord(
        ...
        : RTSPRegisterSender(
              ourServer.envir(), remoteClientNameOrAddress, remoteClientPortNum,
...
unsigned RTSPServer::registerStream(ServerMediaSession* serverMediaSession,
    ...
    new RegisterRequestRecord(
        *this, requestId, remoteClientNameOrAddress, remoteClientPortNum, url,
        responseHandler, authenticator, receiveOurStreamViaTCP, proxyURLSuffix);

    delete[](char*) url;   // we can do this here because it was copied to the
                           // "RegisterRequestRecord"
...
public:
    DeregisterRequestRecord(
        ...
        Authenticator* authenticator, char const* proxyURLSuffix)
        : RTSPDeregisterSender(ourServer.envir(), remoteClientNameOrAddress,
                               remoteClientPortNum, rtspURLToDeregister,
                               rtspDeregisterResponseHandler, authenticator,
                               proxyURLSuffix,
#ifdef DEBUG
                               1 /*verbosityLevel*/,
#else
...
unsigned RTSPServer::deregisterStream(
    ...
    char const* remoteClientNameOrAddress, portNumBits remoteClientPortNum,
    responseHandlerForDEREGISTER* responseHandler, char const* username,
    char const* password, char const* proxyURLSuffix) {
...
void RTSPServer ::RTSPClientConnection::handleCmd_REGISTER(
    char const* cmd /*"REGISTER" or "DEREGISTER"*/, char const* url,
    char const* urlSuffix, char const* fullRequestStr, Boolean reuseConnection,
    Boolean deliverViaTCP, char const* proxyURLSuffix) {
    char* responseStr;
    if (fOurRTSPServer.weImplementREGISTER(cmd, proxyURLSuffix, responseStr)) {
        ...
        ParamsForREGISTER* registerParams =
            new ParamsForREGISTER(cmd, this, url, urlSuffix, reuseConnection,
...
void parseTransportHeaderForREGISTER(char const* buf, Boolean& reuseConnection,
...
void RTSPServer::RTSPClientConnection::continueHandlingREGISTER1(
    ...
    if (socketNumToBackEndServer >= 0) {
        ...
        delete this;
    } else if (!fIsActive && fRecursionCount <= 0 &&
...
RTSPServerWithREGISTERProxying* RTSPServerWithREGISTERProxying ::createNew(
    ...
    return new RTSPServerWithREGISTERProxying(
        env, ourSocketIPv4, ourSocketIPv6, ourPort, authDatabase,
        authDatabaseForREGISTER, reclamationSeconds, streamRTPOverTCP,
        verbosityLevelForProxying, backEndUsername, backEndPassword);
}

RTSPServerWithREGISTERProxying ::RTSPServerWithREGISTERProxying(
    UsageEnvironment& env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort,
    UserAuthenticationDatabase* authDatabase,
    UserAuthenticationDatabase* authDatabaseForREGISTER,
    unsigned reclamationSeconds, Boolean streamRTPOverTCP,
    int verbosityLevelForProxying, char const* backEndUsername,
...
Boolean RTSPServerWithREGISTERProxying ::weImplementREGISTER(
    char const* cmd /*"REGISTER" or "DEREGISTER"*/, char const* proxyURLSuffix,
    char*& responseStr) {
...
void RTSPServerWithREGISTERProxying ::implementCmd_REGISTER(
    ...
    if (strcmp(cmd, "REGISTER") == 0) {
        ...
        ServerMediaSession* sms = ProxyServerMediaSession::createNew(
            ...
            fBackEndPassword, tunnelOverHTTPPortNum, fVerbosityLevelForProxying,
        ...
        lookupServerMediaSession(proxyStreamName,
