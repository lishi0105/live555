...
ServerMediaSession* ServerMediaSession ::createNew(
    UsageEnvironment& env, char const* streamName, char const* info,
...
char* ServerMediaSession::generateSDPDescription(int addressFamily) {
    ...
    if (addressFamily == AF_INET) {
        ...
        for (unsigned i = 0; i < 16; ++i) {
            ((sockaddr_in6&)ourAddress).sin6_addr.s6_addr[i] =
                ourIPv6Address(envir())[i];
        }
    }
    ...
    if (fIsSSM) {
        ...
        sprintf(sourceFilterLine, sourceFilterFmt,
                addressFamily == AF_INET ? "IP4" : "IP6", ipAddressStr.val());
    } else {
    ...
    do {
        ...
        char const* const sdpPrefixFmt =
            ...
            "a=x-qt-text-inf:%s\r\n"
            "%s";
        sdpLength += strlen(sdpPrefixFmt) + 20 + 6 + 20 + 3 /*IP4 or IP6*/ +
                     ipAddressStrSize + strlen(fDescriptionSDPString) +
                     strlen(fInfoSDPString) + strlen(libNameStr) +
                     strlen(libVersionStr) + strlen(sourceFilterLine) +
                     strlen(rangeLine) + strlen(fDescriptionSDPString) +
        ...
        snprintf(
            sdp, sdpLength, sdpPrefixFmt, fCreationTime.tv_sec,
...
void ServerMediaSubsession::seekStream(unsigned /*clientSessionId*/,
                                       void* /*streamToken*/, char*& absStart,
                                       char*& absEnd) {
