...
private:
    AMRDeinterleaver(UsageEnvironment& env, Boolean isWideband,
...
AMRAudioSource* AMRAudioRTPSource::createNew(
    UsageEnvironment& env, Groupsock* RTPgs, RTPSource*& resultRTPSource,
    ...
    resultRTPSource = rawRTPSource = RawAMRRTPSource::createNew(
        env, RTPgs, rtpPayloadFormat, isWideband, isOctetAligned, isInterleaved,
        CRCsArePresent);
...
RawAMRRTPSource* RawAMRRTPSource::createNew(
...
unsigned AMRBufferedPacket::nextEnclosedFrameSize(unsigned char*& framePtr,
    ...
    if (frameSize == FT_INVALID) {
        // Strange TOC entry!
        fOurSource.envir()
            << "AMRBufferedPacket::nextEnclosedFrameSize(): invalid FT: " << FT
            << "\n";
        frameSize =
...
AMRDeinterleaver::AMRDeinterleaver(UsageEnvironment& env, Boolean isWideband,
                                   unsigned numChannels,
                                   unsigned maxInterleaveGroupSize,
                                   RawAMRRTPSource* inputSource)
...
void AMRDeinterleavingBuffer ::deliverIncomingFrame(
    unsigned frameSize, RawAMRRTPSource* source,
    struct timeval presentationTime) {
    fILL = source->ILL();
...
Boolean AMRDeinterleavingBuffer ::retrieveFrame(
    unsigned char* to, unsigned maxSize, unsigned& resultFrameSize,
