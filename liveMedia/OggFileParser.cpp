...
void OggFileParser::continueParsing(void* clientData, unsigned char* ptr,
                                    unsigned size,
            ...
            for (unsigned j = 0; j < fPacketSizeTable->numCompletedPackets &&
                ...
                if (isVorbis) {
                    ...
                    headerIsKnown = firstByte == 0x80 || firstByte == 0x81 ||
                                    firstByte == 0x82;
                    index =
                        firstByte & ~0x80;  // 0x80, 0x81, or 0x82 => 0, 1, or 2
                } else {                    // Opus
    ...
    for (unsigned i = 0; i < vorbis_mapping_count; ++i) {
        ...
        if (bv.getBits(1)) {  // "square polar channel mapping is in use"
            ...
            for (unsigned j = 0; j < vorbis_mapping_coupling_steps; ++j) {
                ...
                bv.skipBits(
                    2 * ilog_audio_channels_minus_1);  // "vorbis_mapping_magnitude",
...
static Boolean parseVorbisSetupHeader(OggTrack* track, u_int8_t const* p,
                                      unsigned headerSize) {
    ...
    do {
        ...
        if (framingFlag == 0) {
            fprintf(
                stderr,
                "Vorbis \"setup\" header did not end with a 'framing flag'!\n");
            break;
        }
...
Boolean OggFileParser::validateHeader(OggTrack* track, u_int8_t const* p,
                                      unsigned headerSize) {
...
Boolean OggFileParser::deliverPacketWithinPage() {
    ...
    if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
        ...
        if (firstByte == 0x4F /*'O'*/ &&
            ...
            unsigned const numFramesInPacket = c == 0   ? 1
                                               : c == 3 ? (secondByte & 0x3F)
                                                        : 2;
