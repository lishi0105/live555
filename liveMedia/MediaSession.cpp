...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
    ...
    while (sdpLine != NULL) {
        ...
        if ((sscanf(sdpLine, "m=%s %hu RTP/AVP %u", mediumName,
            ...
            payloadFormat <= 127) {
            protocolName = "RTP";
        } else if ((sscanf(sdpLine, "m=%s %hu RTP/SAVP %u", mediumName,
                           &subsession->fClientPortNum, &payloadFormat) == 3 ||
...
static Boolean parseStringValue(char const* sdpLine, char const* searchFormat,
                                char*& result) {
...
static Boolean parseTwoStringValues(char const* sdpLine,
                                    char const* searchFormat, char*& result1,
                                    char*& result2) {
    Boolean parseSuccess = False;
...
static MIKEYState* parseSDPAttribute_key_mgmtToMIKEY(char const* sdpLine) {
    ...
    do {
        ...
        if (!parseTwoStringValues(sdpLine, "a=key-mgmt:%s %s", keyMgmtPrtclId,
                                  keyMgmtData))
            break;
        ...
        u_int8_t* keyMgmtData_decoded =
            base64Decode(keyMgmtData, keyMgmtData_decodedSize);
        if (keyMgmtData_decoded == NULL) break;

        resultMIKEYState =
            MIKEYState::createNew(keyMgmtData_decoded, keyMgmtData_decodedSize);
...
static Boolean parseRangeAttribute(char const* sdpLine, double& startTime,
                                   double& endTime) {
    return sscanf(sdpLine, "a=range: npt = %lg - %lg", &startTime, &endTime) ==
           2;
}

static Boolean parseRangeAttribute(char const* sdpLine, char*& absStartTime,
                                   char*& absEndTime) {
...
static Boolean parseSourceFilterAttribute(
    char const* sdpLine, struct sockaddr_storage& sourceFilterAddr) {
    // Check for a "a=source-filter:incl IN IP4 <something> <source>" line (or
    // "IN IP6"). Note: At present, we don't check that <something> really
    // matches one of our multicast addresses.  We also don't support more than
    // one <source> #####
    Boolean result = False;  // until we succeed
    char* sourceName = NULL;
    do {
        int addressFamily;
        if (parseStringValue(sdpLine, "a=source-filter: incl IN IP4 %*s %s",
                             sourceName)) {
            addressFamily = AF_INET;
        } else if (parseStringValue(sdpLine,
                                    "a=source-filter: incl IN IP6 %*s %s",
                                    sourceName)) {
...
MediaSubsession::MediaSubsession(MediaSession& parent)
    ...
    setAttribute(
        ...
        True /*value is hexadecimal*/);  // used with "video/H264"
                                         // This won't work for MPEG-4 (unless
                                         // the value is <10), because for
                                         // MPEG-4, the value is assumed to be a
                                         // decimal string, not a hexadecimal
                                         // string.  NEED TO FIX #####
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
    ...
    do {
        ...
        Boolean const protocolIsRTP =
            useSRTP || strcmp(fProtocolName, "RTP") == 0;

        if (fClientPortNum != 0 &&
            (honorSDPPortChoice || IsMulticastAddress(tempAddr))) {
                ...
                if (fMultiplexRTCPWithRTP) {
                    ...
                    if (isSSM()) {
                        fRTCPSocket = new Groupsock(
                            env(), tempAddr, fSourceFilterAddr, rtcpPortNum);
                    } else {
                        fRTCPSocket =
            ...
            while (1) {
                ...
                if (fRTPSocket == NULL) {
                    env().setResultMsg(
                        "MediaSession::initiate(): unable to create RTP and "
                        "RTCP sockets");
                    break;
                }
        ...
        if (useSRTP) {
            // For SRTP, we need key management.  If MIKEY (key management)
            // state wasn't given to us in the SDP description, then create it
            // now:
            ourCrypto = getCrypto();
            if (ourCrypto ==
                NULL) {  // then fMIKEYState is also NULL; create both
                fMIKEYState = new MIKEYState();
                ourCrypto = fCrypto =
                    new SRTPCryptographicContext(*fMIKEYState);
            }
        ...
        if (fRTPSource != NULL && fRTCPSocket != NULL) {
            ...
            fRTCPInstance = RTCPInstance::createNew(
                ...
                False /* we're not a data transmitter */, ourCrypto);
...
void MediaSubsession::setDestinations(
    struct sockaddr_storage const& defaultDestAddress) {
    ...
    if (fRTCPSocket != NULL && !isSSM() && !fMultiplexRTCPWithRTP) {
        ...
        fRTCPSocket->changeDestinationParameters(destAddress, destPort,
                                                 destTTL);
    }
}
    ...
    do {
        ...
        if (strcmp(fProtocolName, "UDP") == 0) {
            ...
            if (strcmp(fCodecName, "MP2T") == 0) {  // MPEG-2 Transport Stream
                fReadSource =
                    MPEG2TransportStreamFramer::createNew(env(), fReadSource);
            ...
            if (strcmp(fCodecName, "QCELP") == 0) {  // QCELP audio
                ...
                fReadSource =
                    MPEG2TransportStreamFramer::createNew(env(), fRTPSource);
                // this sets "durationInMicroseconds" correctly, based on the
                // PCR values
