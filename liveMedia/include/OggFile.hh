...
typedef void OggDemuxOnDeletionFunc(void* objectToNotify,
...
public:
    ...
    // (to parse the Ogg track headers) before a new object can be initialized,
    // the creation of a new object is signalled by calling - from the event
    // loop - an 'onCreationFunc' that is passed as a parameter to
    // "createNew()".

    OggTrack* lookup(u_int32_t trackNumber);

    OggDemux* newDemux(OggDemuxOnDeletionFunc* onDeletionFunc = NULL,
    ...
    RTPSink* createRTPSinkForTrackNumber(u_int32_t trackNumber,
...
private:
    OggFile(UsageEnvironment& env, char const* fileName,
...
public:
    ...
    Boolean weNeedHeaders() const {
        return vtoHdrs.header[0] == NULL || vtoHdrs.header[1] == NULL ||
               (vtoHdrs.header[2] == NULL &&
                strcmp(mimeType, "audio/OPUS") != 0);
    }
};
...
public:
    ...
    // media track from the file.  This function returns NULL when no more media
    // tracks exist.
