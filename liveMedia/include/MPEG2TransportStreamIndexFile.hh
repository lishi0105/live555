...
public:
    ...
    void lookupPCRFromTSPacketNum(unsigned long& tsPacketNumber,
    ...
    Boolean readIndexRecordValues(unsigned long indexRecordNum,
                                  ...
                                  u_int8_t& offset, u_int8_t& size, float& pcr,
                                  u_int8_t& recordType);
    ...
    // the underlying Transport Stream file. (1,2,4, or 5 (representing H.264).
    // 0 means 'don't know' (usually because the index file is empty))

private:
    MPEG2TransportStreamIndexFile(UsageEnvironment& env,
                                  char const* indexFileName);

    Boolean openFid();
    Boolean seekToIndexRecord(unsigned long indexRecordNumber);
    Boolean readIndexRecord(unsigned long indexRecordNum);  // into "fBuf"
    Boolean readOneIndexRecord(
        unsigned long indexRecordNum);  // closes "fFid" at end
    void closeFid();
...
private:
    ...
    unsigned char
        fBuf[INDEX_RECORD_SIZE];  // used for reading index records from file
};
