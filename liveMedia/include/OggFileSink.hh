...
public:
    static OggFileSink* createNew(
        ...
        // "comment", "setup") to prepend to the output.  (For 'Vorbis" audio
        // and 'Theora' video.)
        unsigned bufferSize = 100000, Boolean oneFilePerFrame = False);
    // See "FileSink.hh" for a description of these parameters.

protected:
    OggFileSink(UsageEnvironment& env, FILE* fid, unsigned samplingFrequency,
                char const* configStr, unsigned bufferSize,
...
private:
    ...
    int64_t fGranulePositionAdjustment;  // used to ensure that
                                         // "fGranulePosition" stays monotonic
    ...
    // Because the last Ogg page before EOF needs to have a special 'eos' bit
    // set in the header, we need to defer the writing of each incoming frame.
    // To do this, we maintain a 2nd buffer:
