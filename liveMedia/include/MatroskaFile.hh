...
typedef void MatroskaDemuxOnDeletionFunc(void* objectToNotify,
...
public:
    ...
    // object immediately.  Instead, because this class requires file reading
    // (to parse the Matroska 'Track' headers) before a new object can be
    // initialized, the creation of a new object is signalled by calling - from
    // the event loop - an 'onCreationFunc' that is passed as a parameter to
    // "createNew()".
    ...
    FramedSource* createSourceForStreaming(FramedSource* baseSource,
                                           unsigned trackNumber,
                                           unsigned& estBitrate,
                                           unsigned& numFiltersInFrontOfTrack);
    // Takes a data source (which must be a demultiplexed track from this file)
    // and returns a (possibly modified) data source that can be used for
    // streaming.

    char const* trackMIMEType(unsigned trackNumber) const;
    // in the form "<medium-name>/<CODEC-NAME>", or NULL if no such track exists

    RTPSink* createRTPSinkForTrackNumber(unsigned trackNumber,
                                         Groupsock* rtpGroupsock,
                                         unsigned char rtpPayloadTypeIfDynamic);
    // Creates a "RTPSink" object that would be appropriate for streaming the
    // specified track, or NULL if no appropriate "RTPSink" exists

    FileSink* createFileSinkForTrackNumber(unsigned trackNumber,
                                           char const* fileName);
    // Creates a "FileSink" object that would be appropriate for recording the
    // contents of the specified track, or NULL if no appropriate "FileSink"
    // exists.

private:
    MatroskaFile(UsageEnvironment& env, char const* fileName,
    ...
    void addCuePoint(double cueTime, u_int64_t clusterOffsetInFile,
                     unsigned blockNumWithinCluster);
    Boolean lookupCuePoint(double& cueTime,
    ...
    void getH264ConfigData(MatroskaTrack const* track, u_int8_t*& sps,
                           unsigned& spsSize, u_int8_t*& pps,
                           unsigned& ppsSize);
    // "sps","pps" are dynamically allocated by this function, and must be
    // delete[]d afterwards
    void getH265ConfigData(MatroskaTrack const* track, u_int8_t*& vps,
                           unsigned& vpsSize, u_int8_t*& sps, unsigned& spsSize,
                           u_int8_t*& pps, unsigned& ppsSize);
    // "vps","sps","pps" are dynamically allocated by this function, and must be
    // delete[]d afterwards
    ...
    // "identificationHeader", "commentHeader", "setupHeader" are dynamically
    // allocated by this function, and must be delete[]d afterwards
...
private:
    ...
    unsigned fChosenVideoTrackNumber, fChosenAudioTrackNumber,
...
// set of track types as a bitmap, representing a set: (Note that
// MATROSKA_TRACK_TYPE_OTHER must be last, and have the largest value.)
...
public:
    ...
    // preferred media track (video, audio, subtitle - in that order) from the
    // file. (Preferred media tracks are based on the file's language
    // preference.) This function returns NULL when no more media tracks exist.
    ...
    // Matroska file. (You should not call this function more than once with the
    // same track number.)
...
private:
    ...
    void continueReading();  // called by a demuxed track to tell us that it has
                             // a pending read ("doGetNextFrame()")
