...
// "ProxyServerMediaSession" object being used. It is used only within the
// implementation of "ProxyServerMediaSession", but is defined here, in case
// developers wish to subclass it.

class ProxyRTSPClient : public RTSPClient {
public:
    ProxyRTSPClient(class ProxyServerMediaSession& ourServerMediaSession,
    ...
    void continueAfterLivenessCommand(int resultCode,
...
typedef ProxyRTSPClient* createNewProxyRTSPClientFunc(
    ProxyServerMediaSession& ourServerMediaSession, char const* rtspURL,
...
public:
    static ProxyServerMediaSession* createNew(
        ...
        GenericMediaServer*
            ourMediaServer,          // Note: We can be used by just one server
        char const* inputStreamURL,  // the "rtsp://" URL of the stream we'll be
                                     // proxying
        char const* streamName = NULL, char const* username = NULL,
        char const* password = NULL, portNumBits tunnelOverHTTPPortNum = 0,
    ...
    // RTP/RTCP-over-TCP, but *not* using HTTP "verbosityLevel" == 1 means
    // display basic proxy setup info; "verbosityLevel" == 2 means display RTSP
    // client protocol also. If "socketNumToServer" is >= 0, then it is the
    // socket number of an already-existing TCP connection to the server.
    ...
    // This can be used - along with "describeCompletedFlag" - to check whether
    // the back-end "DESCRIBE" completed *successfully*.

protected:
    ProxyServerMediaSession(
        UsageEnvironment& env, GenericMediaServer* ourMediaServer,
    ...
    // own function
    // - with signature "createNewProxyRTSPClientFunc" (see above) - that
    // creates a new object of this subclass.  You should also subclass
    // "ProxyServerMediaSession" and, in your subclass's constructor, initialize
    // the parent class (i.e., "ProxyServerMediaSession") constructor by passing
    // your new function as the "ourCreateNewProxyRTSPClientFunc" parameter.

    // Subclasses may redefine the following functions, if they want
    // "ProxyServerSubsession"s to create subclassed "Groupsock" and/or
    ...
    // By default, this function always returns True.  However, a subclass may
    // redefine this if it wishes to restrict which subsessions of a stream get
    // proxied - e.g., if it wishes to proxy only video tracks, but not audio
    // (or other) tracks.
...
private:
    ...
    void continueAfterDESCRIBE(char const* sdpDescription);
    void
    resetDESCRIBEState();  // undoes what was done by "contineAfterDESCRIBE()"
...
// presentation times into wall-clock-aligned presentation times that are
// suitable for our "RTPSink"s (for the corresponding outgoing streams). (For
// multi-subsession (i.e., audio+video) sessions, the outgoing streams'
...
private:
    ...
    PresentationTimeSubsessionNormalizer(
        PresentationTimeSessionNormalizer& parent, FramedSource* inputSource,
...
private:  // called only from within "~PresentationTimeSubsessionNormalizer":
    ...
    void normalizePresentationTime(
...
private:
    ...
    struct timeval
        fPTAdjustment;  // Added to (RTCP-synced) subsession presentation times
                        // to 'normalize' them with wall-clock time.
};
