...
public:
    ...
    // Specifies that PAT and PMT packets should be output every
    // "segmentationDuration" seconds. (If "segmentationDuration" is 0 (the
    // default value), then PAT and PMT packets are output at a preset
    // frequency.) The optional function "onEndOfSegmentFunc" is called after
    // each segment is output.
    double currentSegmentDuration() const { return fCurrentSegmentDuration; }
    // Valid only if "setTimedSegmentation()" was previously called with
    // "segmentationDuration" > 0

    Boolean canDeliverNewFrameImmediately() const {
        return fInputBufferBytesUsed < fInputBufferSize;
    }
    // Can be used by a downstream reader to test whether the next call to
    // "doGetNextFrame()" will deliver data immediately).
...
protected:
    ...
    // If "PID" is not -1, then it (currently, only the low 8 bits) is used as
    // the stream's PID, otherwise the "stream_id" in the PES header is reused
    // to be the stream's PID.
...
private:
    void deliverDataToClient(u_int16_t pid, unsigned char* buffer,
                             unsigned bufferSize,
...
private:
    ...
    // if nonzero, this is the number of seconds between successive 'segments'.
    // Each 'segment' begins with a PAT, followed by a PMT. if zero (the default
    // value), then the frequency of PATs and PMTs depends on the constants
    // PAT_PERIOD_IF_UNTIMED and PMT_PERIOD_IF_UNTIMED, defined in the .cpp
    // file.
    ...
    // 2 if a segment has just ended and the following PAT has been sent; a PMT
    // is next 0 otherwise
    double fCurrentSegmentDuration,
        fPreviousPTS;  // used only if fSegmentationDuration > 0
    onEndOfSegmentFunc*
        fOnEndOfSegmentFunc;          // used only if fSegmentationDuration > 0
    void* fOnEndOfSegmentClientData;  // ditto
};

// The CRC calculation function that Transport Streams use.  We make this
// function public here in case it's useful elsewhere:
