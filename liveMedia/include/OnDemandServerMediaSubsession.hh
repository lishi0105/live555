...
protected:  // redefined virtual functions
    ...
    virtual void getStreamParameters(
        ...
        TLSState* tlsState, struct sockaddr_storage& destinationAddress,
        u_int8_t& destinationTTL, Boolean& isMulticast, Port& serverRTPPort,
        Port& serverRTCPPort, void*& streamToken);
...
protected:  // new virtual functions, possibly redefined by subclasses
    ...
    // "seekNPT".  (If <=0.0, all remaining data is streamed.) "numBytes"
    // returns the size (in bytes) of the data to be streamed, or 0 if unknown
    // or unlimited.
    virtual void seekStreamSource(FramedSource* inputSource, char*& absStart,
                                  char*& absEnd);
    // This routine is used to seek by 'absolute' time.
    // "absStart" should be a string of the form "YYYYMMDDTHHMMSSZ" or
    // "YYYYMMDDTHHMMSS.<frac>Z". "absEnd" should be either NULL (for no end
    // time), or a string of the same form as "absStart". These strings may be
    // modified in-place, or can be reassigned to a newly-allocated value (after
    // delete[]ing the original).
    virtual void setStreamSourceScale(FramedSource* inputSource, float scale);
    virtual void setStreamSourceDuration(FramedSource* inputSource,
...
public:
    ...
    // An alternative to passing the "multiplexRTCPWithRTP" parameter as True in
    // the constructor
    ...
    // future client. (Any current clients are not affected; any "APP" packets
    // from them will continue to be handled by whatever handler existed when
    // the client sent its first RTSP "PLAY" command.) (Call with (NULL, NULL)
    // to remove an existing handler - for future clients only)
    ...
    // "reuseFirstSource" was False), or to all current clients (if
    // "reuseFirstSource" was True). The parameters correspond to their
    // respective fields as described in the RTP/RTCP definition (RFC 3550).
    // Note that only the low-order 5 bits of "subtype" are used, and only the
    // first 4 bytes of "name" are used.  (If "name" has fewer than 4 bytes, or
    // is NULL, then the remaining bytes are '\0'.)
...
// internally, in the implementation of "OnDemandServerMediaSubsession", but we
// expose the definition here, in case subclasses of
// "OnDemandServerMediaSubsession" want to access it.

class Destinations {
public:
    Destinations(struct sockaddr_storage const& destAddr,
                 Port const& rtpDestPort, Port const& rtcpDestPort)
        : isTCP(False),
          addr(destAddr),
          rtpPort(rtpDestPort),
          rtcpPort(rtcpDestPort) {}
    Destinations(int tcpSockNum, unsigned char rtpChanId,
                 unsigned char rtcpChanId, TLSState* tlsSt)
        : isTCP(True),
          rtpPort(0) /*dummy*/,
          rtcpPort(0) /*dummy*/,
