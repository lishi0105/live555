...
/* NOTE: To support receiving your own custom RTP payload format, you must first
   define a new subclass of "MultiFramedRTPSource" (or "BasicUDPSource") that
   implements it. Then define your own subclass of "MediaSession" and
   "MediaSubsession", as follows:
   ...
   sdpDescription); and call this - instead of "MediaSession::createNew()" - in
   your application, when you create a new "MediaSession" object.
       - Reimplement the "createNewMediaSubsession()" virtual function, as
   follows: MediaSubsession* myMediaSession::createNewMediaSubsession() { return
   ...
   similar to this: Boolean myMediaSubsession::createSourceObjects(int
   useSpecialRTPoffset) { if (strcmp(fCodecName, "X-MY-RTP-PAYLOAD-FORMAT") ==
   0) {
               // This subsession uses our custom RTP payload format:
               fReadSource = fRTPSource =
   myRTPPayloadFormatRTPSource::createNew( <parameters> ); return True; } else {
               // This subsession uses some other RTP payload format - perhaps
   one that we already implement: return
...
public:
    ...
    int connectionEndpointNameAddressFamily() const {
        return fConnectionEndpointNameAddressFamily;
    }
...
public:
    ...
    // Changes "readSource()" to "filter" (which must have just been created
    // with "readSource()" as its input)
    ...
    }  // optional hack for audio/MPA-ROBUST; must not be called after
       // initiate()
    ...
    // returns "" if attribute doesn't exist (and has no default value), or is
    // not a string
    ...
    struct {
        u_int16_t seqNum;
        u_int32_t timestamp;
        Boolean infoIsNew;  // not part of the RTSP header; instead, set
                            // whenever this struct is filled in
    } rtpInfo;

    double getNormalPlayTime(struct timeval const& presentationTime);
    // Computes the stream's "Normal Play Time" (NPT) from the given
    // "presentationTime". (For the definition of "Normal Play Time", see RFC
    // 2326, section 3.6.) This function is useful only if the "rtpInfo"
    // structure was previously filled in (e.g., by a "RTP-Info:" header in a
    // RTSP response). Also, for this function to work properly, the RTP
    // stream's presentation times must (eventually) be synchronized via RTCP.
...
protected:
    ...
    float fScale;  // set from a RTSP "Scale:" header
    float fSpeed;
    double fNPT_PTS_Offset;  // set by "getNormalPlayTime()"; add this to a PTS
                             // to get NPT
    HashTable* fAttributeTable;  // for "a=fmtp:" attributes.  (Later an array
                                 // by payload type #####)
