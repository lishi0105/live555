...
**********/
// "liveMedia"
// Copyright (c) 1996-2024 Live Networks, Inc.  All rights reserved.// A filter
...
// I-frames) on the video stream. C++ header
...
public:
    static MPEG2TransportStreamTrickModeFilter* createNew(
        UsageEnvironment& env, FramedSource* inputSource,
        MPEG2TransportStreamIndexFile* indexFile, int scale);

    Boolean seekTo(unsigned long tsPacketNumber,
...
private:
    ...
    unsigned long
        fNextTSPacketNum;  // next to be read from the transport stream file
    unsigned char fInputBuffer[TRANSPORT_PACKET_SIZE];
    unsigned long
        fCurrentTSPacketNum;  // corresponding to data currently in the buffer
