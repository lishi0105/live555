...
public:
    static T140TextRTPSink* createNew(UsageEnvironment& env, Groupsock* RTPgs,
                                      unsigned char rtpPayloadFormat);

protected:
    T140TextRTPSink(UsageEnvironment& env, Groupsock* RTPgs,
                    unsigned char rtpPayloadFormat);
...
// (empty) RTP packets be sent during 'idle periods' when no new text is
// available, we implement "T140TextRTPSink" using a separate "T140IdleFilter"
// class - sitting in front
// -  that delivers, to the "T140TextRTPSink", a continuous sequence of
// (possibly) empty frames. (Note: This class should be used only by
// "T140TextRTPSink", or a subclass.)
