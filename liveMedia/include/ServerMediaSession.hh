...
public:
    ...
    char* generateSDPDescription(
        int addressFamily);  // based on the entire session
    ...
    void testScaleFactor(
        float& scale);  // sets "scale" to the actual supported scale
    float duration() const;
    // a result == 0 means an unbounded session (the default)
    // a result < 0 means: subsession durations differ; the result is -(the
    // largest). a result > 0 means: this is the duration of a bounded session

    virtual void noteLiveness();
    // called whenever a client - accessing this media - notes liveness.
    // The default implementation does nothing, but subclasses can redefine this
    // - e.g., if you want to remove long-unused "ServerMediaSession"s from the
    // server.
    ...
    // us to an 'empty' state Note: If you have already added this
    // "ServerMediaSession" to a server then, before calling this function,
...
public:
    ...
    virtual void getRTPSinkandRTCP(void* streamToken, RTPSink*& rtpSink,
                                   RTCPInstance*& rtcp) = 0;
    // Returns pointers to the "RTPSink" and "RTCPInstance" objects for
    // "streamToken". (This can be useful if you want to get the associated
    // 'Groupsock' objects, for example.) You must not delete these objects, or
    // start/stop playing them; instead, that is done using the "startStream()"
    // and "deleteStream()" functions.
    virtual void deleteStream(unsigned clientSessionId, void*& streamToken);

    virtual void testScaleFactor(
        float& scale);  // sets "scale" to the actual supported scale
