...
RTCPInstance::RTCPInstance(UsageEnvironment& env, Groupsock* RTCPgs,
                           unsigned totSessionBW, unsigned char const* cname,
    ...
    fOutBuf =
        new OutPacketBuffer(preferredRTCPPacketSize, maxRTCPPacketSize, 1500);
...
void RTCPInstance::noteArrivingRR(
    ...
    if (fSpecificRRHandlerTable != NULL) {
        ...
        if (tcpSocketNum < 0) {
            ...
            // Hack: Use the TCP socket number and channel id to look up the
            // handler
...
void RTCPInstance::setupForSRTCP() {
    if (fCrypto == NULL &&
        fSink != NULL) {  // take crypto state (if any) from the sink instead:
...
void RTCPInstance::setByeWithReasonHandler(
    ByeWithReasonHandlerFunc* handlerTask, void* clientData,
    Boolean handleActiveParticipantsOnly) {
    fByeHandlerTask = NULL;
...
void RTCPInstance ::setSpecificRRHandler(
    struct sockaddr_storage const& fromAddress, Port fromPort,
    ...
    RRHandlerRecord* existingRecord =
        (RRHandlerRecord*)fSpecificRRHandlerTable->Add(fromAddress, fromPort,
                                                       rrHandler);
...
void RTCPInstance ::injectReport(u_int8_t const* packet, unsigned packetSize,
...
void RTCPInstance::incomingReportHandler1() {
    do {
        if (fNumBytesAlreadyRead >= maxRTCPPacketSize) {
            envir()
                << "RTCPInstance error: Hit limit when reading incoming packet "
                   "over TCP. (fNumBytesAlreadyRead ("
        ...
        Boolean readResult = fRTCPInterface.handleRead(
            ...
            maxRTCPPacketSize - fNumBytesAlreadyRead, numBytesRead, fromAddress,
        ...
        if (fIsSSMTransmitter && !packetWasFromOurHost) {
            ...
            // we're a SSM transmitter, and SSM receivers send back RTCP "RR"
...
void RTCPInstance ::processIncomingReport(
    ...
    do {
        if (fCrypto != NULL) {  // The packet is assumed to be SRTCP.
                                // Verify/decrypt it first:
            unsigned newPacketSize;
            if (!fCrypto->processIncomingSRTCPPacket(fInBuf, packetSize,
                                                     newPacketSize))
                break;
                ...
                case RTCP_PT_BYE: {
                    ...
                    if ((fByeHandlerTask != NULL ||
                        ...
                        (!fByeHandleActiveParticipantsOnly ||
                         (fSource != NULL && fSource->receptionStatsDB().lookup(
                                                 reportSenderSSRC) != NULL) ||
                         (fSink != NULL && fSink->transmissionStatsDB().lookup(
                                               reportSenderSSRC) != NULL))) {
                        callByeHandler = True;
                    }
		      ...
		      : "???"/* don't try to print out PRIV or unknown items */);
                ...
                case RTCP_PT_PSFB: {
                    ...
                    if (length >= 12 && pkt[4] == 'R' && pkt[5] == 'E' &&
                        ...
                        fprintf(
                            stderr,
                            "\tReceiver Estimated Max Bitrate (REMB): %g bps\n",
                            remb);
                    }
#endif
                    subPacketOK = True;
        ...
        if (callByeHandler) {
            if (fByeHandlerTask !=
                ...
                fByeHandlerTask =
                    NULL;  // because we call the handler only once, by default
...
void RTCPInstance::sendBuiltPacket() {
    ...
    if (fCrypto != NULL) {  // Encrypt/tag the data before sending it:
        ...
        if (!fCrypto->processOutgoingSRTCPPacket(fOutBuf->packet(), reportSize,
                                                 newReportSize))
            return;
