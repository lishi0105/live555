...
public:
    ...
    void deliverIncomingFrame(unsigned frameSize, unsigned char interleaveL,
...
void QCELPDeinterleaver::doGetNextFrame() {
    ...
    if (fDeinterleavingBuffer->retrieveFrame(
            fTo, fMaxSize, fFrameSize, fNumTruncatedBytes, fPresentationTime)) {
        // Success!
        fNeedAFrame = False;
...
void QCELPDeinterleaver ::afterGettingFrame1(unsigned frameSize,
    ...
    fDeinterleavingBuffer->deliverIncomingFrame(
        ...
        source->frameIndex(), source->curPacketRTPSeqNum(), presentationTime);
...
void QCELPDeinterleavingBuffer ::deliverIncomingFrame(
    unsigned frameSize, unsigned char interleaveL, unsigned char interleaveN,
