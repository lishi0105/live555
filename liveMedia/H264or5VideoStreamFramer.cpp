...
private:
    ...
    void removeEmulationBytes(u_int8_t* nalUnitCopy, unsigned maxSize,
    ...
    void analyze_vui_parameters(BitVector& bv, unsigned& num_units_in_tick,
                                unsigned& time_scale);
    void analyze_hrd_parameters(BitVector& bv);
    void analyze_sei_data(u_int8_t nal_unit_type);
    void analyze_sei_payload(unsigned payloadType, unsigned payloadSize,
                             u_int8_t* payload);
...
void H264or5VideoStreamFramer::setPresentationTime() {
    if (fPresentationTimeBase.tv_sec == 0 &&
...
H264or5VideoStreamParser ::H264or5VideoStreamParser(
    int hNumber, H264or5VideoStreamFramer* usingSource,
...
Boolean H264or5VideoStreamParser::usuallyBeginsAccessUnit(
    ...
    return fHNumber == 264 ? (nal_unit_type >= 6 && nal_unit_type <= 9) ||
...
void H264or5VideoStreamParser ::analyze_seq_parameter_set_data(
    ...
    if (fHNumber == 264) {
                ...
                for (unsigned sizeId = 0; sizeId < 4; ++sizeId) {
                    ...
                    for (unsigned matrixId = 0;
                        ...
                        if (!scaling_list_pred_mode_flag) {
                            ...
                            for (i = 0; i < coefNum; ++i) {
                                (void)bv
                                    .get_expGolomb();  // scaling_list_delta_coef
                            }
                        }
                    }
                }
        ...
        if (pcm_enabled_flag) {
            ...
            (void)bv
                .get_expGolomb();  // log2_min_pcm_luma_coding_block_size_minus3
            (void)bv
                .get_expGolomb();  // log2_diff_max_min_pcm_luma_coding_block_size
    ...
    while (j < seiSize) {
        ...
        if (fHNumber == 264) {
            ...
            description =
                ...
                : (payloadType == 132 && nal_unit_type == SUFFIX_SEI_NUT)
    ...
    try {
        ...
        if (haveSeenEOF() || isEOF(nal_unit_type)) {
            ...
            u_int8_t const& next_nal_unit_type =
                fHNumber == 264 ? (firstBytesOfNextNALUnit[0] & 0x1F)
