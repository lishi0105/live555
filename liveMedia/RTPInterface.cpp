...
public:
    tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId,
                    TLSState* tlsState, tcpStreamRecord* next);
...
static SocketDescriptor* lookupSocketDescriptor(
    UsageEnvironment& env, int sockNum, TLSState* tlsState = NULL,
...
void RTPInterface::addStreamSocket(int sockNum, unsigned char streamChannelId,
    ...
    fTCPStreams =
        new tcpStreamRecord(sockNum, streamChannelId, tlsState, fTCPStreams);

    // Also, make sure this new socket is set up for receiving
    // RTP/RTCP-over-TCP:
    SocketDescriptor* socketDescriptor =
        lookupSocketDescriptor(envir(), sockNum, tlsState);
...
static void deregisterSocket(UsageEnvironment& env, int sockNum,
    ...
    SocketDescriptor* socketDescriptor =
        lookupSocketDescriptor(env, sockNum, NULL, False);
    if (socketDescriptor != NULL) {
...
void RTPInterface::setServerRequestAlternativeByteHandler(
    ...
    ServerRequestAlternativeByteHandler* handler, void* clientData) {
    SocketDescriptor* socketDescriptor =
        lookupSocketDescriptor(env, socketNum, NULL, False);
...
Boolean RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize) {
    ...
    for (tcpStreamRecord* stream = fTCPStreams; stream != NULL;
        ...
        if (!sendRTPorRTCPPacketOverTCP(
                packet, packetSize, stream->fStreamSocketNum,
                stream->fStreamChannelId, stream->fTLSState)) {
            success = False;
        }
    }
...
Boolean RTPInterface::handleRead(unsigned char* buffer, unsigned bufferMaxSize,
    ...
    if (fNextTCPReadStreamSocketNum < 0) {
        ...
        int curBytesRead;
        // Because we're calling "readSocket()" on a stream socket, we don't
        // expect "fromAddress" to be filled in, so set it to a 'dummy' value
        // instead:
        ...
        while ((curBytesRead =
                    (fNextTCPReadTLSState != NULL &&
                     fNextTCPReadTLSState->isNeeded)
                        ? fNextTCPReadTLSState->read(&buffer[bytesRead],
                                                     curBytesToRead)
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, TLSState* tlsState,
    ...
    int sendResult = (tlsState != NULL && tlsState->isNeeded)
                         ...
                         : send(socketNum, (char const*)data, dataSize,
                                MSG_NOSIGNAL /*flags*/);
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
    ...
    switch (fTCPReadingState) {
        ...
        case AWAITING_PACKET_DATA: {
            callAgain = False;
            fTCPReadingState =
                AWAITING_DOLLAR;  // the next state, unless we end up having to
                                  // read more data in the current state
...
#endif
                    int result =
                        (fTLSState != NULL && fTLSState->isNeeded)
                            ? fTLSState->read(&c, 1)
                            : readSocket(fEnv, fOurSocketNum, &c, 1, dummy);
