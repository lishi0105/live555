...
VorbisAudioRTPSink ::VorbisAudioRTPSink(
    ...
    if (identificationHeaderSize >= 28) {
        ...
        int bitrate = bitrate_nominal > 0   ? bitrate_nominal
                      ...
                      : bitrate_minimum > 0 ? bitrate_minimum
                                            : 0;
...
char* generateVorbisOrTheoraConfigStr(
    ...
    unsigned
        sizeSize[2];  // The number of bytes used to encode the lengths of the
                      // first two headers (but not the length of the 3rd)
    sizeSize[0] = sizeSize[1] = 0;
    if (identificationHeaderSize > 0) {
        sizeSize[numHeaders++] = identificationHeaderSize < 128     ? 1
                                 : identificationHeaderSize < 16384 ? 2
                                                                    : 3;
    }
    if (commentHeaderSize > 0) {
        sizeSize[numHeaders++] = commentHeaderSize < 128     ? 1
                                 : commentHeaderSize < 16384 ? 2
                                                             : 3;
    }
    ...
    *p++ = numHeaders - 1;  // "n. of headers"
    if (numHeaders > 1) {
        ...
        if (length1 >= 128) {
            *p++ =
                0x80 | ((length1 & 0x3F80) >> 7);  // flag + the second 7 bits
        }
