...
private:
    // Parsing functions:
    Boolean parse();
    // returns True iff we have finished parsing to the end of all 'Track'
    // headers (on initialization)
    ...
    void getCommonFrameBytes(MatroskaTrack* track, u_int8_t* to,
                             unsigned numBytesToGet, unsigned numBytesToSkip);
    ...
    // Note: "result" is dynamically allocated; the caller must delete[] it
    // later
    ...
    // called after a seek or pause to ensure that presentation times continue
    // to be aligned with 'wall clock' time
