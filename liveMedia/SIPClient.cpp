...
SIPClient::SIPClient(UsageEnvironment& env,
    ...
    Port srcPort(0);
    getSourcePort(env, fOurSocket->socketNum(), AF_INET,
                  srcPort);  // later, allow for IPv6
    if (srcPort.num() != 0) {
...
char* SIPClient::invite1(Authenticator* authenticator) {
    do {
        ...
        unsigned inviteCmdSize =
            strlen(cmdFmt) + fURLSize + 2 * fUserNameSize + fOurAddressStrSize +
            20                       /* max int len */
            ...
            if (contentLength > (int)numBodyBytes) {
                ...
                while (numExtraBytesNeeded > 0) {
                    ...
                    Boolean readSuccess = fOurSocket->handleRead(
                        (unsigned char*)ptr, numExtraBytesNeeded, bytesRead2,
                        dummy);
...
Boolean SIPClient::parseSIPURL(UsageEnvironment& env, char const* url,
    ...
    do {
        // Parse the URL as
        // "sip:<username>@<server-name-or-address>:<port>/<etc>" (with
        // ":<port>" and "/<etc>" optional) Also, skip over any
        // "<username>[:<password>]@" preceding <address>
...
char* SIPClient::createAuthenticatorString(Authenticator const* authenticator,
    ...
    if (authenticator != NULL && authenticator->realm() != NULL &&
        ...
        unsigned authBufSize =
            ...
            strlen(authenticator->realm()) + strlen(authenticator->nonce()) +
...
unsigned SIPClient::getResponse(char*& responseBuffer,
    ...
    while (bytesRead < (int)responseBufferSize) {
        ...
        Boolean readSuccess = fOurSocket->handleRead(
            toPosn, responseBufferSize - bytesRead, bytesReadNow, dummy);
