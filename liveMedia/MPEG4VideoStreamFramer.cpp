...
unsigned MPEG4VideoStreamParser::parseVisualObject() {
    ...
    if (visual_object_type != 1) {
        usingSource()->envir()
            << "MPEG4VideoStreamParser::parseVisualObject(): Warning: We don't "
               "handle visual_object_type "
            << visual_object_type << "\n";
    }
...
void MPEG4VideoStreamParser::analyzeVOLHeader() {
    ...
    do {
        ...
        if (marker_bit != 1) {  // sanity check
            usingSource()->envir()
                << "MPEG4VideoStreamParser::analyzeVOLHeader(): marker_bit 1 "
                   "not set!\n";
            break;
        }
        ...
        if (marker_bit != 1) {  // sanity check
            usingSource()->envir()
                << "MPEG4VideoStreamParser::analyzeVOLHeader(): marker_bit 2 "
                   "not set!\n";
            break;
        }
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
    ...
    if (!isVideoObjectLayerStartCode(next4Bytes)) {
        usingSource()->envir()
            << "MPEG4VideoStreamParser::parseVideoObjectLayer(): This appears "
               "to be a 'short video header', which we currently don't "
               "support\n";
    }
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
    ...
    fprintf(
        stderr,
        "time_code: 0x%05x, hours %d, minutes %d, marker_bit %d, seconds %d\n",
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
    ...
    if ((mask >> (fNumVTIRBits - 1)) == 0) {
        usingSource()->envir()
            << "MPEG4VideoStreamParser::parseVideoObjectPlane(): 32-bits are "
               "not enough to get \"vop_time_increment\"!\n";
    }
    ...
    if (fixed_vop_time_increment > 0) {
        ...
        if (newTotalTicks == fPrevNewTotalTicks && fPrevNewTotalTicks > 0) {
            ...
            if (newTotalTicks < fPrevNewTotalTicks && vop_coding_type != 2 /*B*/
                ...
                fprintf(
                    stderr,
                    "Buggy MPEG-4 video stream: \"vop_time_increment\" wrapped "
