...
void MatroskaFileParser::pause() {
    ...
    // to ensure that we presentation times continue from 'wall clock' time
    // after we resume
}
        ...
        switch (id.val()) {
            ...
            case MATROSKA_ID_TRACK_TYPE: {
                ...
                if (parseEBMLVal_unsigned(size, trackType) && track != NULL) {
                    ...
                    track->trackType =
                        trackType == 1      ? MATROSKA_TRACK_TYPE_VIDEO
...
void MatroskaFileParser::parseBlock() {
    ...
    do {
        ...
        MatroskaLacingType lacingType = (c == 0x0)    ? NoLacing
                                        : (c == 0x02) ? XiphLacing
        ...
        if (track->defaultDuration == 0) {
            ...
            if (demuxedTrack->durationImbalance() > 0) {
                ...
                int const adjustmentThreshold =
                    100000;  // don't increase the duration by more than this
                             // amount (in case there's a mistake)
                adjustment =
...
void MatroskaFileParser::resetStateAfterSeeking() {
    // Presentation times aren't affected by the seek; they continue advancing
    // as normal. Therefore, ensure that they continue to be aligned with 'wall
    // clock' time:
    resetPresentationTimes();
