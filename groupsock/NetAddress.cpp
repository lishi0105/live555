...
void copyAddress(struct sockaddr_storage& to, NetAddress const* from) {
    ...
    if (from->length() == sizeof(ipv4AddressBits)) {
        ...
        ((sockaddr_in6&)to).sin6_scope_id =
            0;  // instead, somehow look this up from the address?
    }
}
...
NetAddressList::NetAddressList(char const* hostname, int addressFamily)
    ...
    // First, check whether "hostname" is an IP address literal (check IPv4,
    // then IPv6). If so, return a 1-element list with this address:
...
#define NUM_RECORDS_IN_KEY_FOR_EACH_ADDRESS \
    (sizeof(struct in6_addr) / sizeof(int))
...
static void setKeyFromAddress(int*& key,
                              struct sockaddr_storage const& address) {
    if (address.ss_family == AF_INET) {
        // For an IPv4 address, set the upper 3 words of the key to zero, with
        // the address last:
        *key++ = 0;
        *key++ = 0;
        *key++ = 0;
        *key++ = ((sockaddr_in const&)address).sin_addr.s_addr;
        ...
        // Assume that the address is IPv6; use all 128 bits (4 words) for the
        // key:
...
AddressString::AddressString(ipv4AddressBits const& addr) { init(addr); }
...
AddressString::AddressString(ipv6AddressBits const& addr) { init(addr); }
...
void setPortNum(struct sockaddr_storage& address,
                portNumBits portNum /*in network order*/) {
    ((sockaddr_in&)address).sin_port =
        portNum;  // position will be the same for "sockaddr_in6"
}
