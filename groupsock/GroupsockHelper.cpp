...
static int createSocket(int domain, int type) {
    ...
    int const domain2 = domain == AF_INET    ? PF_INET
                        : domain == AF_INET6 ? PF_INET6
                                             : domain;
...
int setupDatagramSocket(UsageEnvironment& env, Port port, int domain) {
    ...
    if (setsockopt(newSocket, domain == AF_INET ? IPPROTO_IP : IPPROTO_IPV6,
                   domain == AF_INET ? IP_MULTICAST_LOOP : IPV6_MULTICAST_LOOP,
                   (const char*)&loop, sizeof loop) < 0) {
        if (domain ==
            AF_INET) {  // For some unknown reason, this might not work for IPv6
    ...
    if (domain == AF_INET) {
        ...
        if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
            char tmpBuffer[100];
            sprintf(tmpBuffer,
                    "IPv4 bind() error (port number: %d): ", ntohs(port.num()));
        ...
        if (port.num() != 0) {
            // For IPv6 sockets, we need the IPV6_V6ONLY flag set to 1,
            // otherwise we would not be able to have an IPv4 socket and an IPv6
            // socket bound to the same port:
            int const one = 1;
            (void)setsockopt(newSocket, IPPROTO_IPV6, IPV6_V6ONLY,
                             (const char*)&one, sizeof one);

            MAKE_SOCKADDR_IN6(name, addr, port.num());
            if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
                char tmpBuffer[100];
                sprintf(tmpBuffer, "IPv6 bind() error (port number: %d): ",
                        ntohs(port.num()));
                socketErr(env, tmpBuffer);
                closeSocket(newSocket);
                return -1;
...
int readSocket(UsageEnvironment& env, int socket, unsigned char* buffer,
               unsigned bufferSize, struct sockaddr_storage& fromAddress) {
...
Boolean writeSocket(UsageEnvironment& env, int socket,
                    ...
                    u_int8_t ttlArg, unsigned char* buffer,
                    unsigned bufferSize) {
...
Boolean writeSocket(UsageEnvironment& env, int socket,
    ...
    do {
        ...
        int bytesSent =
            sendto(socket, (char*)buffer, bufferSize, MSG_NOSIGNAL,
...
Boolean socketJoinGroup(UsageEnvironment& env, int socket,
    ...
    switch (groupAddress.ss_family) {
        case AF_INET: {
            imr4.imr_multiaddr.s_addr =
                ((struct sockaddr_in&)groupAddress).sin_addr.s_addr;
        ...
        case AF_INET6: {
            imr6.ipv6mr_multiaddr =
                ((struct sockaddr_in6&)groupAddress).sin6_addr;
    ...
    if (setsockopt(socket, level, option_name, (const char*)option_value,
                   option_len) < 0) {
#if defined(__WIN32__) || defined(_WIN32)
        if (env.getErrno() != 0) {
...
Boolean socketJoinGroupSSM(UsageEnvironment& env, int socket,
    ...
    imr.imr_sourceaddr =
        ((struct sockaddr_in&)sourceFilterAddr).sin_addr.s_addr;
    imr.imr_interface = ReceivingInterfaceAddr;
#else
    imr.imr_multiaddr.s_addr =
        ((struct sockaddr_in&)groupAddress).sin_addr.s_addr;
    imr.imr_sourceaddr.s_addr =
        ((struct sockaddr_in&)sourceFilterAddr).sin_addr.s_addr;
...
Boolean getSourcePort(UsageEnvironment& env, int socket, int domain,
                      Port& port) {
...
static Boolean isBadIPv6AddressForUs(ipv6AddressBits addr) {
    ...
    //   - the first 10 bits are 0xFE8, indicating a link-local or site-local
    //   address, or
    //   - the first 15 bytes are 0, and the 16th byte is 0 (unspecified) or 1
    //   (loopback)
...
static void copyAddress(struct sockaddr_storage& to,
                        struct sockaddr const* from) {
    // Copy a "struct sockaddr" to a "struct sockaddr_storage" (assumed to be
    // large enough)
    if (from == NULL) return;
    ...
    switch (from->sa_family) {
        case AF_INET: {
            ...
            ((sockaddr_in&)to).sin_addr.s_addr =
                ((sockaddr_in const*)from)->sin_addr.s_addr;
            break;
        }
        case AF_INET6: {
#ifdef HAVE_SOCKADDR_LEN
            ...
            for (unsigned i = 0; i < 16; ++i) {
                ((sockaddr_in6&)to).sin6_addr.s6_addr[i] =
                    ((sockaddr_in6 const*)from)->sin6_addr.s6_addr[i];
            }
            break;
        }
    }
...
void getOurIPAddresses(UsageEnvironment& env) {
    ...
    // (or if "getifaddrs()" is not defined), then we use an alternative (more
    // old-fashioned) mechanism: First get our host name, then try resolving
    // this host name.
    ...
    if (getifaddrs(&ifap) == 0) {
        ...
        for (struct ifaddrs* p = ifap; p != NULL; p = p->ifa_next) {
            ...
            // Also ignore the interface if the address is considered 'bad' for
            // us:
    ...
    if (!getifaddrsWorks) do {
            ...
            while ((address = iter.nextAddress()) != NULL) {
                ...
                if (address->length() == sizeof(ipv4AddressBits) &&
                    addressIsNull(foundIPv4Address)) {
                    copyAddress(foundIPv4Address, address);
                } else if (address->length() == sizeof(ipv6AddressBits) &&
                           addressIsNull(foundIPv6Address)) {
    ...
    for (unsigned i = 0; i < 16; ++i) {
        _ourIPv6Address[i] =
            ((sockaddr_in6&)foundIPv6Address).sin6_addr.s6_addr[i];
        if (_ourIPv6Address[i] != 0) _weHaveAnIPv6Address = True;
    }

    if (!_weHaveAnIPv4Address && !_weHaveAnIPv6Address) {
        env.setResultMsg(
            "This computer does not have a valid IP (v4 or v6) address!");
    }
    ...
    for (unsigned i = 0; i < 16; i += 4) {
        seed ^= (_ourIPv6Address[i] << 24) | (_ourIPv6Address[i + 1] << 16) |
                (_ourIPv6Address[i + 2] << 8) | _ourIPv6Address[i + 3];
    }
    our_srandom(seed);
}
