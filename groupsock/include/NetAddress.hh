...
// Note that the type "netAddressBits" is no longer defined; use
...
public:
    NetAddress(
        u_int8_t const* data,
        unsigned length =
            4 /* default: 32 bits (for IPv4); use 16 (128 bits) for IPv6 */);
...
// compares the family and address parts only; not the port number or anything
// else
...
public:
    ...
    AddressString(ipv4AddressBits const&
                      addr);  // "addr" is assumed to be in network byte order
...
private:
    void init(ipv4AddressBits const&
                  addr);  // used to implement the IPv4 constructors
    void init(ipv6AddressBits const&
                  addr);  // used to implement the IPv6 constructors

private:
    char* fVal;  // The result ASCII string: allocated by the constructor;
