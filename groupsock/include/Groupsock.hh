...
public:
    ...
    virtual destRecord* createNewDestRecord(struct sockaddr_storage const& addr,
    ...
    // (If a parameter is 0 (or ~0 for ttl), then no change is made to that
    // parameter.) (If no existing "destRecord" exists with this "sessionId",
    // then we add a new "destRecord".)
    unsigned lookupSessionIdFromDestination(
        struct sockaddr_storage const& destAddrAndPort) const;
    // returns 0 if not found

    // As a special case, we also allow multiple destinations (addresses &
    // ports) (This can be used to implement multi-unicast.)
    ...
    virtual Boolean output(UsageEnvironment& env, unsigned char* buffer,
                           unsigned bufferSize);
...
private:
    ...
    // used to implement (the public) "removeDestination()", and
