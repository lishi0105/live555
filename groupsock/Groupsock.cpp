...
Boolean OutputSocket::write(struct sockaddr_storage const& addressAndPort,
                            u_int8_t ttl, unsigned char* buffer,
                            unsigned bufferSize) {
    if ((unsigned)ttl == fLastSentTTL) {
        // Optimization: Don't do a 'set TTL' system call again
        if (!writeSocket(env(), socketNum(), addressAndPort, buffer,
                         bufferSize))
            return False;
    } else {
        if (!writeSocket(env(), socketNum(), addressAndPort, ttl, buffer,
                         bufferSize))
            return False;
    ...
    if (sourcePortNum() == 0) {
        ...
        if (!getSourcePort(env(), socketNum(), addressAndPort.ss_family,
                           fSourcePort)) {
            if (DebugLevel >= 1)
                env() << *this
...
Boolean OutputSocket ::handleRead(
    unsigned char* /*buffer*/, unsigned /*bufferMaxSize*/,
...
Groupsock::Groupsock(UsageEnvironment& env,
                     struct sockaddr_storage const& groupAddr, Port port,
                     u_int8_t ttl)
...
destRecord* Groupsock ::createNewDestRecord(struct sockaddr_storage const& addr,
...
void Groupsock::changeDestinationParameters(
    struct sockaddr_storage const& newDestAddr, Port newDestPort,
    int newDestTTL, unsigned sessionId) {
    destRecord* dest;
...
Boolean Groupsock::output(UsageEnvironment& env, unsigned char* buffer,
    ...
    do {
        ...
        for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
            if (!write(dests->fGroupEId.groupAddress(), dests->fGroupEId.ttl(),
                       buffer, bufferSize)) {
                writeSuccess = False;
                break;
            }
...
Boolean Groupsock::wasLoopedBackFromUs(
    UsageEnvironment& env, struct sockaddr_storage const& fromAddressAndPort) {
    if (fromAddressAndPort.ss_family != AF_INET)
        return False;  // later update for IPv6

    struct sockaddr_in const& fromAddressAndPort4 =
...
UsageEnvironment& operator<<(UsageEnvironment& s, const Groupsock& g) {
    UsageEnvironment& s1 =
        s << timestampString() << " Groupsock(" << g.socketNum() << ": "
...
Groupsock* GroupsockLookupTable::Fetch(
    UsageEnvironment& env, struct sockaddr_storage const& groupAddress,
...
Groupsock* GroupsockLookupTable::Lookup(
...
Boolean GroupsockLookupTable::Remove(Groupsock const* groupsock) {
    ...
    return fTable.Remove(groupsock->groupAddress(),
                         groupsock->sourceFilterAddress(), groupsock->port());
}

Groupsock* GroupsockLookupTable::AddNew(
    UsageEnvironment& env, struct sockaddr_storage const& groupAddress,
    struct sockaddr_storage const& sourceFilterAddress, Port port,
    u_int8_t ttl) {
    Groupsock* groupsock;
    do {
        if (addressIsNull(sourceFilterAddress)) {
            ...
            // SSM groupsock
            groupsock =
                new Groupsock(env, groupAddress, sourceFilterAddress, port);
        }
