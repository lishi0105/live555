...
public:
    ...
    // like setResultMsg(), except that an 'errno' message is appended.  (If
    // "err == 0", the "getErrno()" code is used instead.)
...
#define SOCKET_EXCEPTION (1 << 3)
    ...
    // sequentially (as a single thread of control). (If "watchVariable" is not
    // NULL, then we return from this routine when *watchVariable != 0)
    ...
    // (from the event loop) using "eventHandlerProc". (Returns 0 iff no such
    // trigger can be created (e.g., because of implementation limits on the
    // number of triggers).)
    ...
    // event to be handled (from the event loop). The handler function is called
    // with "clientData" as parameter. Note: This function (unlike other library
    // functions) may be called from an external thread
    // - to signal an external event.
    // (In fact, this is the *only* LIVE555 function that can be called from a
    // non-LIVE555 thread.) (However, "triggerEvent()" should not be called with
    // the same 'event trigger id' from different threads.  Also, once
    // "triggerEvent()" is called with one 'event trigger id', it should not be
    // called again with the same 'event trigger id' until after its event has
    // been handled.)
