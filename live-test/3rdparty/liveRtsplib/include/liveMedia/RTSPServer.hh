...
typedef std::function<void(const char*, unsigned, Boolean)>
    clientConnectCallback;
class RTSPServer : public GenericMediaServer {
public:
    static RTSPServer* createNew(
        UsageEnvironment& env, Port ourPort = 554,
        UserAuthenticationDatabase* authDatabase = NULL,
        unsigned reclamationSeconds = 65, const char* suffix = NULL);
    ...
    typedef void(responseHandlerForREGISTER)(RTSPServer* rtspServer,
                                             unsigned requestId, int resultCode,
                                             char* resultString);
    ...
    // remote client (specifed by name and port number). This is done using our
    // custom "REGISTER" RTSP command. The function returns a unique number that
    // can be used to identify the request; this number is also passed to
    // "responseHandler". When a response is received from the remote client (or
    // the "REGISTER" request fails), the specified response handler
    //   (if non-NULL) is called.  (Note that the "resultString" passed to the
    //   handler was dynamically allocated, and should be delete[]d by the
    //   handler after use.)
    // If "receiveOurStreamViaTCP" is True, then we're requesting that the
    // remote client access our stream using RTP/RTCP-over-TCP.
    ...
    // proxy server.
    //   It tells the proxy server the suffix that it should use in its
    //   "rtsp://" URL (when front-end clients access the stream)

    typedef void(responseHandlerForDEREGISTER)(RTSPServer* rtspServer,
                                               unsigned requestId,
                                               int resultCode,
                                               char* resultString);
    unsigned deregisterStream(ServerMediaSession* serverMediaSession,
                              ...
                              char const* password = NULL,
                              char const* proxyURLSuffix = NULL);
    // Used to turn off a previous "registerStream()" - using our custom
    // "DEREGISTER" RTSP command.
    ...
    // Shortcuts:
    char* ipv4rtspURL(ServerMediaSession const* serverMediaSession,
                      int clientSocket = -1) {
        return rtspURL(serverMediaSession, clientSocket, False);
    }
    char* ipv6rtspURL(ServerMediaSession const* serverMediaSession,
                      int clientSocket = -1) {
        return rtspURL(serverMediaSession, clientSocket, True);
    }

    virtual char* rtspURLPrefix(int clientSocket = -1,
    ...
    char* ipv4rtspURLPrefix(int clientSocket = -1) {
        return rtspURLPrefix(clientSocket, False);
    }
    char* ipv6rtspURLPrefix(int clientSocket = -1) {
        return rtspURLPrefix(clientSocket, True);
    }
    ...
    // pointer to the old database (if there was one). "newDB" may be NULL (you
    // can use this to disable authentication at runtime, if desired).
    ...
    // Returns True iff the specified port can be used in this way (i.e., it's
    // not already being used for a separate HTTP server). Note: RTSP-over-HTTP
    // tunneling is described in
    ...
    portNumBits httpServerPortNum()
        const;  // in host byte order.  (Returns 0 if not present.)
    ...
    void setClientConnectCallback(clientConnectCallback cb) {
        fClientConnectCallback = cb;
    }

protected:
    RTSPServer(UsageEnvironment& env, int ourSocketIPv4, int ourSocketIPv6,
    ...
    virtual ~RTSPServer();
    virtual char const*
    allowedCommandNames();  // used to implement
    ...
    virtual UserAuthenticationDatabase* getAuthenticationDatabaseForCommand(
        char const* cmdName);
    virtual Boolean specialClientAccessCheck(
        int clientSocket, struct sockaddr_storage const& clientAddr,
        char const* urlSuffix);
    // a hook that allows subclassed servers to do server-specific access
    // checking on each client (e.g., based on client IP address), without using
    // digest authentication.
    virtual Boolean specialClientUserAccessCheck(
        int clientSocket, struct sockaddr_storage const& clientAddr,
        char const* urlSuffix, char const* username);
    // another hook that allows subclassed servers to do server-specific access
    // checking
    // - this time after normal digest authentication has already taken place
    // (and would otherwise allow access). (This test can only be used to
    // further restrict access, not to grant additional access.)
    virtual void specialHandlingOfAuthenticationFailure(
        int clientSocket, struct sockaddr_storage const& clientAddr,
        char const* urlSuffix);
    // a hook that allows subclassed servers to take extra action whenevever an
...
public:  // should be protected, but some old compilers complain otherwise
    ...
    public:
        ...
        public:
            ParamsForREGISTER(char const* cmd /*"REGISTER" or "DEREGISTER"*/,
                              RTSPClientConnection* ourConnection,
    ...
    protected:
        ...
        // subclasses to reimplement them, if necessary:
        ...
        virtual void handleCmd_GET_PARAMETER(
            char const* fullRequestStr);  // when operating on the entire server
        virtual void handleCmd_SET_PARAMETER(
            char const* fullRequestStr);  // when operating on the entire server
        virtual void handleCmd_DESCRIBE(char const* urlPreSuffix,
                                        char const* urlSuffix,
                                        char const* fullRequestStr);
        static void DESCRIBELookupCompletionFunction(
            void* clientData, ServerMediaSession* sessionLookedUp);
        virtual void handleCmd_DESCRIBE_afterLookup(
    ...
    protected:
        ...
        Authenticator
            fCurrentAuthenticator;  // used if access control is needed
    ...
    protected:
        ...
        virtual void handleCmd_SETUP(RTSPClientConnection* ourClientConnection,
                                     ...
                                     char const* urlSuffix,
                                     char const* fullRequestStr);
        static void SETUPLookupCompletionFunction1(
            void* clientData, ServerMediaSession* sessionLookedUp);
        virtual void handleCmd_SETUP_afterLookup1(ServerMediaSession* sms);
        static void SETUPLookupCompletionFunction2(
            void* clientData, ServerMediaSession* sessionLookedUp);
...
protected:  // redefined virtual functions
    ...
    // virtual function in order to create new objects of your subclass:
...
private:
    ...
    int fHTTPServerSocketIPv4,
        fHTTPServerSocketIPv6;  // for optional RTSP-over-HTTP tunneling
    Port fHTTPServerPort;       // ditto
    HashTable*
        fClientConnectionsForHTTPTunneling;  // maps client-supplied 'session
                                             // cookie' strings to
                                             // "RTSPClientConnection"s (used
                                             // only for optional RTSP-over-HTTP
                                             // tunneling)
    HashTable* fTCPStreamingDatabase;
    // maps TCP socket numbers to ids of sessions that are streaming over it
    // (RTP/RTCP-over-TCP)
    ...
    clientConnectCallback fClientConnectCallback;
};
...
public:
    static RTSPServerWithREGISTERProxying* createNew(
        ...
        unsigned reclamationSeconds = 65, Boolean streamRTPOverTCP = False,
        int verbosityLevelForProxying = 0, char const* backEndUsername = NULL,
...
protected:
    RTSPServerWithREGISTERProxying(
        ...
        unsigned reclamationSeconds, Boolean streamRTPOverTCP,
        int verbosityLevelForProxying, char const* backEndUsername,
