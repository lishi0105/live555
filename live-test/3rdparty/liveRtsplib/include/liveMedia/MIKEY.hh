...
public:
    MIKEYState(
        Boolean useEncryption = True);  // initialize with default parameters
    virtual ~MIKEYState();

    static MIKEYState* createNew(u_int8_t const* messageToParse,
                                 unsigned messageSize);
    // (Attempts to) parse a binary MIKEY message, returning a new "MIKEYState"
    // if successful (or NULL if unsuccessful).

    u_int8_t* generateMessage(unsigned& messageSize) const;
    // Returns a binary message representing the current MIKEY state, of size
    // "messageSize" bytes. This array is dynamically allocated by this routine,
...
private:
    MIKEYState(u_int8_t const* messageToParse, unsigned messageSize,
               Boolean& parsedOK);
    // called only by "createNew()"

    void addNewPayload(class MIKEYPayload* newPayload);
    Boolean parseHDRPayload(u_int8_t const*& ptr, u_int8_t const* endPtr,
                            u_int8_t& nextPayloadType);
    Boolean parseNonHDRPayload(u_int8_t const*& ptr, u_int8_t const* endPtr,
                               u_int8_t& nextPayloadType);

private:
    // Encryption/authentication parameters, either set by default
    // (if the first (parameterless) constructor is used), or set by parsing an
    // input message (if the second constructor is used):
    Boolean fEncryptSRTP;
    Boolean fEncryptSRTCP;
    u_int8_t fKeyData[16 + 14];  // encryption key + salt
    u_int32_t fMKI;  // used only if encryption is used. (We assume a MKI length
                     // of 4.)
