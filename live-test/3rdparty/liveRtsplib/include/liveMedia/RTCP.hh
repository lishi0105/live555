...
typedef void RTCPAppHandlerFunc(void* clientData, u_int8_t subtype,
                                u_int32_t nameBytes /*big-endian order*/,
...
public:
    ...
    // (To remove an existing "BYE" handler, call "setByeHandler()" again, with
    // a "handlerTask" of NULL.)
    void setByeWithReasonHandler(ByeWithReasonHandlerFunc* handlerTask,
                                 void* clientData,
                                 Boolean handleActiveParticipantsOnly = True);
    // Like "setByeHandler()", except that a string 'reason for the bye'
    // (received as part of the RTCP "BYE" packet) is passed to the handler
    // function (along with "clientData"). (The 'reason' parameter to the
    // handler function will be a dynamically-allocated string, or NULL, and
    // should be delete[]d by the handler function.)
    ...
    void setSpecificRRHandler(struct sockaddr_storage const& fromAddress,
                              Port fromPort, TaskFunc* handlerTask,
                              void* clientData);
    ...
    // Assigns a handler routine to be called whenever an "APP" packet arrives.
    // (To turn off handling, call the function again with "handlerTask" (and
    // "clientData") as NULL.)
    void sendAppPacket(u_int8_t subtype, char const* name,
                       u_int8_t* appDependentData,
                       unsigned appDependentDataSize);
    // Sends a custom RTCP "APP" packet to the peer(s).  The parameters
    // correspond to their respective fields as described in the RTP/RTCP
    // definition (RFC 3550). Note that only the low-order 5 bits of "subtype"
    // are used, and only the first 4 bytes of "name" are used.  (If "name" has
    // fewer than 4 bytes, or is NULL, then the remaining bytes are '\0'.)
    ...
    void injectReport(u_int8_t const* packet, unsigned packetSize,
                      struct sockaddr_storage const& fromAddress);
    // Allows an outside party to inject an RTCP report (from other than the
    // network interface)
...
protected:
    ...
    virtual void noteArrivingRR(
        struct sockaddr_storage const& fromAddressAndPort, int tcpSocketNum,
...
private:
    ...
    void processIncomingReport(
        unsigned packetSize, struct sockaddr_storage const& fromAddressAndPort,
...
const unsigned char RTCP_PT_AVB =
    208;  // AVB RTCP packet ["Standard for Layer 3 Transport Protocol for Time
          // Sensitive Applications in Local Area Networks." Work in progress.]
const unsigned char RTCP_PT_RSI =
    209;  // Receiver Summary Information [RFC5760]
