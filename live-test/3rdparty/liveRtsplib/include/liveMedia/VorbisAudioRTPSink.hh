...
public:
    ...
    // 'configuration' string, rather than the raw configuration headers as
    // parameter.
...
private:
    u_int32_t fIdent;  // "Ident" field used by this stream.  (Only the low 24
                       // bits of this are used.)
    char* fFmtpSDPLine;
};

// A general function used by both "VorbisAudioRTPSink" and "TheoraVideoRTPSink"
// to construct a Base64-encoded 'config' string (for SDP) from
// "identification", "comment", "setup" headers. (Note: The result string was
// heap-allocated, and the caller should delete[] it afterwards.)

char* generateVorbisOrTheoraConfigStr(
    u_int8_t* identificationHeader, unsigned identificationHeaderSize,
