...
public:
    ...
    void setOnSendErrorFunc(onSendErrorFunc* onSendErrorFunc,
        ...
        // Can be used to set a callback function to be called if there's an
        // error sending RTP packets on our socket.
...
protected:
    ...
    // returns the number of overflow bytes that would be produced by adding a
    // new frame of size "newFrameSize" to the current RTP packet. (By default,
    // this just calls "numOverflowBytes()", but subclasses can redefine this to
    // (e.g.) impose a granularity upon RTP payload fragments.)
...
private:
    ...
    unsigned fCurFrameSpecificHeaderSize;     // size in bytes of cur
                                              // frame-specific header
    unsigned fTotalFrameSpecificHeaderSizes;  // size of all frame-specific hdrs
                                              // in pkt
