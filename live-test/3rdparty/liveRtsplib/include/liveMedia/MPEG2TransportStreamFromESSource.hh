...
class MPEG2TransportStreamFromESSource
    : public MPEG2TransportStreamMultiplexor {
public:
    ...
    // Note: In these functions, if "PID" is not -1, then it (currently, just
    // the low 8 bits) is used as the stream's PID.  Otherwise (if "PID" is -1)
    // the 'stream_id' is used as the PID.
...
protected:
    ...
    void addNewInputSource(FramedSource* inputSource, u_int8_t streamId,
