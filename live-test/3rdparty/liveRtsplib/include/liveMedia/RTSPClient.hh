...
public:
    static RTSPClient* createNew(UsageEnvironment& env, char const* rtspURL,
                                 ...
                                 Boolean bRtpOverTcp = False);
    // If "tunnelOverHTTPPortNum" is non-zero, we tunnel RTSP (and RTP)
    //     over a HTTP connection with the given port number, using the
    //     technique described in Apple's document
    ...
    // are as follows:
    //     "rtspClient": The "RTSPClient" object on which the original command
    //     was issued. "resultCode": If zero, then the command completed
    //     successfully.  If non-zero, then the command did not complete
    //         successfully, and "resultCode" indicates the error, as follows:
    //             A positive "resultCode" is a RTSP error code (for example,
    //             404 means "not found") A negative "resultCode" indicates a
    //             socket/network error; 0-"resultCode" is the standard "errno"
    //             code.
    //     "resultString": A ('\0'-terminated) string returned along with the
    //     response, or else NULL.
    //         In particular:
    //             "resultString" for a successful "DESCRIBE" command will be
    //             the media session's SDP description. "resultString" for a
    //             successful "OPTIONS" command will be a list of allowed
    //             commands.
    //         Note that this string can be present (i.e., not NULL) even if
    //         "resultCode" is non-zero - i.e., an error message. Also,
    //         "resultString" can be NULL, even if "resultCode" is zero (e.g.,
    //         if the RTSP command succeeded, but without
    ...
    // that was used in the command. The (programmer-supplied) "responseHandler"
    // function is called later to handle the response
    //     (or is called immediately - with an error code - if the command
    //     cannot be sent).
    ...
    //     "Authenticator(username, password) object". (Note that if you supply
    //     a non-NULL "authenticator" parameter, you need do this only for the
    //     first command you send.)
    ...
    // that was used in the command. (The "responseHandler" and "authenticator"
    // parameters are as described for "sendDescribeCommand".)
    ...
    // (The "responseHandler" and "authenticator" parameters are as described
    // for "sendDescribeCommand".)

    unsigned sendSetupCommand_v1(MediaSubsession& subsession,
                                 responseHandler* responseHandler,
    ...
    unsigned sendSetupCommand(MediaSubsession& subsession,
                              ...
                              Boolean forceMulticastOnUnspecified = False,
                              Authenticator* authenticator = NULL);
    // Issues a RTSP "SETUP" command, then returns the "CSeq" sequence number
    // that was used in the command. (The "responseHandler" and "authenticator"
    // parameters are as described for "sendDescribeCommand".)
    ...
    // "CSeq" sequence number that was used in the command. (Note: start=-1
    // means 'resume'; end=-1 means 'play to end') (The "responseHandler" and
    // "authenticator" parameters are as described for "sendDescribeCommand".)
    ...
    // sequence number that was used in the command. (Note: start=-1 means
    // 'resume'; end=-1 means 'play to end') (The "responseHandler" and
    // "authenticator" parameters are as described for "sendDescribeCommand".)

    // Alternative forms of "sendPlayCommand()", used to send "PLAY" commands
    // that include an 'absolute' time range: (The "absStartTime" string (and
    // "absEndTime" string, if present) *must* be of the form
    ...
    // "CSeq" sequence number that was used in the command. (The
    // "responseHandler" and "authenticator" parameters are as described for
    // "sendDescribeCommand".)
    unsigned sendPauseCommand(MediaSubsession& subsession,
                              responseHandler* responseHandler,
                              Authenticator* authenticator = NULL);
    // Issues a RTSP "PAUSE" command on "subsession", then returns the "CSeq"
    // sequence number that was used in the command. (The "responseHandler" and
    // "authenticator" parameters are as described for "sendDescribeCommand".)
    ...
    // "responseHandler" and "authenticator" parameters are as described for
    // "sendDescribeCommand".)
    unsigned sendRecordCommand(MediaSubsession& subsession,
                               responseHandler* responseHandler,
                               Authenticator* authenticator = NULL);
    ...
    unsigned sendTeardownCommand(MediaSession& session,
                                 responseHandler* responseHandler,
                                 Authenticator* authenticator = NULL);
    // Issues an aggregate RTSP "TEARDOWN" command on "session", then returns
    // the "CSeq" sequence number that was used in the command. (The
    // "responseHandler" and "authenticator" parameters are as described for
    // "sendDescribeCommand".)
    ...
    unsigned sendSetParameterCommand(MediaSession& session,
                                     ...
                                     Authenticator* authenticator = NULL);
    // Issues an aggregate RTSP "SET_PARAMETER" command on "session", then
    // returns the "CSeq" sequence number that was used in the command. (The
    // "responseHandler" and "authenticator" parameters are as described for
    // "sendDescribeCommand".)
    ...
    unsigned sendGetParameterCommand(MediaSession& session,
                                     ...
                                     char const* parameterName,
                                     Authenticator* authenticator = NULL);
    // Issues an aggregate RTSP "GET_PARAMETER" command on "session", then
    // returns the "CSeq" sequence number that was used in the command. (The
    // "responseHandler" and "authenticator" parameters are as described for
    // "sendDescribeCommand".)

    void setRequireValue(char const* requireValue = NULL);
    // Sets a string to be used as the value of a "Require:" header to be
    // included in subsequent RTSP commands.  Call "setRequireValue()" again
    // (i.e., with no parameter) to clear this (and so stop "Require:" headers
    // from being included in subsequent cmds).

    void sendDummyUDPPackets(MediaSession& session,
                             unsigned numDummyPackets = 2);
    void sendDummyUDPPackets(MediaSubsession& subsession,
                             unsigned numDummyPackets = 2);
    // Sends short 'dummy' (i.e., non-RTP or RTCP) UDP packets towards the
    // server, to increase the likelihood of RTP/RTCP packets from the server
    // reaching us if we're behind a NAT. (If we requested RTP-over-TCP
    // streaming, then these functions have no effect.) Our implementation
    // automatically does this just prior to sending each "PLAY" command; You
    // should not call these functions yourself unless you know what you're
    // doing.

    void setSpeed(MediaSession& session, float speed = 1.0f);
    // Set (recorded) media download speed to given value to support faster
    // download using 'Speed:' option on 'PLAY' command.

    Boolean changeResponseHandler(unsigned cseq,
                                  responseHandler* newResponseHandler);
    // Changes the response handler for the previously-performed command (whose
    // operation returned "cseq"). (To turn off any response handling for the
    // command, use a "newResponseHandler" value of NULL.  This might be done as
    // part
    //  of an implementation of a 'timeout handler' on the command, for
    //  example.)
    // This function returns True iff "cseq" was for a valid
    // previously-performed command (whose response is still unhandled).
    ...
    // (which would cause the client to send usernames and passwords over the
    // net).
...
public:  // Some compilers complain if this is "private:"
    ...
    public:
        RequestRecord(unsigned cseq, char const* commandName,
                      ...
                      u_int32_t booleanFlags = 0, double start = 0.0f,
                      double end = -1.0f, float scale = 1.0f,
    ...
    private:
        ...
        double fStart, fEnd;
        char *fAbsStartTime,
            *fAbsEndTime;  // used for optional 'absolute' (i.e., "time=") range
                           // specifications
        float fScale;
        char* fContentStr;
...
protected:
    RTSPClient(UsageEnvironment& env, char const* rtspURL, int verbosityLevel,
               ...
               int socketNumToServer, Boolean bRtpOverTcp = False);
    ...
    virtual int connectToServer(
        int socketNum,
        portNumBits
            remotePortNum);  // used to implement "openConnection()"; result
                             // values: -1: failure; 0: pending; 1: success
...
private:
    ...
    public:
        RequestQueue();
        RequestQueue(RequestQueue& origQueue);  // moves the queue contents to
                                                // the new queue
    ...
    char* createKeyMgmtString(char const* url,
    ...
    Boolean parseRTPInfoParams(char const*& paramStr, u_int16_t& seqNum,
                               u_int32_t& timestamp);
    Boolean handleSETUPResponse(MediaSubsession& subsession,
    ...
    Boolean handleTEARDOWNResponse(MediaSession& session,
                                   MediaSubsession& subsession);
    Boolean handleGET_PARAMETERResponse(char const* parameterName,
...
public:
    ...
    // If set to a value >0, then a "Blocksize:" header with this value (minus
    // an allowance for IP, UDP, and RTP headers) will be sent with each "SETUP"
    // request.
...
// request arrives (specifying the "rtsp://" URL of a stream).  The new
// "RTSPClient" object will be created with the specified URL, and passed to the
...
public:
    static HandlerServerForREGISTERCommand* createNew(
...
protected:
    ...
    virtual RTSPClient* createNewRTSPClient(char const* rtspURL,
                                            int verbosityLevel,
                                            char const* applicationName,
                                            int socketNumToServer,
                                            Boolean bRtpOverTcp = False);
    // This function - by default - creates a (base) "RTSPClient" object.  If
    // you want to create a subclass of "RTSPClient" instead, then subclass this
    // class, and redefine this virtual function.

protected:                                      // redefined virtual functions
    virtual char const* allowedCommandNames();  // "OPTIONS", "REGISTER", and
                                                // (perhaps) "DEREGISTER" only
    ...
    virtual void implementCmd_REGISTER(
