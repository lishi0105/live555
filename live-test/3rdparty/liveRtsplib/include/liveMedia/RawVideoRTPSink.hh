...
public:
    static RawVideoRTPSink* createNew(
        UsageEnvironment& env, Groupsock* RTPgs, u_int8_t rtpPayloadFormat,
        unsigned width, unsigned height,
        unsigned depth,  // as defined by RFC 4175, sec 6.1
        char const* sampling, char const* colorimetry);

protected:
    RawVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs,
                    u_int8_t rtpPayloadFormat, unsigned width, unsigned height,
                    unsigned depth, char const* sampling,
...
private:
    ...
    unsigned getNumLinesInPacket(unsigned fragOffset, u_int16_t*& lengths,
                                 u_int16_t*& offsets) const;
    //  return the number of lines, their lengths and offsets from the
    //  fragmentation offset of the whole frame.
