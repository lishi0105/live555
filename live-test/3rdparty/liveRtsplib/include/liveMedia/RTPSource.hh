...
public:
    ...
    // Note: This is the SSRC in the most recently received RTP packet; not
    // *our* SSRC
    ...
    // following two functions, because RTP sequence numbers and timestamps are
    // usually not useful to receivers. (Our implementation of RTP reception
    // already does all needed handling of RTP sequence numbers and timestamps.)
    u_int16_t curPacketRTPSeqNum() const { return fCurPacketRTPSeqNum; }

private:
    friend class MediaSubsession;  // "MediaSubsession" is the only outside
                                   // class that ever needs to see RTP
                                   // timestamps!
...
public:
    ...
    void noteIncomingSR(u_int32_t SSRC, u_int32_t ntpTimestampMSW,
