...
public:
    ...
    // Authenticate (if necessary) and decrypt (if necessary) incoming SRTP and
    // SRTCP packets. Returns True iff the packet is well-formed and
    // authenticates OK.
    ...
    // outgoing RTP and RTCP packet. Returns True iff the packet is well-formed.
...
#define SRTP_AUTH_TAG_LENGTH (80 / 8)      // in bytes
    ...
    Boolean verifySRTPAuthenticationTag(u_int8_t* dataToAuthenticate,
                                        unsigned numBytesToAuthenticate,
                                        u_int32_t roc,
                                        u_int8_t const* authenticationTag);
    Boolean verifySRTCPAuthenticationTag(u_int8_t const* dataToAuthenticate,
                                         unsigned numBytesToAuthenticate,
                                         u_int8_t const* authenticationTag);
    ...
    unsigned generateAuthenticationTag(derivedKeys& keysToUse,
                                       ...
                                       u_int8_t* resultAuthenticationTag);
    // returns the size of the resulting authentication tag
    // "resultAuthenticationTag" must point to an array of at least
    ...
    void deriveSingleKey(u_int8_t const* masterKey, u_int8_t const* salt,
                         SRTPKeyDerivationLabel label, unsigned resultKeyLength,
                         u_int8_t* resultKey);
...
private:
    ...
    u_int8_t const* masterSalt() const {
        return &masterKeyPlusSalt()[SRTP_CIPHER_KEY_LENGTH];
    }
