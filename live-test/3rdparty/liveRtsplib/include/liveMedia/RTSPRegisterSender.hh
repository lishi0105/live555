...
// "DEREGISTER") command to a specified client. C++ header
...
public:  // Some compilers complain if this is "protected:"
    // A subclass of "RTSPClient::RequestRecord", specific to our "REGISTER" and
    // "DEREGISTER" commands:
    class RequestRecord_REGISTER_or_DEREGISTER
        : public RTSPClient::RequestRecord {
    public:
...
public:
    static RTSPRegisterSender* createNew(
        ...
        char const* proxyURLSuffix = NULL, Boolean reuseConnection = False,
        int verbosityLevel = 0, char const* applicationName = NULL);

    void grabConnection(int& sock,
                        struct sockaddr_storage&
                            remoteAddress);  // so that the socket doesn't get
                                             // closed when we're deleted
...
public:  // Some compilers complain if this is "protected:"
    // A subclass of "RequestRecord_REGISTER_or_DEREGISTER", specific to our
    // "REGISTER" command:
    class RequestRecord_REGISTER : public RTSPRegisterOrDeregisterSender::
                                       RequestRecord_REGISTER_or_DEREGISTER {
    public:
        RequestRecord_REGISTER(unsigned cseq,
                               RTSPClient::responseHandler* rtspResponseHandler,
                               char const* rtspURLToRegister,
        ...
        char const* rtspURLToRegister() const {
            return fRTSPURLToRegisterOrDeregister;
        }
...
public:
    static RTSPDeregisterSender* createNew(
        UsageEnvironment& env, char const* remoteClientNameOrAddress,
        portNumBits remoteClientPortNum, char const* rtspURLToDeregister,
        RTSPClient::responseHandler* rtspResponseHandler,
        Authenticator* authenticator = NULL, char const* proxyURLSuffix = NULL,
        int verbosityLevel = 0, char const* applicationName = NULL);
...
protected:
    RTSPDeregisterSender(UsageEnvironment& env,
                         ...
                         RTSPClient::responseHandler* rtspResponseHandler,
                         Authenticator* authenticator,
                         char const* proxyURLSuffix, int verbosityLevel,
                         char const* applicationName);
    // called only by "createNew()"
...
public:  // Some compilers complain if this is "protected:"
    // A subclass of "RequestRecord_REGISTER_or_DEREGISTER", specific to our
    // "DEREGISTER" command:
    class RequestRecord_DEREGISTER : public RTSPRegisterOrDeregisterSender::
                                         RequestRecord_REGISTER_or_DEREGISTER {
    public:
        RequestRecord_DEREGISTER(
            unsigned cseq, RTSPClient::responseHandler* rtspResponseHandler,
