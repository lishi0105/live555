...
// process RTSP commands that may also appear within the same TCP connection.  A
// RTSP server implementation would supply a function like this - as a parameter
// to "ServerMediaSubsession::startStream()".
...
public:
    ...
    static void setServerRequestAlternativeByteHandler(
    ...
    // Note: If "tcpSocketNum" < 0, then the packet was received over UDP, and
    // "tcpStreamChannelId"
    //   is undefined (and irrelevant).

    // Otherwise (if "tcpSocketNum" >= 0), the packet was received (interleaved)
    // over TCP, and
    ...
    // This may be called - *only immediately prior* to deleting this - to
    // prevent our destructor from turning off background reading on the
    // 'groupsock'.  (This is in case the 'groupsock' is also being read from
    // elsewhere.)
...
private:
    ...
    Boolean sendRTPorRTCPPacketOverTCP(unsigned char* packet,
                                       unsigned packetSize, int socketNum,
...
private:
    ...
    class tcpStreamRecord*
        fTCPStreams;  // optional, for RTP-over-TCP streaming/receiving
