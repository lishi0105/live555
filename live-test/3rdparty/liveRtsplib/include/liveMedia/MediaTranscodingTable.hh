...
public:
    virtual FramedFilter* lookupTranscoder(
        MediaSubsession& /*inputCodecDescription*/,  // in
        char*& outputCodecName /* out; must be delete[]d later */) {
        // Default implementation: Return NULL (indicating: no transcoding).
        // You would reimplement this virtual function in a subclass to return a
        // new 'transcoding' "FramedFilter" (subclass) object for each
        // ("mediumName","codecName") that you wish to transcode (or return NULL
        // for no transcoding). (Note that "inputCodecDescription" must have a
        // non-NULL "readSource()"; this is used
    ...
    virtual Boolean weWillTranscode(char const* /*mediumName*/,
        ...
        // Note: Unlike "lookupTranscoder()", this function does not actually
        // create any 'transcoding' filter objects.  (It may be called before
        // "MediaSubsession::initiate()".)
        return False;
    }

protected:  // we are to be subclassed only
    MediaTranscodingTable(UsageEnvironment& env) : Medium(env) {}
