...
// Typedef for a handler function that gets called when
// "lookupServerMediaSession()" (defined below) completes:
typedef void lookupServerMediaSessionCompletionFunc(
...
public:
    ...
    virtual void lookupServerMediaSession(
        ...
        void* completionClientData, Boolean isFirstLookupInSession = True);
    // Note: This is a virtual function, so can be reimplemented by subclasses.
    void lookupServerMediaSession(
        char const* streamName,
        void (GenericMediaServer::*memberFunc)(ServerMediaSession*));
    // Special case of "lookupServerMediaSession()" where the 'completion
    // function' is a member function of "GenericMediaServer" (and the
    // 'completion client data' is "this".)

    void removeServerMediaSession(ServerMediaSession* serverMediaSession);
    // Removes the "ServerMediaSession" object from our lookup table, so it will
    // no longer be accessible by new clients. (However, any *existing* client
    // sessions that use this "ServerMediaSession" object will continue
    // streaming.
    //  The "ServerMediaSession" object will not get deleted until all of these
    //  client sessions have closed.)
    // (To both delete the "ServerMediaSession" object *and* close all client
    ...
    // Closes (from the server) all client sessions that are currently using
    // this "ServerMediaSession" object. Note, however, that the
    // "ServerMediaSession" object remains accessible by new clients.
    virtual void closeAllClientSessionsForServerMediaSession(
        char const* streamName);
    // ditto
...
protected:
    GenericMediaServer(UsageEnvironment& env, int ourSocketIPv4,
                       int ourSocketIPv6, Port ourPort,
                       unsigned reclamationSeconds);
    // If "reclamationSeconds" > 0, then the "ClientSession" state for each
    // client will get reclaimed if no activity from the client is detected in
    // at least "reclamationSeconds".
...
public:  // should be protected, but some old compilers complain otherwise
    ...
    protected:
        ...
        friend class RTSPServer;  // needed to make some broken Windows
                                  // compilers work; remove this in the future
                                  // when we end support for Windows
...
protected:
    ...
    // Generates a new (unused) random session id, and calls the
    // "createNewClientSession()" virtual function with this session id as
    // parameter.
    ...
    // The basic, synchronous "ServerMediaSession" lookup operation; only for
    // subclasses:
...
protected:
    ...
    HashTable*
        fClientConnections;  // the "ClientConnection" objects that we're using
    HashTable* fClientSessions;  // maps 'session id' strings to "ClientSession"
                                 // objects
...
public:
    ...
    // from, the database is actually the value computed by
    // md5(<username>:<realm>:<actual-password>)
