...
public:
    static AC3AudioStreamFramer* createNew(UsageEnvironment& env,
                                           FramedSource* inputSource,
                                           unsigned char streamCode = 0);
    // If "streamCode" != 0, then we assume that there's a 1-byte code at the
    // beginning of each chunk of data that we read from our source.  If that
    // code is not the value we want, we discard the chunk of data. However, if
    // "streamCode" == 0 (the default), then we don't expect this 1-byte code.

    unsigned samplingRate();

    void
    flushInput();  // called if there is a discontinuity (seeking) in the input
...
private:
    ...
    static void handleNewData(void* clientData, unsigned char* ptr,
