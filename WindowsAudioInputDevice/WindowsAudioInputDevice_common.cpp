...
WindowsAudioInputDevice_common ::WindowsAudioInputDevice_common(
    ...
    : AudioInputDevice(env, bitsPerSample, numChannels, samplingFrequency,
                       granularityInMS),
      fCurPortIndex(-1),
      fHaveStarted(False) {
...
Boolean WindowsAudioInputDevice_common::initialSetInputPort(int portIndex) {
    if (!setInputPort(portIndex)) {
        ...
        sprintf(errMsgPrefix,
                "Failed to set audio input port number to %d: ", portIndex);
...
void WindowsAudioInputDevice_common::audioReadyPoller(void* clientData) {
    WindowsAudioInputDevice_common* inputDevice =
        (WindowsAudioInputDevice_common*)clientData;
...
void WindowsAudioInputDevice_common::audioReadyPoller1() {
    ...
    if (readHead != NULL) {
        ...
        nextTask() = envir().taskScheduler().scheduleDelayedTask(
            uSecondsToDelay, (TaskFunc*)audioReadyPoller, this);
    }
}
...
Boolean WindowsAudioInputDevice_common::openWavInPort(
    int index, unsigned numChannels, unsigned samplingFrequency,
    unsigned granularityInMS) {
    uSecsPerByte =
    ...
    // etc., and also for some audio cards that buffer as much as 5 seconds of
    // audio.
    ...
    // needed, but the "rat" code does this:
    SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS);
    return True;
}

Boolean WindowsAudioInputDevice_common::waveIn_open(unsigned uid,
                                                    WAVEFORMATEX& wfx) {
    if (shWaveIn != NULL) return True;  // already open

    do {
        waveIn_reset();
        if (waveInOpen(&shWaveIn, uid, &wfx, (DWORD)waveInCallback, 0,
        ...
        for (unsigned i = 0; i < numBlocks; ++i) {
            ...
            if (waveInPrepareHeader(shWaveIn, &readHdrs[i], sizeof(WAVEHDR)) !=
                MMSYSERR_NOERROR)
                break;
            if (waveInAddBuffer(shWaveIn, &readHdrs[i], sizeof(WAVEHDR)) !=
                MMSYSERR_NOERROR)
                break;
        }
...
unsigned WindowsAudioInputDevice_common::readFromBuffers(
    unsigned char* to, unsigned numBytesWanted, struct timeval& creationTime) {
    // Begin by computing the creation time of (the first bytes of) this
    // returned audio data:
    if (readHead != NULL) {
        int hdrIndex = readHead - readHdrs;
        creationTime = readTimes[hdrIndex];

        // Adjust this time to allow for any data that's already been read from
        // this buffer:
        if (bytesUsedAtReadHead > 0) {
            creationTime.tv_usec +=
                (unsigned)(uSecsPerByte * bytesUsedAtReadHead);
            creationTime.tv_sec += creationTime.tv_usec / 1000000;
            creationTime.tv_usec %= 1000000;
        }
    }

    // Then, read from each available buffer, until we have the data that we
    // want:
    unsigned numBytesRead = 0;
    while (readHead != NULL && numBytesRead < numBytesWanted) {
        unsigned thisRead = min(readHead->dwBytesRecorded - bytesUsedAtReadHead,
                                numBytesWanted - numBytesRead);
        memmove(&to[numBytesRead], &readHead->lpData[bytesUsedAtReadHead],
                thisRead);
