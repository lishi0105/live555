...
public:
    ...
    void open(unsigned numChannels, unsigned samplingFrequency,
              unsigned granularityInMS);
    void open();  // open with default parameters
    void getPortsInfo();
    Boolean enableInputPort(unsigned portIndex, char const*& errReason,
                            MMRESULT& errCode);
    void close();
...
AudioInputDevice* AudioInputDevice::createNew(UsageEnvironment& env,
                                              int inputPortNumber,
    ...
    WindowsAudioInputDevice* newSource = new WindowsAudioInputDevice(
        env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency,
        granularityInMS, success);
    if (!success) {
        delete newSource;
        newSource = NULL;
    }
...
AudioPortNames* AudioInputDevice::getPortNames() {
    ...
    portNames->portName =
        new char*[WindowsAudioInputDevice::numInputPortsTotal];

    // If there's more than one mixer, print only the port name.
    // If there's two or more mixers, also include the mixer name
    // (to disambiguate port names that may be the same name in different
    // mixers)
	...
	if (mixerNameBuffer[k] == ' ') {
	  ...
	  break;
        ...
        for (unsigned j = 0; j < mixer.numPorts; ++j) {
            sprintf(portNameBuffer, portNameFmt, mixer.ports[j].name,
                    mixerNameBuffer);
...
WindowsAudioInputDevice ::WindowsAudioInputDevice(
    UsageEnvironment& env, int inputPortNumber, unsigned char bitsPerSample,
    unsigned char numChannels, unsigned samplingFrequency,
    unsigned granularityInMS, Boolean& success)
    : WindowsAudioInputDevice_common(env, inputPortNumber, bitsPerSample,
...
Boolean WindowsAudioInputDevice::setInputPort(int portIndex) {
    ...
    if (allowedDeviceNames != NULL) {
        ...
        if (allowedDeviceNames[i] ==
            NULL) {  // this mixer is not on the allowed list
            envir().setResultMsg(
                "Access to this audio device is not allowed\n");
            return False;
        }
    }
    ...
    if (newMixerId != fCurMixerId) {
        ...
        ourMixers[fCurMixerId].open(fNumChannels, fSamplingFrequency,
                                    fGranularityInMS);
    }
    if (portIndex != fCurPortIndex) {
        ...
        MMRESULT errCode;
        if (!ourMixers[newMixerId].enableInputPort(portWithinMixer, errReason,
                                                   errCode)) {
            char resultMsg[100];
            sprintf(resultMsg,
                    "Failed to enable input port: %s failed (0x%08x)\n",
                    errReason, errCode);
...
Mixer::~Mixer() { delete[] ports; }

void Mixer::open(unsigned numChannels, unsigned samplingFrequency,
                 unsigned granularityInMS) {
    ...
    do {
        ...
        if (mixerOpen(&newHMixer, index, (unsigned long)NULL,
                      (unsigned long)NULL,
                      MIXER_OBJECTF_MIXER) != MMSYSERR_NOERROR)
            break;
        if (newHMixer == NULL) break;

        // Sanity check: re-call "mixerGetDevCaps()" using the mixer device
        // handle:
        if (mixerGetDevCaps((UINT)newHMixer, &mc, sizeof mc) !=
            MMSYSERR_NOERROR)
            break;
        if (mc.cDestinations < 1)
            break;  // error: this mixer has no destinations

        if (!WindowsAudioInputDevice_common::openWavInPort(
                uWavIn, numChannels, samplingFrequency, granularityInMS))
            break;
...
void Mixer::open() { open(1, 8000, 20); }
...
void Mixer::getPortsInfo() {
    ...
    for (i = 0; i < mc.cDestinations; ++i) {
        ...
        mlt.dwDestination = i;
        if (mixerGetLineInfo((HMIXEROBJ)hMixer, &mlt,
    ...
    for (i = 0; i < numPorts; ++i) {
        ...
        mlc.dwSource = i;
        mixerGetLineInfo((HMIXEROBJ)hMixer, &mlc,
...
Boolean Mixer::enableInputPort(unsigned portIndex, char const*& errReason,
                               MMRESULT& errCode) {
  ...
  if ((errCode = mixerGetLineControls((HMIXEROBJ)hMixer, &mlc, MIXER_GETLINECONTROLSF_ONEBYTYPE/*|MIXER_OBJECTF_HMIXER*/)) != MMSYSERR_NOERROR) {
    ...
    if ((errCode = mixerGetLineInfo((HMIXEROBJ)hMixer, &ml,
    ...
    if ((errCode = mixerGetLineControls(
             (HMIXEROBJ)hMixer, &mlc,
    ...
    if (mc.cMultipleItems > 1) {
        ...
        MIXERCONTROLDETAILS_LISTTEXT* mcdlText =
            new MIXERCONTROLDETAILS_LISTTEXT[mc.cMultipleItems];
        ...
        if (mc.dwControlID != 0xDEADBEEF) {  // we know the control id for real
            ...
            // Hack: We couldn't find a MUX or MIXER control, so try to guess
            // the control id:
    ...
    MIXERCONTROLDETAILS_BOOLEAN* mcdbState =
        new MIXERCONTROLDETAILS_BOOLEAN[mc.cMultipleItems];
    ...
    if ((errCode = mixerSetControlDetails((HMIXEROBJ)hMixer, &mcd,
